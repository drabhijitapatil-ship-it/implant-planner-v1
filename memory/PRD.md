...
- **3 new Straumann BLT (Bone Level Tapered) systems** wired end-to-end:
  ...
  - `BLT Roxolid SLA` — 21 (Ø, L) combinations
  - `BLT Ti SLA` — 18 (Ø, L) combinations
- **Legacy 4-row 'BLT' stub removed** from `implant_library_data.py`; replaced by the full 63-row matrix.
- **Files added**: `/app/backend/straumann_blt_data.py` (size matrix + drilling generator), `/app/backend/_seed_straumann_blt.py` (idempotent startup seed).
- **`server.py` wiring**: `IMPLANT_INDICATIONS` entries (3), `DRILLING_PROTOCOLS` entries (3), dispatcher branch in `POST /api/drilling-protocols/generate`, `protocol_type` label, `insertion_torque` ('≤35 Ncm — check bed if >35 reached early'), and startup-seed hook beside BLX/Adin.
- **Drilling protocol schema** strictly matches frontend: `{step, drill_type, code, diameter, depth, cortical_only, rpm, irrigation, note}`. 8 steps (D2) → 9 steps (D1 dense).
- **Verified live**:
  - `GET /api/implant-library/systems` → 7 Straumann systems (3 BLT + 4 BLX), no legacy stub.
  - `GET /api/drilling-protocols/available` → 3 BLT systems listed.
  - `POST /api/drilling-protocols/generate` → valid 8-9 step protocols for all 3 BLT × D1/D4 bone densities.
- **Test report**: `/app/test_reports/iteration_292.json` (100% pass).
...
## Iteration 324 (Jun 2026) — e-signature black screen on deployed Expo Go (continued)
User confirmed: redeployed after SVG fix, STILL plain black screen (no text) on tapping e-Sign; Expo Go via QR. Evidence gathered: deployed host (implant-app-r-1774251381.emergent.host) serves ONLY backend API at root; consent-texts endpoint live; route file committed & not gitignored; iOS entry bundle registers consent-sign route; local iOS bundling succeeds; react-native-svg used in tab bar (works natively); no auth route guard; screen-capture guard is app-wide/pre-existing. Preview pod runs expo --lan (tunnel intentionally disabled per apply.sh — ngrok stale tunnels crash expo), so no native repro possible from workspace. Black screen with NO redbox suggests production-mode JS error (no LogBox in prod) or native-stack quirk with unregistered route.
CHANGES: (1) registered procedures/consent-sign/[id] in root Stack (was the ONLY user-facing pushed procedure route not registered), (2) exported expo-router ErrorBoundary from the route — any error now renders a readable message + Try Again/Go Back instead of black (works in production mode too; testID consent-sign-error-boundary), (3) mount console.log breadcrumb.
PENDING USER VERIFICATION: redeploy → tap e-Sign → either it works, or an on-screen error message appears which the user should report verbatim.

## Iteration 325 (Jun 2026) — TRUE ROOT CAUSE of e-signature native crash: FIXED
User's ErrorBoundary photo revealed: "Cannot read property 'layout' of null".
ROOT CAUSE: consent-sign screen's `onLayout={e => setPadSize(p => ({...p, w: e.nativeEvent.layout.width}))}` read `e.nativeEvent` INSIDE the deferred functional state updater — on native iOS the synthetic event is recycled (nativeEvent=null) by the time the updater runs; RN-Web does not recycle, hence web never crashed. (Earlier SVG-percentage theory was wrong; this line existed since the first implementation.)
...
## Iteration 424 (Jun 2026) — Chunk G: Phase 4 Step 1 (Generate Lab Slip) refinements
User asks:
1. When Phase 2 recorded MUA, Phase 4 Step 1 auto-populates the MUA table (implant/angulation/cuff-height per implant, all types — Zygoma/Pterygoid/Conventional). Each row must be individually Confirmed by the prosthodontist before the Lab Slip PDF is generated. Rows remain editable. YES/NO toggle removed (auto-detected from Phase 2).
2. BUG FIX: Zygoma/Pterygoid/Conventional cases were getting "Phase 3 must be approved before starting Phase 4" on submit even when Phase 3 was legit. Relax the check.
3. In Phase 4 Step 1 → Impressions → Select Impression Type → when "Intra-Oral Scans Made" is picked, capture three new multi-select sub-fields (Type of Scan Body: PEEK/Metal/Hybrid; Scan Type: Vertical Scan Body/Horizontal Scan Bodies (Flags)/Photogrammetry; Scan Level: Abutment/Multiunit Level/Implant Level). All values export to the Lab Slip PDF as bulleted lists.
CHANGES:
- /app/backend/server.py — POST /procedures/{id}/stage2/prosthetic status gate widened to {stage2_surgical_approved, pending_stage2_prosthetic, stage2_prosthetic_step1_approved, pending_final_delivery} plus a current_phase>=4 fallback. Stage2ProstheticSubmit model gains scan_body_types / scan_types / scan_levels (List[str], nulled when conventional). PDF Lab Slip renderer emits bulleted lines under Impression Type for the 3 scan fields.
- /app/frontend/app/procedures/submit-stage2-prosthetic/[id].tsx — MUA rows auto-seed from phase2_data on load when phase4_step1_data has no override. Each row gains a Confirm checkbox (testID mua-confirm-{idx}); handleGenerateLabSlip blocks with Alert until all rows confirmed. Payload sends the confirmed flag. Intra-Oral Scan sub-fields UI (multi-select chips) mounted after impression_type='intraoral_scans'; hydrated + serialised into payload.
...
## Iteration 434 (Jun 2026) — Case Assistant (optional PG-student read-only observer)
- **Phase 1 form**: "Name of the Assistant (optional)" dropdown after Procedure Date/Time in the Schedule section (also on the Pre-Implant Augmentation schedule). Lists all `student` users except the signed-in user (`GET /api/procedures/assistant-candidates`). Stored as `assistant_id` / `assistant_name` on the procedure (all three create paths: `/procedures`, `/procedures/with-existing-implants`, `/procedures/augmentation-case`). Backend rejects self / non-student (400).
- **Notification**: fires once when Phase 1 is fully approved (supervisor + in-charge, or in-charge self-submit) — type `assistant_added`, "You have been added as an assistant by <scheduling user>. Patient … · scheduled on … at … · <procedure type>." Idempotent via `assistant_notified_at`. Reschedules also notify the assistant. Removal after approval → `assistant_removed`.
- **Access**: assistant can `GET /procedures/{id}` (read-only; `viewer_is_assistant=true`, Phase 5 `followups` stripped) and read-only stakeholder endpoints (augmentation checklist / clinical evaluation). All writes stay 403. `GET /procedures?scope=assisted` lists assisted cases; default list never includes them.
- **Change / Remove**: `PATCH /procedures/{id}/assistant {assistant_id}` — owner, case supervisor, in-charge, admin. Appends `assistant_change_log` + HIPAA access log (`assistant_added|changed|removed`). Transfer acceptance clears the assistant if the new owner was the assistant.
- **UI**: `components/AssistantPicker.tsx` (dropdown), `components/AssistantCard.tsx` (case-detail card with Add/Change/Remove bottom sheet), `components/AssistingTodayStrip.tsx` (student dashboard, next 7 days). My Cases: student-only 5th tab **Assisted (n)** (filter row now horizontally scrollable); assisted cards show "Assisting · read-only" chip, no three-dot menu. Case detail: purple read-only banner, Phase 5 hidden, checklist Regenerate hidden. Case PDF + drilling-protocol PDF print "Assistant: <name>".
- Tests: `backend/tests/test_assistant_flow.py`, `backend/tests/test_case_assistant_iter_jun2026.py` (16/16 pass); frontend flows verified by testing agent (iteration report in /app/test_reports).

## Iteration 435 (Jun 2026) — Assistant Logbook (training record)
- `GET /api/me/assistant-logbook` (+ `/export` CSV): total, by_status, by_procedure_type, by_year, case list for the cases a PG student assisted. Students see own; supervisor / in-charge / admin can pass `?student_id=` to review a trainee (other students → 403).
- Profile → new **Training Records** section (students only) with "Cases assisted" card (`components/AssistantLogbookCard.tsx`) → `/assistant-logbook` screen: summary card, by-type bars, status filter chips, case list (taps into read-only case view), CSV export.
- Tests: `backend/tests/test_iter_jun2026_assistant_logbook.py` (8/8), UI verified across student / empty-state / supervisor roles.

## Iteration 436 (Jun 2026) — Phase 4 Step 1: structured Impressions + Intraoral Scanner master list
- **Conventional**: "Tray Technique" → **Impression Technique** (Open Tray / Direct, Closed Tray / Indirect; ids unchanged `open_tray|closed_tray`). **Impression Material Used**: 8 options grouped PVS / Polyether / Condensation Silicone + **Other** (free text `impression_material_other`). Legacy iter-192 ids still labelled for old cases.
- **Intraoral Scan** (all 4 mandatory): **Intraoral Scanner Used** (searchable company → model → Other at either level; `ios_scanner_company/_other`, `ios_scanner_model/_other`), **Scan Body Material** (PEEK/Metal/Hybrid, `ios_scan_body_material`), **Type of Scan Body** (multi-select, `ios_scan_body_types`), **Scan Level** (Abutment/Implant/Multiunit, `ios_scan_level`). Legacy `scan_body_types/scan_types/scan_levels` still rendered for old cases.
- **Scanner master list**: `backend/impression_options.py` (35 companies / 89 models from user's file) seeded idempotently into `intraoral_scanners`; `GET /api/intraoral-scanners`, `GET /api/impression-options`. "Other" entries become `pending` rows → In-Charge/Admin review at Profile → Compliance → **Intraoral scanner suggestions** (`/admin/scanner-suggestions`; approve w/ spelling fix or reject; `GET /intraoral-scanners/pending`, `POST /{id}/approve|reject`). Matching verified rows auto-normalise later "Other" entries.
- **Visibility/Export**: shared `impressionRows()` (backend + `frontend/utils/impressionOptions.ts`) drives the case-detail Phase 4 Step 1 card, case PDF, Lab Slip (frontend `pdfGenerator.ts` + backend fpdf) and AI case context.
- **Analytics**: `GET /api/analytics/impressions` + new "Impressions & Scanners" tab in Advanced Analytics (IOS vs conventional, monthly trend, IOS usage by scanner/company, material trends & families, technique, scan-body/level mix, modality by procedure type).
- Tests: `backend/tests/test_iter_jun2026_impressions_ios.py` (18/18); UI verified by testing agent.

## Iteration 437 (Jun 2026) — Lab Slip Scan Files (STL/PLY attachments, secure lab link, Lab directory, Email lab)
- **Backend module** `backend/lab_files.py` (wired via `setup_lab_files` in server.py). Storage = Emergent Object Storage (`EMERGENT_LLM_KEY`, backend-only; path `implanr/scan-files/{pid}/{uuid}.ext`). Chunked upload: `POST /procedures/{id}/scan-files/init` → `PUT .../{upload_id}/chunk/{i}` (raw or `?encoding=base64`) → `POST .../complete` (assemble, sha256, single streaming PUT). Allowed .stl .ply .obj .zip .dcm .pdf, ≤500 MB, ≤20 files/case. List/download (Bearer or `?token=`), soft delete, `PUT /scan-link` (portal URL). Permissions: owner/creator/in-charge edit; supervisor/nurse/assistant/prev-owner read.
- **Secure lab link**: `POST /procedures/{id}/lab-share` → HMAC token (30 days, nonce) + QR PNG; `PUT /lab-share/slip` stores on-device-rendered Lab Slip HTML; `POST /lab-share/revoke`. Public (no login) `GET /api/lab/{token}` page with "Open Lab Slip", "Download all (ZIP)", per-file download, portal link; `/files/{id}`, `/zip`, `/slip`; 410 when revoked/expired. Downloads counted + written to access_logs.
- **Lab directory** (`labs` collection): `GET /labs` any role; POST/PUT/DELETE in-charge/admin → `/admin/labs` screen (Profile → Compliance → Lab directory).
...
## Iteration 438 (Jun 2026) — Esthetic Risk Assessment (ERA) for anterior maxilla
- **Trigger**: any of FDI 11/12/13/21/22/23 marked missing on a non-full-arch Phase 1 case (not Overdenture-with-Attachment, not Existing Implant). Posterior-only cases keep the 2-field section (Smile Line + Gingival Biotype, with the new biotype options). Full-arch types show no section.
- **Factors (10)**: Smile Line, Smile Type (Toothy/Mixed/Gummy), Gingival Biotype (Thick, Low scalloped / Medium, Medium Scalloped / Thin, High scalloped), Adjacent Teeth Right + Left (Non-restored / Restored / Root canal treated), Infection at site (Absent/Chronic/Acute), Alveolar Ridge Condition (4), Bone Level at Adjacent Teeth (≤5 / 5.5–6.5 / ≥7 mm), Width of Edentulous Span (READ-ONLY, derived from FDI chart runs touching the anterior maxilla: 1 tooth Low / ≥2 High), Patient Esthetic Expectations. Per-factor green/orange/red pill; overall ITI-style (any High→High, else Medium, else Low) with n/10 assessed meta. 9 selectable factors mandatory (listed under Clinical Examination missing fields as "Aesthetic Risk — <factor>").
- **Storage**: top-level `smile_line`, `gingival_biotype` (unchanged) + `procedure.aesthetic_risk {smile_type, adjacent_teeth_right, adjacent_teeth_left, infection_at_site, ridge_condition, bone_level_adjacent, patient_expectations, edentulous_span, overall_risk, assessed_count, anterior_maxilla}`. Backend `aesthetic_risk.py` sanitises + stamps overall on `/procedures`, `/augmentation/complete-phase1`, and re-stamps on `/edit-fields`. Legacy Thin/Thick mapped for display.
...
## Iteration 440 (Jun 2026) — ERA per anterior area + mm-based span + collapsible guidance
- **Width of Edentulous Span** auto-filled from Clinical Examination Mesiodistal Space (mm): cluster inputs `edentulous_site_measurements[leader|tooth].md`, else flat `mesiodistal_space` (single-tooth / SCI flow). ITI grading: single tooth ≥ 7 mm Low · < 7 mm Medium · ≥ 2 contiguous teeth High; displayed "Single tooth < 7 mm · 6 mm". Legacy iter-438 values ("Single tooth"/"Two or more teeth") kept with their old grade when no mm exists.
- **Per-area ERA**: `anteriorAreas()` = contiguous missing runs touching FDI 11–13/21–23. Patient-level factors once (Smile Line, Smile Type, Gingival Biotype, Patient Expectations); site factors per area (Adjacent R/L, Infection, Ridge, Bone Level, Span). Site grade includes patient factors; case overall = highest area. Model: `aesthetic_risk.sites[<leader>] = {positions, factors…, span_mm, edentulous_span, overall_risk}`, plus `overall_risk / assessed_count / total_factors / anterior_maxilla`. Legacy flat docs read as one site and lifted into `sites` on next stamp.
- **Guidance**: collapsible (`EraGuidance` rows/overall/complete props; `<id>-toggle`, `<id>-body`), header "… (n tips)", footer "Guidance only", rendered per area only when that area is fully graded and Medium/High.
- Form testids: patient `era-<key>`, site `era-<key>-<leader>` when >1 area (unsuffixed for single area), `era-site-<leader>`, `era-site-<leader>-overall`, `era-site-<leader>-guidance`, case `era-overall`. Case Details: `era-detail-site-<leader>`, rows `era-detail-row-<key>-<leader>`, deep-path inline edit `aesthetic_risk.sites.<leader>.<key>` (saveField now deep-merges N-level paths; InfoRow deep seed; resolveFieldOptions maps `aesthetic_risk.*` keys). Client + backend PDFs and AI context print per-area blocks. Analytics adds `summary.anterior_areas` + `area_distribution` (factor counts are per area).
- Tests: `test_iter_jun2026_aesthetic_risk.py` (8), `test_iter439_analytics_edges.py` (4+1 skip), `test_iter440_era_additional.py` (3) — all green.
