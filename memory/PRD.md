...
- Session 13 (Feb 19, 2026): (1)–(9) as before. (10) **Printable Patient Consent Form (PDF template)**: New `GET /api/procedures/{id}/consent-form-template` endpoint generates a pre-filled A4 PDF using reportlab — includes patient info, procedure details, Nature of Procedure, Risks & Complications (8 bulleted), Alternatives, Patient Responsibilities, Consent Statement, and signature fields (Patient, Guardian-if-minor, Treating Clinician, Witness) with printed-name + date lines. New `/app/frontend/utils/consentPdf.ts` helper handles download & opens with native share sheet (iOS/Android) or browser tab (web). Wired into three places: (a) case detail page — prominent **Print blank consent template** button when consent is missing, and a discreet Reprint link after upload; (b) Nurse dashboard pending-consent cards — Print button next to Upload; (c) explanatory helper text telling users "print, sign, upload". Verified: PDF generated (5.9KB, valid `%PDF-1.4` header/trailer), 200 OK for stakeholders, role-gated for strangers.
