...
- Session 13 (Feb 19, 2026): (1) Fixed P0 EAS deployment blocker. (2) Edit Mode chip pickers for predefined-option fields. (3) Medical Assessment inline chip editing with auto risk re-calc. (4) Compact Edit History footer visible to students. (5) Full edit-log timeline modal (backend `edit_log[]` array with `$push` diff entries; frontend slide-up Modal with dot-connected timeline, red→green value diff, editor + role + timestamp, two close paths). (6) **Production-readiness batch — tablet scaling + legal + session timeout**: Added `TabletFrame` component (detects `useWindowDimensions >= 768`) that centers the phone-sized UI inside a 480px max-width column with dark navy side-gutters — prevents stretched UI on iPads / Android tablets. Created `/legal/privacy-policy.tsx` and `/legal/terms.tsx` screens with medical-app appropriate content (13-section Privacy Policy covering PHI handling, third-party AI, user rights; 13-section Terms covering clinical disclaimer, patient consent, IP, liability, governing law). Registered both in root `_layout.tsx`. Added "Legal" section in Profile tab with links to both screens + auto-logout footnote. Added pre-register consent line on Register screen with inline links to Terms & Privacy. Added **20-minute session timeout** — `AuthContext` now tracks `lastActivity` via ref; root layout's `ActivityTracker` captures all touch events to reset; a 30-sec interval checks elapsed time and logs the user out with an `Alert` explaining session expiry (critical for shared clinic devices).
