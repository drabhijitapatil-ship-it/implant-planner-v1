...
- Session 13 (Feb 19, 2026): (1)–(8) as before. (9) **Patient Consent Form feature**: Added optional `patient_consent_form` field to procedure schema with a versioned `consent_history[]` audit trail (b3 from spec). Backend endpoints: `POST /uploads/consent-temp` (pre-create), `POST /procedures/{id}/upload-consent` (with replace + push), `GET /procedures/nurse/pending-consents` (nurse/in-charge/supervisor). Phase 1 `new-procedure.tsx` now has a **Patient Consent** tile between Medical Assessment and the Continue button — blue "Upload Patient Consent Form" CTA using `showUploadPicker` (camera / gallery / PDF). When a case is created without consent, all nurses receive an in-app notification + Expo push ("Consent form pending"). Nurse dashboard now shows a new **Patient Consent Forms** section at the top, listing all cases awaiting consent with a per-card upload button; completed uploads drop off the list. Case detail page replaces the "PHASE 1 APPROVED · Tap to complete Phase 2" button with **"UPLOAD PATIENT CONSENT FORM"** (blue) when consent is missing — Phase 2 unlocks immediately after upload. Phase 2 submit endpoint returns HTTP 400 if consent is missing, preventing back-door submission. Verified end-to-end: 13 pending cases → upload → 12; Phase 2 submit blocked → 400.
