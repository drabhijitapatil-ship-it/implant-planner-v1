...
- Session 13 (Feb 19, 2026): (1) Fixed P0 EAS deployment blocker. (2) Edit Mode chip pickers. (3) Medical Assessment inline chip editing. (4) Edit History footer. (5) Full edit-log timeline modal. (6) Tablet scaling + Privacy Policy + Terms + 20-min session auto-logout. (7) **Recent Activity widget on Home dashboard** — new backend endpoint `GET /api/procedures/recent-activity?limit=N` (role-gated to supervisor/implant_incharge/administrator; uses MongoDB aggregation pipeline with `$unwind: edit_log` + `$sort: edited_at desc`). Frontend `/app/frontend/components/RecentActivityWidget.tsx` renders as a card below the procedure calendar on both `SupervisorDashboard` and `InChargeDashboard`, showing last 5 field edits across the department with patient name, field label, old→new diff (red strike-through → green), editor + role, relative time ("5 min ago"). Each row taps through to `/procedures/{id}`. Route ordering fix: moved `recent-activity` above `{procedure_id}` to prevent ObjectId parse crash. Filters out bulk-dict entries on the client. Verified In-Charge gets 200 with 5 filtered entries, student gets 403.
