...
- Session 13 (Feb 19, 2026): (1) Fixed P0 EAS deployment blocker. (2) Edit Mode chip pickers. (3) Medical Assessment inline chip editing. (4) Edit History footer. (5) Full edit-log timeline modal. (6) Tablet scaling + Privacy Policy + Terms + 20-min session auto-logout. (7) Recent Activity widget on Home dashboard for supervisors/in-charges. (8) **Push-notification badges on edit events**: Extended the `PATCH /api/procedures/{id}/edit-fields` handler to insert `db.notifications` rows AND call `send_expo_push_notifications` for all non-editor stakeholders (student owner, assigned supervisor, assigned in-charge) whenever a case is edited. Message format: `"{Role} {Name} edited {Patient}: {fields}"`. Badge on the Alerts tab (already polls `/notifications/unread-count` every 30s) increments automatically; physical-device push banner delivered via existing `usePushNotifications` hook. Verified end-to-end: In-Charge edits student's case → student unread +1 (92→93), supervisor notified, in-charge self-excluded from own action.
