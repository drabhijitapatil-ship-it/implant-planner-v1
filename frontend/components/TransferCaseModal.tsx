// iter-374 — Transfer Case modal (Feb 2026)
// Student-initiated ownership handoff. Popup opens from My Cases 3-dot menu.
// Flow: pick recipient student → mandatory reason → send for supervisor +
// implant-incharge approval → recipient acceptance. See backend
// /procedures/{id}/transfer/request.
...
type Props = {
  procedureId: string;
  patientHint?: string;   // hidden by design — PHI never surfaced here
  onClose: () => void;
  onSubmitted?: () => void;
};
    ...
    const load = async () => {
      try {
        ...
        const list: Student[] = (r.data || []).map((u: any) => ({
          id: u.id || u._id, name: u.name || u.username, username: u.username,
        }));
        setStudents(list);
      } catch (e: any) {
    ...
    <Modal transparent animationType="fade" visible onRequestClose={onClose}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={s.backdrop}>
        <View style={s.card} data-testid="transfer-case-modal">
          ...
          <Text style={s.help}>
            The receiving student needs Supervisor + Implant In-Charge approval, then must accept within 48h.
            Only age & sex are shared in the AI handoff brief — patient name and demographics are never disclosed.
          </Text>
