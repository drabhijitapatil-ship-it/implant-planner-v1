...
export default function CalendarPicker({
  ...
  const clear = () => { onChange(''); setOpen(false); setView('day'); };
      ...
      <View style={cs.calHeader}>
        <TouchableOpacity
          onPress={() => {
            ...
            else if (view === 'month') setViewYear(Math.max(MIN_YEAR, viewYear - 1));
        ...
        >
          ...
          {view !== 'year' && <Ionicons name="chevron-down" size={14} color="#1A73E8" />}
        </TouchableOpacity>

        <TouchableOpacity
          onPress={() => {
            if (view === 'day') nextMonth();
            else if (view === 'month') setViewYear(Math.min(MAX_YEAR, viewYear + 1));
      ...
      {view === 'month' && (
        <View style={cs.tileGrid}>
          {monthNames.map((mn, mi) => {
            ...
            const isThisMonth = viewYear === today.getFullYear() && mi === today.getMonth();
            return (
              <TouchableOpacity
                key={mn}
                disabled={disabled}
                onPress={() => { setViewMonth(mi); setView('day'); }}
                style={[
                  cs.tile,
              ...
              >
                <Text style={[
                  ...
                  !selected && isThisMonth && { color: '#1A73E8', fontWeight: '800' },
                ]}>{mn.slice(0, 3)}</Text>
              </TouchableOpacity>
            );
          })}
            ...
            return (
              ...
              >
                <Text style={[
                  ...
                  disabled && { color: '#CFD8DC' },
                  selected && { color: '#FFF', fontWeight: '800' },
                  !selected && isThisYear && { color: '#1A73E8', fontWeight: '800' },
                ]}>{y}</Text>
              </TouchableOpacity>
            );
          })}
    ...
    <View style={compact ? cs.wrapCompact : cs.wrap}>
      ...
      {compact && (
        <Modal
          ...
          onRequestClose={() => { setOpen(false); setView('day'); }}
        >
          <Pressable style={cs.backdrop} onPress={() => { setOpen(false); setView('day'); }}>
