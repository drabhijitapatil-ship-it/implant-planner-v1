...
type Props = {
  ...
  followupMode?: {
    phase4ByTooth: Record<string, string>;
 ...
 */
export default function RadiographCompare({
  procedure,
  iopaUploads,
  opgUpload,
  followupMode,
  ...
  const canEditNotes = useMemo(() => {
    ...
    if (
      ...
      return true;
    if (role === "supervisor" && procedure.supervisor_id === user.id)
      return true;
    if (role === "student" && procedure.student_id === user.id) return true;
    return false;
  }, [user, procedure]);

  const notesMap: Record<string, CompareNote> =
    procedure?.radiograph_compare_notes || {};

  const isExisting = procedure?.case_origin === "existing_implants";
  const effectiveProcType =
    isExisting && procedure?.original_procedure_type
  ...
  const baselineByTooth = useMemo(() => {
    ...
    if (isExisting) {
      ...
      files.forEach((f) => {
        ...
        if (rawLabel && rawLabel !== stripped)
          map[rawLabel] = String(f.filename);
      });
    }
    return map;
  ...
  const baselineLabel = isExisting
    ...
    : "Baseline — Phase 2 (post-surgical)";
  const currentLabel =
    followupMode?.currentLabel || "Current — Phase 4 (post-delivery)";
  ...
  const hasAnyBaseline = isFullArch
    ? !!baselineOpg
    : Object.keys(baselineByTooth).length > 0;
  ...
  const allBaselineMissing = isFullArch
    ? !baselineOpg
    : missingBaselineCount === teethCount;
          ...
          {isFullArch ? (
            <ComparisonRow
              ...
              onOpen={(b, c) =>
                setViewer({
                  baseline: b,
                  mid: followupMode
                    ? followupMode.phase4Opg || null
                    : undefined,
                  current: c,
                  toothKey: followupMode ? "fu_opg" : "opg",
                  toothLabel: "Full Arch — OPG",
                })
              }
            />
            ...
            teeth.map((tooth) => (
              <ComparisonRow
                ...
                mid={
                  followupMode
                    ? followupMode.phase4ByTooth[tooth] || null
                    : undefined
                }
                midLabel={phase4Label}
                ...
                onOpen={(b, c) =>
                  setViewer({
                    ...
                    mid: followupMode
                      ...
                      : undefined,
                    current: c,
                    toothKey: followupMode ? `fu_${tooth}` : tooth,
                    toothLabel: `Tooth ${tooth}`,
                  })
                }
              />
  ...
  return (
    <View style={s.row} testID={`compare-row-${toothLabel}`}>
      <View style={s.toothBadge}>
        <Text style={s.toothBadgeText} numberOfLines={1}>
          {toothLabel}
        </Text>
      </View>
      <CompareThumb
        ...
        onPress={canOpen ? () => onOpen(baseline, current) : undefined}
        testID={`compare-baseline-${toothLabel}`}
      />
      {mid !== undefined && (
        <>
          ...
          <CompareThumb
            filename={mid}
            caption={midLabel || "Phase 4"}
            placeholder="No Phase 4"
            onPress={canOpen ? () => onOpen(baseline, current) : undefined}
            testID={`compare-mid-${toothLabel}`}
          />
        </>
      )}
      <Ionicons
        name="arrow-forward"
        size={mid !== undefined ? 14 : 18}
        color="#90A4AE"
      ...
      <CompareThumb
        ...
        onPress={canOpen ? () => onOpen(baseline, current) : undefined}
        testID={`compare-current-${toothLabel}`}
      />
    </View>
  );
}
...
function CompareThumb({
  ...
  const inner = (
    <View style={[s.thumbBox, isMissing && s.thumbMissing]}>
      ...
      <Text
        style={[s.thumbCaption, isMissing && { color: "#90A4AE" }]}
        numberOfLines={2}
      >
      ...
      <View style={s.modalRoot}>
        <View style={s.modalHeader}>
          ...
          <TouchableOpacity
            onPress={onClose}
            style={s.modalClose}
            testID="radiograph-compare-close"
          >
        ...
        <ScrollView contentContainerStyle={s.modalScroll}>
          <View style={s.modalPanesWrap}>
            <FullPane
              filename={baseline}
              caption={baselineLabel}
              testID="fullscreen-baseline"
            />
            {mid !== undefined && (
              <FullPane
                filename={mid}
                caption={midLabel || "Phase 4"}
                testID="fullscreen-mid"
              />
            )}
            <FullPane
              filename={current}
              caption={currentLabel}
              testID="fullscreen-current"
            />
          </View>
          ...
          <AINotesPanel
            ...
            baselineLabel={
              mid !== undefined ? midLabel || "Phase 4" : baselineLabel
            }
            currentLabel={currentLabel}
...
function AINotesPanel({
  ...
  const baselineIsPdf =
    !!baselineFilename && baselineFilename.toLowerCase().endsWith(".pdf");
  const currentIsPdf =
  ...
  const onGenerate = async () => {
    if (!hasGeneratable) {
      Alert.alert(
        "Both radiographs required",
        "Upload the post-delivery IOPA first, then generate AI notes.",
      );
      return;
    }
    if (blockedByPdf) {
      Alert.alert(
        "Image required",
        "AI analysis needs a JPG/PNG radiograph. Re-upload the file as an image to use this feature.",
      );
      return;
    }
    setGenerating(true);
    try {
      const res = await api.post(
        `/procedures/${procedureId}/radiograph-compare/ai-notes`,
        {
      ...
      Alert.alert(
        ...
        e?.response?.data?.detail ||
          "Could not generate AI notes. Please try again.",
      );
    } finally {
      setGenerating(false);
    }
  ...
  const onSave = async () => {
    ...
    try {
      const res = await api.put(
        `/procedures/${procedureId}/radiograph-compare/notes`,
        {
          tooth_label: toothKey,
          notes: draft,
        },
      ...
      Alert.alert(
        "Save failed",
        e?.response?.data?.detail ||
          "Could not save your edits. Please try again.",
      );
    } finally {
      setSaving(false);
    }
  ...
  const aiSubtitle = note?.ai_generated_at
    ...
    : null;
  const editorLine =
    note?.edited_at && !note?.is_ai_only
  ...
  return (
    <View style={s.aiPanel} testID="ai-notes-panel">
      <View style={s.aiHeader}>
        ...
        {note?.ai_generated ? (
          <View style={s.aiChip}>
            <Text style={s.aiChipText}>
              {note.is_ai_only ? "AI draft" : "Edited"}
            </Text>
          </View>
        ) : null}
      </View>
      ...
      <Text style={s.aiDisclaimer}>
        AI compares the two radiographs and drafts clinical observations. This
        is a clinician aid — not a diagnostic substitute. Please review and edit
        before sign-off.
      </Text>
      ...
      {!note?.ai_generated && !generating && (
        <TouchableOpacity
          style={[
            s.aiPrimaryBtn,
            (!hasGeneratable || blockedByPdf) && s.aiBtnDisabled,
          ]}
          onPress={onGenerate}
      ...
      {generating && (
        <View style={s.aiLoading}>
          ...
          <Text style={s.aiLoadingText}>
            Analysing radiographs… this can take 10–20 seconds.
          </Text>
        </View>
      )}
      ...
      {!hasGeneratable && !note?.ai_generated && (
        <Text style={s.aiHint}>
          ...
          {!currentFilename
            ? "Upload the post-delivery radiograph to enable AI analysis."
            : ""}
        </Text>
      )}
      {blockedByPdf && (
        <Text style={s.aiHint}>
          One of the radiographs is a PDF — AI analysis needs an image (JPG /
          PNG).
        </Text>
      )}
        ...
        <>
          <View style={s.aiEditorWrap}>
            {canEdit ? (
              ...
              <Text style={s.aiReadonly} testID="ai-notes-readonly">
                {draft || "(No notes yet)"}
              </Text>
            )}
          </View>
...
function FullPane({
  ...
  return (
    <Pressable style={s.pane} onPress={openExternal} testID={testID}>
      <Text style={s.paneCaption} numberOfLines={1}>
        {caption}
      </Text>
      <View style={s.paneImgWrap}>
        {!filename ? (
          ...
          <View style={s.paneEmpty}>
            ...
            <Text style={[s.paneEmptyText, { color: "#FFF" }]}>
              Tap to open PDF
            </Text>
          </View>
        ) : uri ? (
