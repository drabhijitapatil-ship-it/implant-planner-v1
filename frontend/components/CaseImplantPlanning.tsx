...
export default function CaseImplantPlanning({
  ...
  // Drilling Protocol isn't clinically relevant yet during Phase 1
  // planning (implant not placed) — only show it from Phase 2 onward.
  const isPhase1 =
    !procedureStatus ||
    procedureStatus === "draft" ||
    procedureStatus === "pending_phase1";
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      if (planRes.status === "fulfilled")
        setPlans(planRes.value.data.implant_plans || []);
  ...
  return (
    <View style={st.container} data-testid="case-implant-planning">
      ...
      {plans.map((plan, idx) => {
        ...
        const isInactive =
          surv?.status === "Failed" || surv?.status === "Replaced";
        return (
          <View
            key={`${plan.position}-${idx}`}
            style={[st.implantCard, isInactive && st.implantCardInactive]}
          ...
          >
            ...
            <View
              style={[
                st.statusBadge,
                isInactive ? st.statusBadgeInactive : st.statusBadgeActive,
              ]}
            ...
            <View style={st.implantCardHeader}>
              ...
              <View style={st.implantInfo}>
                <Text style={[st.implantTitle, isInactive && st.textMuted]}>
            ...
            <View style={st.implantDetails}>
              ...
              {plan.risk_score !== undefined && plan.risk_score !== null && (
                <Text style={st.detailText}>
                  Risk Score: {plan.risk_score}/{plan.risk_score > 15 ? 18 : 15}
                </Text>
              )}
              {torqueValues && torqueValues[idx] !== undefined && (
                ...
                >
                  ...
                  <Text style={st.torqueText}>
                    Torque:{" "}
                    <Text style={st.torqueValue}>{torqueValues[idx]} Ncm</Text>
                  </Text>
                </View>
              )}
            </View>
              ...
              <View style={st.implantActions}>
                ...
                {!isPhase1 && (
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      setExpandedProtocol(expandedProtocol === idx ? null : idx)
                    }
            ...
            {!isPhase1 && expandedProtocol === idx && plan.bone_type && (
      ...
      {survivalReview?.implants
        ? Object.entries(survivalReview.implants).map(
            ([keyStr, entry]: [string, any]) => {
              ...
              const tooth =
                repl.tooth_number ||
                originalPlan?.position ||
                entry?.new_tooth_number ||
                "-";
              const brand =
                repl.brand ||
                (repl.system || "").split(" — ")[0] ||
              ...
              return (
                <View
                  ...
                  data-testid={`implant-revision-${keyStr}`}
                >
                  ...
                  <View style={st.implantCardHeader}>
                    <View
                      style={[st.positionBadge, { backgroundColor: "#2E7D32" }]}
                    >
                    ...
                    <ColorStripe
                      ...
                      active
                      testID={`implant-revision-stripe-${keyStr}`}
                    />
                    <View style={st.implantInfo}>
                      <Text style={st.implantTitle}>
                      ...
                      <Text style={st.revisionNote}>
                        Revision R{repl.revision_number || 1} · replaced #
                        {originalPlan?.position || tooth}
                      </Text>
                    </View>
                  </View>
                  <View style={st.implantDetails}>
                    ...
                    {repl.lot_number && (
                      <Text style={st.detailText}>Lot: {repl.lot_number}</Text>
                    )}
                    {repl.placement_date && (
                      <Text style={st.detailText}>
  ...
  useEffect(() => {
    if (visible) {
      if (editItem) {
        ...
        resetForm();
        // Bone Width / Bone Height are entered manually by the student from
        // CBCT — no longer auto-prefilled from Clinical Examination oc/md.
        if (presetPosition) {
              ...
              <View>
                ...
                {mode === "choose" ? (
                  <>
                    ...
                    {selectedSystem && !streamlinedEdit && !isPhase1 && (
                      <View
              ...
              <View>
                ...
                result.narrow_ridge_evaluation?.blocked ? (
                  ...
                  <>
                    {(() => {
                      ...
                      if (mode === "choose") {
                        ...
                        if (
                          hasNarrowRidge &&
                          result.narrow_options?.length > 0
                        ) {
                          implants = result.narrow_options;
                        } else {
                          implants = result.recommended?.length
                            ? result.recommended
                            : result.all_options || [];
                        }
                      } else {
                      ...
                      const sortedImplants = [...implants].sort(
                      ...
                      const allSystemOptions =
                        mode === "choose"
                          ? result.all_options || []
                          : sortedImplants;
                      const remainingAll = allSystemOptions.filter(
                        (opt: any) =>
                          !topMatches.some(
                            (t: any) =>
                              t.diameter === opt.diameter &&
                              t.length === opt.length,
                          ),
                      );
                      const remaining =
                        mode === "choose"
                          ? remainingAll
                          : sortedImplants.slice(5);
                      const displayed = showAllResults
                        ? [...topMatches, ...remaining]
                        : topMatches;
                      return (
                        <>
                          {isRestrictedResult && (
                          ...
                          <Text style={ms.matchHeader}>
                            {isRestrictedResult
                              ...
                              : isNarrowResult
                                ? `Narrow Diameter Options (${implants.length})`
                                ...
                                {!imp.priority &&
                                  ...
                                  !warning && (
                                    <View style={ms.bestBadge}>
                                      <Text style={ms.bestBadgeText}>Best</Text>
                                    </View>
                                  )}
                                {!imp.priority &&
                                  i === 1 &&
                  ...
                  to keep the implant resize flow focused. */}
                    {selectedImplant && !streamlinedEdit && !isPhase1 && (
                      <View
