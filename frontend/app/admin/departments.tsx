...
// Mirrors backend DEPARTMENT_COLOR_PALETTE — a fixed set of distinct swatches
// so departments stay visually distinguishable across the whole app (badges,
// filter chips, dashboard cards, student/supervisor headers).
// Pastel palette — mirrors backend DEPARTMENT_COLOR_PALETTE. Light enough
// that dark text/icons on top always stay readable, so every badge/chip
// that uses a department color pairs it with a fixed dark foreground
// (DEPARTMENT_COLOR_TEXT) instead of white.
...
const DEPARTMENT_COLOR_TEXT = "#37474F";
...
export default function DepartmentsScreen() {
  ...
  const [unassignedIncharges, setUnassignedIncharges] = useState<
    InchargeUser[]
  >([]);
  ...
  const loadIncharges = useCallback(async (deptId: string) => {
    ...
    try {
      // A department incharge can be an Implant In-Charge, an Administrator,
      // or the Org Admin — fetched separately by is_admin since a clinic org
      // stores the owner's role as "chief_dentist" etc, which wouldn't match
      // either role filter above.
      const [inchargeRes, adminRes, orgAdminRes] = await Promise.all([
        api.get("/users", { params: { role: "implant_incharge" } }),
        api.get("/users", { params: { role: "administrator" } }),
        api.get("/users", { params: { is_admin: true } }),
      ]);
      const userMap = new Map<string, InchargeUser>();
      [
        ...(inchargeRes.data || []),
        ...(adminRes.data || []),
        ...(orgAdminRes.data || []),
      ].forEach((u: InchargeUser) => {
        userMap.set(u.id, u);
      });
      const all = Array.from(userMap.values());
      const getDeptIds = (u: InchargeUser): string[] => {
        if (Array.isArray(u.department_ids) && u.department_ids.length > 0) {
          return u.department_ids;
        }
        return u.department_id ? [u.department_id] : [];
      };
     
       
      setAssignedIncharges(all.filter((u) => getDeptIds(u).includes(deptId)));
      setUnassignedIncharges(
        all.filter(
          (u) =>
            !getDeptIds(u).includes(deptId) &&
            getDeptIds(u).length < MAX_INCHARGES_PER_DEPT,
        ),
      );
    } catch (error) {
  ...
  // Keyed on editingDept alone — NOT showModal. The assign-picker/new-incharge
  // sub-modals hide this screen's own modal (showModal false) while they're
  // open, to avoid the two-stacked-native-Modal iOS freeze; if this effect
  // depended on showModal too, that hide would wipe assigned/unassigned right
  // as the picker opens, showing "no available" every time.
  useEffect(() => {
    if (editingDept) {
  ...
  const openCreateModal = () => {
    ...
    setColorInput(
      DEPARTMENT_COLOR_PALETTE[
        departments.length % DEPARTMENT_COLOR_PALETTE.length
      ],
    );
    setShowModal(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingDept) {
        ...
        const res = await api.post("/departments", { name, color: colorInput });
  ...
  const openNewInchargeForm = () => {
    setEmailStatus("idle");
    setShowModal(false);
    setShowNewInchargeForm(true);
  };
  const closeNewInchargeForm = () => {
    setEmailStatus("idle");
  ...
  const handleAssignExisting = async (u: InchargeUser) => {
    ...
    setAssigning(true);
    try {
      await api.put(`/users/${u.id}`, { add_department_id: editingDept.id });
      closeAssignPicker();
      loadIncharges(editingDept.id);
    } catch (error: any) {
      Alert.alert(
        "Error",
        error.response?.data?.detail || "Failed to assign incharge",
      );
    } finally {
      setAssigning(false);
    }
  ...
  const handleRemoveIncharge = (u: InchargeUser) => {
    ...
    Alert.alert(
      "Remove Incharge",
      `Remove ${u.name} from ${editingDept.name}?`,
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Remove",
          ...
          onPress: async () => {
            try {
              await api.put(`/users/${u.id}`, {
                remove_department_id: editingDept.id,
              });
              loadIncharges(editingDept.id);
            } catch (error: any) {
              Alert.alert(
                "Error",
                error.response?.data?.detail || "Failed to remove incharge",
              );
            }
          },
        },
  ...
  const handleCreateIncharge = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Error",
        error.response?.data?.detail || "Failed to create incharge",
      );
    } finally {
      setAssigning(false);
    }
            ...
            >
              <View
                style={[
                  styles.deptIconBg,
                  {
                    backgroundColor: item.color || DEPARTMENT_COLOR_PALETTE[0],
                  },
                ]}
              >
                <Ionicons
            ...
            <View style={styles.modalContent} data-testid="department-modal">
              ...
              >
                ...
                <View
                  ...
                  data-testid="department-color-picker"
                >
                  {DEPARTMENT_COLOR_PALETTE.map((c) => (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.colorSwatch,
                        { backgroundColor: c },
                        colorInput === c && styles.colorSwatchSelected,
                      ]}
                      onPress={() => setColorInput(c)}
                      data-testid={`department-color-${c}`}
                    >
                      {colorInput === c && (
                        <Ionicons
                          name="checkmark"
                    ...
                    {loadingIncharges ? (
                      ...
                      <>
                        ...
                        {assignedIncharges.map((u) => (
                          <View
                            key={u.id}
                            style={styles.inchargeRow}
                            data-testid={`assigned-incharge-${u.id}`}
                          >
                            ...
                            <View style={{ flex: 1 }}>
                              ...
                              <Text style={styles.inchargeEmail}>
                                {u.email}
                              </Text>
                            </View>
                            <TouchableOpacity
                          ...
                          <View style={styles.inchargeActionsRow}>
                            ...
                            >
                              <Ionicons
                                ...
                                color="#1A73E8"
                              />
                              <Text style={styles.inchargeActionBtnText}>
                                Assign Existing
                              </Text>
                            </TouchableOpacity>
                            <TouchableOpacity
          ...
          <View style={styles.pickerSheet}>
            ...
            <ScrollView style={{ maxHeight: 360 }}>
              {unassignedIncharges.length === 0 ? (
                <Text style={styles.hintSmall}>
                  No available Implant In-Charge or Admin — create a new one
                  instead.
                </Text>
              ) : (
                unassignedIncharges.map((u) => {
                  const deptCount =
                    Array.isArray(u.department_ids) &&
          ...
          <View style={styles.modalOverlay}>
            <View style={styles.modalContent} data-testid="new-incharge-modal">
              ...
              <TextInput
                style={[
                  styles.input,
                  emailStatus === "exists" && { borderColor: "#EF5350" },
                ]}
                placeholder="jane.doe@dental.edu"
                placeholderTextColor="#999"
                value={newIncharge.email}
                onChangeText={(text) =>
                  setNewIncharge((v) => ({ ...v, email: text }))
                }
              ...
              {emailStatus === "checking" && (
                <Text
                  ...
                  data-testid="new-incharge-email-checking"
                >
                  Checking email…
                </Text>
              )}
              {emailStatus === "exists" && (
                <Text
                  style={[styles.hintSmall, { color: "#EF5350" }]}
                  data-testid="new-incharge-email-exists"
                >
              ...
              {emailStatus === "available" && (
                <Text
                  style={[styles.hintSmall, { color: "#2E7D32" }]}
                  data-testid="new-incharge-email-available"
                >
                  Email is available
                </Text>
              )}
...
const styles = StyleSheet.create({
  ...
  colorSwatchSelected: { borderColor: "#1A202C" },
  input: {
    borderWidth: 1,
    borderColor: "#E2E8F0",
    borderRadius: 10,
