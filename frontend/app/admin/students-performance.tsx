...
export default function StudentsPerformanceScreen() {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return list;
    return list.filter((r) => (r.student_name || '').toLowerCase().includes(q));
  ...
  return (
    <SafeAreaView style={s.c} edges={['top', 'bottom']}>
      ...
      >
        {filtered.length === 0 ? (
          <Text style={s.empty}>{query || selectedDept !== 'all' ? 'No students match your filter.' : 'No student data yet.'}</Text>
        ) : (
          filtered.map((st, idx) => (
            <TouchableOpacity
              key={st.student_id}
