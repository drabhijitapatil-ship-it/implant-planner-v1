...
function Dropdown({
  ...
  return (
    <>
      ...
      >
        <Text
          style={[s.selectT, !value && { color: "#90A4AE" }]}
          numberOfLines={1}
        >
          {value || placeholder}
        </Text>
...
export default function SurvivalReview() {
  ...
  const [procedureMeta, setProcedureMeta] = useState<{
  ...
  const [survivalReviewState, setSurvivalReviewState] = useState<any | null>(
    null,
  );
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        Alert.alert(
          "Error",
          e?.response?.data?.detail || "Failed to load survival review data",
        );
      } finally {
        setLoading(false);
      }
  ...
  const setField = <K extends keyof FailureEntry>(
    idx: number,
    key: K,
    value: FailureEntry[K],
  ) => {
    setFailures((prev) => ({ ...prev, [idx]: { ...prev[idx], [key]: value } }));
  };
  const setReplField = (
    idx: number,
    patch: Partial<FailureEntry["replacement"]>,
  ) => {
    setFailures((prev) => ({
      ...prev,
      [idx]: {
        ...prev[idx],
        replacement: { ...prev[idx].replacement, ...patch },
      },
    }));
  };
  ...
  const canSubmit = () => {
    ...
    if (allSurvived === "no" && Object.keys(failures).length === 0)
      return false;
    for (const f of Object.values(failures)) {
      ...
      if (f.replaced) {
        ...
        if (r.procedure_type === "Two Stage") {
          ...
          if (
            r.prosthetic_component === "Healing Abutment" &&
            !r.healing_abutment_mm
          )
            return false;
        }
        if (
          r.procedure_type === "Immediate Loading" &&
          !isFullArchProcedure(procedureMeta.implant_procedure_type)
        ) {
          if (!r.immediate_loading_prosthesis) return false;
          if (
            r.immediate_loading_prosthesis === "Other" &&
            !r.immediate_loading_prosthesis_other.trim()
          )
            return false;
        }
      }
  ...
  const handleReplIopaUpload = async (idx: number) => {
    try {
      ...
      if (Platform.OS === "web") {
        ...
        fd.append(
          "file",
          new File([blob], picked.name || "iopa", {
      ...
      Alert.alert(
        "Upload failed",
        e?.response?.data?.detail || e?.message || "Could not upload the IOPA.",
      );
  ...
  const handleSubmit = async (afterSave: "phase3" | "back") => {
    if (allSurvived && !canSubmit()) {
      Alert.alert(
        "Incomplete",
        "Please fill all required fields (reason, site change target, and replacement details).",
      );
      return;
    }
    setSaving(true);
    try {
      ...
      if (allSurvived === "no") {
        body.failures = Object.values(failures).map((f) => {
          const r = f.replacement;
          const finalSystem = r.system_is_other
            ? r.system_other_text || "Other"
            : r.system;
          const finalReason =
            f.reason === "Other"
          ...
          return {
            ...
            replacement: f.replaced
              ? {
                  ...
                  insertion_torque_ncm: r.insertion_torque_ncm
                    ? Number(r.insertion_torque_ncm)
                    : null,
                  ...
                  prosthetic_component:
                    r.procedure_type === "Two Stage"
                      ? r.prosthetic_component
                      : null,
                  healing_abutment_mm:
                    r.procedure_type === "Two Stage" &&
                    r.prosthetic_component === "Healing Abutment" &&
                    r.healing_abutment_mm
                      ? Number(r.healing_abutment_mm)
                      : null,
                  immediate_loading_prosthesis:
                    r.procedure_type === "Immediate Loading"
                      ? r.immediate_loading_prosthesis === "Other"
  ...
  const handleEndTreatment = async () => {
    ...
    if (!endModal.reason)
      return Alert.alert("Missing field", "Please pick a failure reason.");
    if (
      endModal.decision_maker !== "Patient" &&
      endModal.decision_maker !== "Operator"
    ) {
      return Alert.alert(
        "Missing field",
        "Please pick who decided to end treatment.",
      );
    }
    if (!endModal.end_reason.trim())
      return Alert.alert(
        "Missing field",
        "Please describe the rationale for ending treatment.",
      );
    setEndModal((m) => ({ ...m, submitting: true }));
    try {
      const now = new Date().toISOString();
      const impls =
        implants.length > 0 ? implants : [{ tooth_number: null, tooth: null }];
      const body: any = {
        all_survived: false,
  ...
  const label =
    implants.length === 1 ? "Implant Survived" : "All Implants Survived";
    ...
    <SafeAreaView style={s.c} edges={["top", "bottom"]}>
      <View style={s.h}>
        <TouchableOpacity
          onPress={() => router.back()}
          style={{ padding: 8 }}
          data-testid="survival-back"
          testID="survival-back"
        >
      ...
      >
        ...
        >
          ...
          {survivalReviewState?.events &&
          survivalReviewState.events.length > 0 ? (
            <View
              style={s.histCard}
              data-testid="survival-history"
              testID="survival-history"
            >
              <View
                style={{
                  flexDirection: "row",
              ...
              >
                ...
                <Text style={s.histTitle}>
                  Previous survival reviews ({survivalReviewState.events.length}
                  )
                </Text>
              </View>
              {survivalReviewState.events.map((ev: any, i: number) => (
                <View
                  key={i}
                  style={s.histRow}
                  data-testid={`survival-history-event-${i}`}
                >
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    (!ev.failures || ev.failures.length === 0) ? (
                      <Text style={s.histBody}>
                        All implants confirmed survived.
                      </Text>
                    ) : (
                      <Text style={s.histBody}>
                        {ev.failures.length} failure
                        {ev.failures.length > 1 ? "s" : ""} reported:
                        {ev.failures
                          .map(
                            (f: any, j: number) =>
          ...
          <View style={s.card}>
            ...
            <View style={{ flexDirection: "row", gap: 12, marginTop: 12 }}>
              <TouchableOpacity
                style={[s.pill, allSurvived === "yes" && s.pillOn]}
                onPress={() => setAllSurvived("yes")}
                data-testid="survival-all-yes"
                testID="survival-all-yes"
              >
                <Text style={[s.pillT, allSurvived === "yes" && s.pillTOn]}>
                  Yes
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[s.pill, allSurvived === "no" && s.pillOn]}
                onPress={() => setAllSurvived("no")}
                data-testid="survival-all-no"
                testID="survival-all-no"
              >
                <Text style={[s.pillT, allSurvived === "no" && s.pillTOn]}>
                  No
                </Text>
              </TouchableOpacity>
            </View>
          ...
          {allSurvived === "no" &&
            implants.map((imp: any, i: number) => {
              const f = failures[i];
              const failed = !!f;
              const failedTooth = String(imp.tooth_number || imp.tooth || "");
              const cat =
                f?.replacement.system && !f.replacement.system_is_other
                  ? findSystemByLabel(f.replacement.system)
                  : undefined;
              const diameters = cat?.diameters?.map(String) || [];
              const lengths = cat?.lengths?.map(String) || [];
              const immOptions = getImmediateLoadingOptions(
                procedureMeta.implant_procedure_type,
                procedureMeta.number_of_implants,
              );
              const hideImmediate = isFullArchProcedure(
                procedureMeta.implant_procedure_type,
              );
              const procTypeOptions = (hideImmediate
                ? PROC_TYPES.filter((t) => t !== "Immediate Loading")
                : PROC_TYPES) as unknown as string[];
              ...
              return (
                <View key={i} style={s.card}>
                  ...
                  >
                    <TouchableOpacity
                      style={[s.pillSm, !failed && s.pillOn]}
                      onPress={() => toggleFailure(i, imp, "yes")}
                      data-testid={`imp-${i}-survived-yes`}
                      testID={`imp-${i}-survived-yes`}
                    >
                      <Text style={[s.pillTSm, !failed && s.pillTOn]}>
                        Survived
                      </Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                      style={[s.pillSm, failed && s.pillOnR]}
                      onPress={() => toggleFailure(i, imp, "no")}
                      data-testid={`imp-${i}-survived-no`}
                      testID={`imp-${i}-survived-no`}
                    >
                      <Text style={[s.pillTSm, failed && s.pillTOn]}>
                        Failed
                      </Text>
                  ...
                  {failed && (
                    <View style={{ marginTop: 14, gap: 12 }}>
                        ...
                        {f.reason === "Other" && (
                          <View style={{ marginTop: 8 }}>
                            <TextInput
                              ...
                              onChangeText={(v) =>
                                setField(i, "reason_other_text", v)
                              }
                            ...
                            <Text
                              style={[
                                s.helper,
                                wordCount(f.reason_other_text) > 100 && {
                                  color: "#C62828",
                                },
                              ]}
                            >
                      ...
                      >
                        ...
                        <TouchableOpacity
                          style={[s.pillTiny, f.removed && s.pillOn]}
                          onPress={() => setField(i, "removed", true)}
                          data-testid={`imp-${i}-removed-yes`}
                          testID={`imp-${i}-removed-yes`}
                        >
                          <Text style={[s.pillTT, f.removed && s.pillTOn]}>
                            Yes
                          </Text>
                        </TouchableOpacity>
                        <TouchableOpacity
                          style={[s.pillTiny, !f.removed && s.pillOn]}
                          onPress={() => setField(i, "removed", false)}
                          data-testid={`imp-${i}-removed-no`}
                          testID={`imp-${i}-removed-no`}
                        >
                          <Text style={[s.pillTT, !f.removed && s.pillTOn]}>
                            No
                          </Text>
                      ...
                      >
                        ...
                        <TouchableOpacity
                          style={[s.pillTiny, f.site_changed && s.pillOn]}
                          onPress={() => setField(i, "site_changed", true)}
                          data-testid={`imp-${i}-site-yes`}
                          testID={`imp-${i}-site-yes`}
                        >
                          <Text style={[s.pillTT, f.site_changed && s.pillTOn]}>
                            Yes
                          </Text>
                        </TouchableOpacity>
                        <TouchableOpacity
                          style={[s.pillTiny, !f.site_changed && s.pillOn]}
                          onPress={() => {
                            setField(i, "site_changed", false);
                            setField(i, "new_tooth_number", null);
                          }}
                          data-testid={`imp-${i}-site-no`}
                          testID={`imp-${i}-site-no`}
                        >
                          <Text
                            style={[s.pillTT, !f.site_changed && s.pillTOn]}
                          >
                            No
                          </Text>
                        </TouchableOpacity>
                      ...
                      {f.site_changed && (
                        <View style={s.fdiBox}>
                          <Text style={s.fdiHelp}>
                            Select a new tooth in the same quadrant as{" "}
                            {failedTooth}. The chosen tooth will replace{" "}
                            {failedTooth} for Phase 3 and Phase 4.
                          </Text>
                          <FDIChart
                      ...
                      >
                        ...
                        <TouchableOpacity
                          style={[s.pillTiny, f.replaced && s.pillOn]}
                          onPress={() => setField(i, "replaced", true)}
                          data-testid={`imp-${i}-replaced-yes`}
                          testID={`imp-${i}-replaced-yes`}
                        >
                          <Text style={[s.pillTT, f.replaced && s.pillTOn]}>
                            Yes
                          </Text>
                        </TouchableOpacity>
                        <TouchableOpacity
                          style={[s.pillTiny, !f.replaced && s.pillOn]}
                          onPress={() => setField(i, "replaced", false)}
                          data-testid={`imp-${i}-replaced-no`}
                          testID={`imp-${i}-replaced-no`}
                        >
                          <Text style={[s.pillTT, !f.replaced && s.pillTOn]}>
                            No
                          </Text>
                        ...
                        <View style={s.replBox}>
                          ...
                          {f.replacement.system_is_other && (
                            <TextInput
                              ...
                              onChangeText={(v) =>
                                setReplField(i, { system_other_text: v })
                              }
                          ...
                          <View style={{ flexDirection: "row", gap: 8 }}>
                            {diameters.length > 0 ? (
                              <View style={{ flex: 1 }}>
                                <Dropdown
                                  value={
                                    f.replacement.diameter
                                      ? `${f.replacement.diameter} mm`
                                      : ""
                                  }
                              ...
                              <TextInput
                                ...
                                value={f.replacement.diameter}
                                onChangeText={(v) =>
                                  setReplField(i, { diameter: v })
                                }
                                data-testid={`imp-${i}-repl-diameter`}
                                testID={`imp-${i}-repl-diameter`}
                              />
                            )}
                            {lengths.length > 0 ? (
                              <View style={{ flex: 1 }}>
                                <Dropdown
                                  value={
                                    f.replacement.length
                                      ? `${f.replacement.length} mm`
                                      : ""
                                  }
                              ...
                              <TextInput
                                ...
                                placeholder="Length (mm)"
                                keyboardType="decimal-pad"
                                value={f.replacement.length}
                                onChangeText={(v) =>
                                  setReplField(i, { length: v })
                                }
                                data-testid={`imp-${i}-repl-length`}
                                testID={`imp-${i}-repl-length`}
                              />
                            )}
                          </View>
                          ...
                          <Text
                            style={[s.lbl, { marginTop: 4, color: "#C62828" }]}
                          >
                            IOPA Radiograph *
                          </Text>
                          <TouchableOpacity
                            style={[
                              s.iopaBtn,
                              !f.replacement.iopa_url && s.iopaBtnInvalid,
                            ]}
                          ...
                          {f.replacement.iopa_url ? (
                            <View style={{ marginTop: 6 }}>
                              <RadiographThumb
                                filename={f.replacement.iopa_url}
                                testID={`imp-${i}-repl-iopa-thumb`}
                                label="R-Revision IOPA"
                              />
                            </View>
                          ) : null}
                          ...
                          <TextInput
                            ...
                            onChangeText={(v) =>
                              setReplField(i, { insertion_torque_ncm: v })
                            }
                          ...
                          <TextInput
                            ...
                            placeholder="e.g. 72"
                            keyboardType="decimal-pad"
                            value={f.replacement.isq}
                            onChangeText={(v) => setReplField(i, { isq: v })}
                            data-testid={`imp-${i}-repl-isq`}
                            testID={`imp-${i}-repl-isq`}
                          />
                          ...
                          <PlacementDatePicker
                            ...
                            onChange={(iso: string) =>
                              setReplField(i, { placement_date: iso })
                            }
                          ...
                          {f.replacement.procedure_type === "Two Stage" && (
                            <View style={{ gap: 8 }}>
                              ...
                              <Dropdown
                                ...
                                onChange={(v) =>
                                  setReplField(i, {
                                    prosthetic_component: v,
                                    healing_abutment_mm: "",
                                  })
                                }
                                testID={`imp-${i}-repl-prostheticcomp`}
                              />
                              {f.replacement.prosthetic_component ===
                                "Healing Abutment" && (
                                <TextInput
                                  style={s.input}
                                  ...
                                  onChangeText={(v) =>
                                    setReplField(i, { healing_abutment_mm: v })
                                  }
                          ...
                          {f.replacement.procedure_type ===
                            ...
                            !hideImmediate && (
                              <View style={{ gap: 8 }}>
                                ...
                                <Dropdown
                                  value={
                                    f.replacement.immediate_loading_prosthesis
                                  }
                                  options={immOptions}
                                  placeholder="Select prosthesis type"
                                  onChange={(v) =>
                                    setReplField(i, {
                                      immediate_loading_prosthesis: v,
                                      immediate_loading_prosthesis_other: "",
                                    })
                                  }
                                ...
                                {f.replacement.immediate_loading_prosthesis ===
                                  ...
                                  <TextInput
                                    ...
                                    onChangeText={(v) =>
                                      setReplField(i, {
                                        immediate_loading_prosthesis_other: v,
                                      })
                                    }
                          ...
                          <View style={{ marginTop: 8 }}>
                            ...
                            >
                              <TouchableOpacity
                                style={[
                                  s.pillTiny,
                                  f.replacement.aug_used === "Yes" && s.pillOn,
                                ]}
                                onPress={() =>
                                  setReplField(i, { aug_used: "Yes" })
                                }
                                data-testid={`imp-${i}-repl-aug-yes`}
                                testID={`imp-${i}-repl-aug-yes`}
                              >
                                <Text
                                  style={[
                                    s.pillTT,
                              ...
                              <TouchableOpacity
                                ...
                                data-testid={`imp-${i}-repl-aug-no`}
                                testID={`imp-${i}-repl-aug-no`}
                              >
                                <Text
                                  style={[
                                    s.pillTT,
            ...
            <View style={{ flexDirection: "row", gap: 8 }}>
              ...
              >
                {saving ? (
                  ...
                  <>
                    <Ionicons name="save-outline" size={16} color="#1565C0" />
                    <Text style={s.submitAltT}>Update &amp; go back</Text>
                  </>
                )}
              </TouchableOpacity>
              <TouchableOpacity
              ...
              >
                {saving ? (
                  ...
                  <>
                    ...
                    <Text style={s.submitT}>Continue to Phase 3</Text>
                  </>
                )}
              </TouchableOpacity>
            </View>
          ...
          <View style={s.endTreatmentGlobalWrap}>
            ...
            <Text style={s.endTreatmentGlobalHelp}>
              Terminates the entire case. No further replacements or phases can
              be recorded.
            </Text>
          </View>
        </ScrollView>
      ...
      <Modal
        transparent
        visible={endModal.open}
        animationType="fade"
        onRequestClose={() => setEndModal((m) => ({ ...m, open: false }))}
      >
        <Pressable
          style={s.endModalBackdrop}
          onPress={() => setEndModal((m) => ({ ...m, open: false }))}
        >
          <Pressable
            style={s.endModalSheet}
          ...
          >
            <View style={s.endModalHeader}>
              ...
              <TouchableOpacity
                onPress={() => setEndModal((m) => ({ ...m, open: false }))}
                testID="end-treatment-close"
              >
                <Ionicons name="close" size={22} color="#37474F" />
              </TouchableOpacity>
            </View>
            <Text style={s.endModalWarn}>
              This terminates the entire case. No further replacements or phases
              can be recorded for this patient.
            </Text>
            ...
            <Dropdown
              ...
              onChange={(v) =>
                setEndModal((m) => ({
                  ...m,
                  decision_maker: v as "Patient" | "Operator",
                }))
              }
              testID="end-treatment-decision"
            />

            <Text style={[s.lbl, { marginTop: 12 }]}>
            ...
            <TouchableOpacity
              style={[
                s.endTreatmentGlobalBtn,
                { marginTop: 16 },
                endModal.submitting && { opacity: 0.6 },
              ]}
            ...
            >
              {endModal.submitting ? (
                ...
                <>
                  <Ionicons name="close-circle" size={18} color="#FFF" />
                  <Text style={s.endTreatmentGlobalT}>
