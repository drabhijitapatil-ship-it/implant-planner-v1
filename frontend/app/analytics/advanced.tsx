...
const PALETTE = ['#1E88E5', '#43A047', '#FB8C00', '#8E24AA', '#00838F', '#6D4C41', '#C62828', '#5E35B1'];
...
export default function AdvancedAnalyticsHub() {
  ...
  if (!canAccess) {
    return (
      <SafeAreaView style={s.center}>
        ...
        <Text style={{ marginTop: 12, color: '#546E7A' }}>Advanced Analytics not available for your role.</Text>
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView style={s.container} edges={['top']}>
      <View style={s.header}>
        <TouchableOpacity onPress={() => router.back()} style={s.headerIcon} testID="adv-analytics-back">
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 60 }}>
        ...
        {section === 'crosstab' && <CrossTabPane fromDate={fromDate} toDate={toDate} />}
        ...
        <View style={s.pillRow}>
          {(['procedure_type', 'system'] as const).map(g => (
            <TouchableOpacity key={g} onPress={() => setGroupBy(g)}
              style={[s.pill, groupBy === g && s.pillActive]} testID={`km-group-${g}`}>
              <Text style={[s.pillTxt, groupBy === g && s.pillTxtActive]}>
                {g === 'procedure_type' ? 'By Procedure Type' : 'By Implant System'}
              </Text>
            </TouchableOpacity>
          ))}
        </View>
              ...
              <View style={s.legendWrap}>
                {data.curves.slice(0, 6).map((c: any, idx: number) => (
                  <View key={c.key} style={s.legendItem}>
                    <View style={[s.legendDot, { backgroundColor: PALETTE[idx % PALETTE.length] }]} />
                    <Text style={s.legendTxt} numberOfLines={1}>
                      {c.key} · n={c.n_implants} · {(c.final_survival * 100).toFixed(1)}%
                    </Text>
                  </View>
                ))}
              </View>
...
// ─────────────────────── Torque × ISQ Scatter ───────────────────────
function ScatterPane({ fromDate, toDate }: { fromDate: string; toDate: string }) {
  const [data, setData] = useState<any>(null);
  const [loading, setLoading] = useState(false);
  useEffect(() => {
    (async () => {
  ...
  const width = 340, height = 260, padL = 40, padB = 30, padT = 10, padR = 10;
  const xMin = 0, xMax = 80;                   // Torque
  const yMin = 40, yMax = 90;                  // ISQ
  const xScale = (v: number) => padL + ((v - xMin) / (xMax - xMin)) * (width - padL - padR);
  const yScale = (v: number) => padT + (1 - (v - yMin) / (yMax - yMin)) * (height - padT - padB);

  return (
    <View style={s.pane}>
      <SectionCard title="Torque × ISQ scatter" hint="Green = still active. Red = failed / replaced. The green cluster in the top-right is your primary-stability sweet spot.">
        {loading ? <ActivityIndicator style={{ marginVertical: 20 }} /> :
          !data?.points?.length ? <EmptyMsg /> : (
            <>
              <Svg width={width} height={height} testID="scatter-chart">
                ...
                <Line x1={padL} y1={height - padB} x2={width - padR} y2={height - padB} stroke="#CFD8DC" strokeWidth="1" />
                {[0, 20, 40, 60, 80].map(v => (
                  <SvgText key={v} x={xScale(v)} y={height - padB + 12} fontSize="8" fill="#78909C" textAnchor="middle">{v}</SvgText>
                ))}
                {[50, 60, 70, 80].map(v => (
                  <SvgText key={v} x={padL - 5} y={yScale(v) + 3} fontSize="8" fill="#78909C" textAnchor="end">{v}</SvgText>
                ))}
                {/* Sweet-spot band ~35-45 Ncm × 70-80 ISQ */}
                <Rect x={xScale(35)} y={yScale(80)} width={xScale(45) - xScale(35)} height={yScale(70) - yScale(80)} fill="#E8F5E9" stroke="#A5D6A7" strokeDasharray="3,3" />
                {/* points */}
                {data.points.slice(0, 400).map((p: any, idx: number) => {
                  const color = p.status === 'Failed' || p.status === 'Replaced' ? '#C62828' : '#43A047';
                  return <Circle key={idx} cx={xScale(p.torque)} cy={yScale(p.isq)} r="3.5" fill={color} opacity="0.75" />;
                })}
                <SvgText x={width / 2} y={height - 5} fontSize="9" fill="#546E7A" textAnchor="middle">Insertion Torque (Ncm)</SvgText>
                <SvgText x={12} y={height / 2} fontSize="9" fill="#546E7A" textAnchor="middle" transform={`rotate(-90 12 ${height / 2})`}>ISQ</SvgText>
              </Svg>
              <View style={s.sweetCard} data-testid="scatter-sweet-spot">
                <Text style={s.sweetHead}>Survivor sweet-spot (n={data.sweet_spot.n_survivors})</Text>
                <Text style={s.sweetLine}>Torque: median <Text style={{ fontWeight: '800' }}>{data.sweet_spot.median_torque ?? '—'} Ncm</Text> · mean {data.sweet_spot.mean_torque ?? '—'}</Text>
                <Text style={s.sweetLine}>ISQ: median <Text style={{ fontWeight: '800' }}>{data.sweet_spot.median_isq ?? '—'}</Text> · mean {data.sweet_spot.mean_isq ?? '—'}</Text>
              </View>
              <Text style={s.paneNote}>Showing up to 400 points ({data.n_points} total)</Text>
            </>
          )}
      </SectionCard>
    </View>
...
function HeatmapPane({ fromDate, toDate }: { fromDate: string; toDate: string }) {
  const [data, setData] = useState<any>(null);
  const [loading, setLoading] = useState(false);
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        if (fromDate) params.from_date = fromDate;
        if (toDate) params.to_date = toDate;
        const r = await api.get('/analytics/bone-procedure-heatmap', { params });
        setData(r.data);
      } finally { setLoading(false); }
    })();
  }, [fromDate, toDate]);
  ...
  return (
    <View style={s.pane}>
      <SectionCard title="Bone × Procedure × Outcome" hint="Rows = Misch/L-Z bone density (D1-D4). Columns = procedure types. Colour = survival %.">
        {loading ? <ActivityIndicator style={{ marginVertical: 20 }} /> :
          !data?.matrix?.length ? <EmptyMsg /> : (
            <ScrollView horizontal showsHorizontalScrollIndicator={false} testID="heatmap-scroll">
              <View>
                {/* header row */}
                <View style={{ flexDirection: 'row', marginLeft: 45 }}>
                  {data.procedure_types.map((pt: string) => (
                    <View key={pt} style={{ width: 68, alignItems: 'center' }}>
                      <Text style={s.heatColHead} numberOfLines={2}>{pt}</Text>
                    </View>
                  ))}
                </View>
                {data.matrix.map((row: any) => (
                  <View key={row.bone} style={{ flexDirection: 'row', alignItems: 'center' }}>
                    <View style={{ width: 45 }}><Text style={s.heatRowHead}>{row.bone}</Text></View>
                    {row.cells.map((c: any, ci: number) => (
                      <View key={ci} style={[s.heatCell, { backgroundColor: cellColor(c.survival) }]}>
                        <Text style={s.heatCellVal}>{c.survival !== null ? `${c.survival}%` : '—'}</Text>
...
// ─────────────────────── Cross-Tab Builder ───────────────────────
const DIMS = ['procedure_type', 'system', 'bone_type', 'region', 'diameter', 'length'] as const;
const METRICS = ['count', 'success_rate', 'mean_torque', 'mean_isq', 'mean_days'] as const;
const METRIC_LABEL: Record<string, string> = { count: 'Count', success_rate: 'Success %', mean_torque: 'Mean Torque', mean_isq: 'Mean ISQ', mean_days: 'Mean Days' };
const DIM_LABEL: Record<string, string> = { procedure_type: 'Procedure Type', system: 'System', bone_type: 'Bone Type', region: 'Region', diameter: 'Ø', length: 'Length' };

function CrossTabPane({ fromDate, toDate }: { fromDate: string; toDate: string }) {
  ...
  return (
    <View style={s.pane}>
      <SectionCard title="Cross-tab builder" hint="Pivot any dimension against any other. Great for research questions.">
        <Text style={s.miniLbl}>Row</Text>
        <View style={s.chipsRow}>
          {DIMS.map(d => (
            <TouchableOpacity key={d} onPress={() => setRowDim(d)} style={[s.chipTiny, rowDim === d && s.chipTinyActive]} testID={`ct-row-${d}`}>
              <Text style={[s.chipTinyTxt, rowDim === d && s.chipTinyTxtActive]}>{DIM_LABEL[d]}</Text>
            </TouchableOpacity>
          ))}
        </View>
        <Text style={s.miniLbl}>Column (optional)</Text>
        <View style={s.chipsRow}>
          <TouchableOpacity onPress={() => setColDim(null)} style={[s.chipTiny, colDim === null && s.chipTinyActive]} testID="ct-col-none">
            <Text style={[s.chipTinyTxt, colDim === null && s.chipTinyTxtActive]}>None</Text>
          </TouchableOpacity>
          {DIMS.filter(d => d !== rowDim).map(d => (
            <TouchableOpacity key={d} onPress={() => setColDim(d)} style={[s.chipTiny, colDim === d && s.chipTinyActive]} testID={`ct-col-${d}`}>
              <Text style={[s.chipTinyTxt, colDim === d && s.chipTinyTxtActive]}>{DIM_LABEL[d]}</Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        <View style={s.chipsRow}>
          {METRICS.map(m => (
            <TouchableOpacity key={m} onPress={() => setMetric(m)} style={[s.chipTiny, metric === m && s.chipTinyActive]} testID={`ct-metric-${m}`}>
              <Text style={[s.chipTinyTxt, metric === m && s.chipTinyTxtActive]}>{METRIC_LABEL[m]}</Text>
            </TouchableOpacity>
          ))}
        </View>
        <TouchableOpacity onPress={run} style={s.applyBtn} testID="ct-run"><Text style={s.applyBtnTxt}>Run</Text></TouchableOpacity>
        {loading ? <ActivityIndicator style={{ marginVertical: 20 }} /> :
          !data?.grid?.length ? <EmptyMsg /> : (
            <ScrollView horizontal showsHorizontalScrollIndicator style={{ marginTop: 10 }}>
              <View>
                <View style={{ flexDirection: 'row' }}>
                  <View style={[s.ctCell, s.ctHead, { width: 120 }]}><Text style={s.ctHeadTxt}>{DIM_LABEL[rowDim]}</Text></View>
                  {(data.cols as string[]).map(c => (
                    <View key={c} style={[s.ctCell, s.ctHead]}><Text style={s.ctHeadTxt} numberOfLines={2}>{c === '_total' ? 'Total' : c}</Text></View>
                  ))}
                </View>
                {data.grid.map((row: any) => (
                  <View key={row.row} style={{ flexDirection: 'row' }}>
                    <View style={[s.ctCell, { width: 120, backgroundColor: '#F5F7FB' }]}><Text style={s.ctRowLbl} numberOfLines={2}>{row.row}</Text></View>
                    {row.cells.map((c: any, ci: number) => (
                      <View key={ci} style={s.ctCell}>
                        <Text style={s.ctCellVal}>{c.value === null || c.value === undefined ? '—' : c.value}</Text>
...
function LearningCurvePane({ isFaculty }: { isFaculty: boolean }) {
  ...
  useEffect(() => { if (!isFaculty) load(); /* eslint-disable-next-line */ }, []);

  const width = 340, height = 200, padL = 35, padB = 25, padT = 10, padR = 10;
  ...
  return (
    <View style={s.pane}>
      <SectionCard title="Learning curve" hint="Running success rate as cases accumulate. A trend upward = the student is improving.">
        {isFaculty && (
        ...
        {loading ? <ActivityIndicator style={{ marginVertical: 20 }} /> :
          !series.length ? <EmptyMsg /> : (
            <>
              <Text style={s.paneNote}>{data.student_name || 'Student'} · {data.n_cases} cases</Text>
              <Svg width={width} height={height} testID="lc-chart">
                <Line x1={padL} y1={padT} x2={padL} y2={height - padB} stroke="#CFD8DC" />
                <Line x1={padL} y1={height - padB} x2={width - padR} y2={height - padB} stroke="#CFD8DC" />
                {[0, 50, 100].map(v => (
                  <SvgText key={v} x={padL - 5} y={yScale(v) + 3} fontSize="8" fill="#78909C" textAnchor="end">{v}%</SvgText>
                ))}
                {(() => {
                  ...
                  series.forEach((row: any, i: number) => {
                    if (row.running_success_rate === null || row.running_success_rate === undefined) return;
                    const x = xScale(i + 1); const y = yScale(row.running_success_rate);
                    d += d ? ` L ${x} ${y}` : `M ${x} ${y}`;
                  });
                  return d ? <Path d={d} stroke="#1E88E5" strokeWidth="2" fill="none" /> : null;
                })()}
                {series.map((row: any, i: number) => {
                  const color = row.status_bucket === 'completed' ? '#43A047'
                              : row.status_bucket === 'terminated' ? '#C62828'
                              : '#90A4AE';
                  return <Circle key={i} cx={xScale(i + 1)} cy={yScale(row.running_success_rate ?? 0)} r="3" fill={color} />;
                })}
                <SvgText x={width / 2} y={height - 5} fontSize="9" fill="#546E7A" textAnchor="middle">Case number</SvgText>
              </Svg>
            </>
          )}
      </SectionCard>
...
function CaseMixPane({ fromDate, toDate }: { fromDate: string; toDate: string }) {
  ...
  return (
    <View style={s.pane}>
      <SectionCard title="Case-Mix Index (CMI)" hint="Complexity-weighted volume. Higher CMI = handling more complex procedures. Weights: Zygomatic 5.0 · All-on-X 4.0 · All-on-4/6 3.5-3.8 · Sinus/GBR 2.5 · Immediate 2.0 · Multiple 1.5 · Single 1.0.">
        <View style={s.pillRow}>
          {(['students', 'supervisors'] as const).map(g => (
            <TouchableOpacity key={g} onPress={() => setScope(g)} style={[s.pill, scope === g && s.pillActive]} testID={`cmi-${g}`}>
              <Text style={[s.pillTxt, scope === g && s.pillTxtActive]}>By {g}</Text>
            </TouchableOpacity>
          ))}
        </View>
        {loading ? <ActivityIndicator style={{ marginVertical: 20 }} /> :
          !data?.rows?.length ? <EmptyMsg /> : (
            <View style={{ marginTop: 10 }}>
              {data.rows.slice(0, 30).map((r: any) => (
                <View key={r.user_id} style={s.cmiRow} testID={`cmi-row-${r.user_id}`}>
                  <Text style={s.cmiName} numberOfLines={1}>{r.user_name || r.user_id}</Text>
                  <View style={{ alignItems: 'flex-end' }}>
                    <Text style={s.cmiIdx}>{r.case_mix_index}</Text>
                    <Text style={s.cmiSub}>{r.n_cases} cases · Σ{r.complexity_score}</Text>
                  </View>
                </View>
              ))}
            </View>
...
function ComplicationsPane({ fromDate, toDate }: { fromDate: string; toDate: string }) {
  const [data, setData] = useState<any>(null);
  const [loading, setLoading] = useState(false);
  useEffect(() => {
    (async () => {
  ...
  return (
    <View style={s.pane}>
      <SectionCard title="Complication pareto" hint="Failure reasons ranked. The 80/20 line reveals the few reasons responsible for most events.">
        {loading ? <ActivityIndicator style={{ marginVertical: 20 }} /> :
          !data?.pareto?.length ? <EmptyMsg /> : (
            <>
              ...
              {data.pareto.slice(0, 15).map((p: any) => (
                <View key={p.reason} style={{ marginTop: 8 }}>
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 3 }}>
                    <Text style={s.paretoLbl} numberOfLines={1}>{p.reason}</Text>
                    <Text style={s.paretoRight}>{p.count} · {p.cum_pct}%</Text>
                  </View>
                  <View style={s.paretoBar}>
                    <View style={[s.paretoBarFill, { width: `${(p.count / max) * 100}%` }]} />
                  </View>
                </View>
              ))}
            </>
...
function BenchmarksPane({ fromDate, toDate }: { fromDate: string; toDate: string }) {
  ...
  return (
    <View style={s.pane}>
      <SectionCard title="Benchmarks vs literature" hint="Compare your survival to published ranges. Verdict badges: ▲ above literature · = on par · ▼ below.">
        {loading ? <ActivityIndicator style={{ marginVertical: 20 }} /> :
          !data?.rows?.length ? <EmptyMsg /> : (
            data.rows.map((r: any) => {
              const badgeColor = r.verdict === 'above' ? '#2E7D32'
                              : r.verdict === 'on_par' ? '#0277BD'
                              : r.verdict === 'below' ? '#C62828' : '#90A4AE';
              const badgeLabel = r.verdict === 'above' ? '▲ Above'
                              : r.verdict === 'on_par' ? '= On par'
                              : r.verdict === 'below' ? '▼ Below' : 'n/a';
              return (
                <View key={r.procedure_type} style={s.benchCard} testID={`bench-${r.procedure_type.replace(/\s+/g, '-').toLowerCase()}`}>
                  ...
                  <View style={{ marginTop: 6, flexDirection: 'row', gap: 12 }}>
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={s.benchYou}>{r.your_survival !== null ? `${r.your_survival}%` : '—'}</Text>
                      <Text style={s.benchN}>n={r.your_n}</Text>
                    </View>
                    <View style={{ flex: 1 }}>
                      <Text style={s.benchLbl}>Literature</Text>
                      <Text style={s.benchLit}>{r.literature_low}–{r.literature_high}%</Text>
                    </View>
                  </View>
                  <Text style={s.benchCite} numberOfLines={2}>{r.citation}</Text>
                </View>
              );
            })
          )}
...
// ─────────────────────── Research Export ───────────────────────
function ResearchExportPane({ fromDate, toDate }: { fromDate: string; toDate: string }) {
      ...
      <SectionCard title="Research export (de-identified)" hint="JSON bundle with per-implant rows + a data dictionary sheet. All identifiers are hashed. Safe for statistical analysis or publication.">
        ...
        {preview && (
          <View style={{ marginTop: 12 }} testID="research-preview-block">
            <Text style={s.paneNote}>Rows: <Text style={{ fontWeight: '800' }}>{preview.n_rows}</Text> · Generated: {preview.generated_at?.slice(0, 19)}Z</Text>
            <Text style={[s.paneNote, { marginTop: 8, fontWeight: '700' }]}>Data dictionary</Text>
            {preview.data_dictionary.slice(0, 6).map((d: any) => (
              <Text key={d.field} style={s.dictLine}>
                <Text style={{ fontWeight: '700' }}>{d.field}</Text> · {d.type} · {d.desc}
              </Text>
            ))}
            <Text style={[s.paneNote, { marginTop: 6, fontStyle: 'italic' }]}>+{preview.data_dictionary.length - 6} more fields</Text>
          </View>
        )}
      </SectionCard>
    </View>
...
function EmptyMsg() { return <Text style={{ color: '#B0BEC5', fontStyle: 'italic', marginTop: 10 }}>No data yet for this view.</Text>; }
...
const s = StyleSheet.create({
  ...
  cardTitle: { fontSize: 14, fontWeight: '800', color: '#1A2332', marginBottom: 4 },
  cardHint: { fontSize: 11, color: '#78909C', lineHeight: 15, marginBottom: 10 },
  paneNote: { fontSize: 11, color: '#78909C', marginTop: 6 },

  pillRow: { flexDirection: 'row', gap: 6, marginBottom: 8 },
  pill: { paddingHorizontal: 10, paddingVertical: 7, borderRadius: 999, borderWidth: 1.5, borderColor: '#CFD8DC', backgroundColor: '#FAFCFF' },
  ...
  applyBtn: { alignSelf: 'flex-start', paddingHorizontal: 16, paddingVertical: 9, backgroundColor: '#1E88E5', borderRadius: 999, marginTop: 8 },
  applyBtnTxt: { color: '#FFF', fontWeight: '700', fontSize: 12, letterSpacing: 0.3, textAlign: 'center' },

  legendWrap: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 8 },
  legendItem: { flexDirection: 'row', alignItems: 'center', gap: 4, flexBasis: '45%' },
  legendDot: { width: 10, height: 10, borderRadius: 5 },
  legendTxt: { fontSize: 10, color: '#546E7A', flex: 1 },

  sweetCard: { marginTop: 10, backgroundColor: '#E8F5E9', borderRadius: 10, padding: 10, borderWidth: 1, borderColor: '#A5D6A7' },
  sweetHead: { fontSize: 12, fontWeight: '800', color: '#1B5E20' },
  sweetLine: { fontSize: 11, color: '#33691E', marginTop: 3 },

  heatColHead: { fontSize: 9, fontWeight: '700', color: '#546E7A', textAlign: 'center', paddingHorizontal: 3, marginBottom: 3 },
  heatRowHead: { fontSize: 11, fontWeight: '800', color: '#1A2332' },
  heatCell: { width: 66, height: 42, marginRight: 2, marginBottom: 2, alignItems: 'center', justifyContent: 'center' },
  ...
  chipTiny: { paddingHorizontal: 9, paddingVertical: 5, borderRadius: 999, borderWidth: 1, borderColor: '#CFD8DC', backgroundColor: '#FAFCFF' },
  ...
  miniLbl: { fontSize: 10, color: '#78909C', textTransform: 'uppercase', letterSpacing: 0.4, marginTop: 8, marginBottom: 4, fontWeight: '700' },

  ctCell: { width: 90, minHeight: 44, borderWidth: 1, borderColor: '#F0F4F8', alignItems: 'center', justifyContent: 'center', paddingHorizontal: 4 },
  ctHead: { backgroundColor: '#F5F7FB' },
  ctHeadTxt: { fontSize: 10, fontWeight: '800', color: '#546E7A', textAlign: 'center' },
  ...
  cmiRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: '#F0F4F8' },
  cmiName: { fontSize: 13, fontWeight: '600', color: '#1A2332', flex: 1, marginRight: 8 },
  cmiIdx: { fontSize: 18, fontWeight: '800', color: '#1565C0' },
  cmiSub: { fontSize: 10, color: '#90A4AE' },

  paretoLbl: { fontSize: 11, fontWeight: '700', color: '#1A2332', flex: 1, marginRight: 8 },
  paretoRight: { fontSize: 11, fontWeight: '700', color: '#78909C' },
  paretoBar: { height: 10, backgroundColor: '#ECEFF1', borderRadius: 5, overflow: 'hidden' },
  paretoBarFill: { height: '100%', backgroundColor: '#C62828' },

  benchCard: { padding: 12, borderRadius: 10, backgroundColor: '#FAFCFF', borderWidth: 1, borderColor: '#E1E7EF', marginTop: 8 },
  ...
  benchLbl: { fontSize: 9, color: '#78909C', letterSpacing: 0.4, textTransform: 'uppercase', fontWeight: '700' },
