...
import {
  ...
  SURGERY_APPROACH_TYPES,
  GUIDED_SURGERY_TYPES,
  STATIC_GUIDE_TYPES,
  SLEEVE_TYPES,
  DYNAMIC_NAV_SYSTEMS,
        ...
        <ScrollView style={styles.dropdownList} nestedScrollEnabled={true}>
          {options.map((opt) => (
            <TouchableOpacity
              ...
              style={[
                styles.dropdownItem,
                values.includes(opt) && styles.dropdownItemActive,
              ]}
            ...
            >
              ...
              >
                <Ionicons
                  ...
                  size={20}
                  color={values.includes(opt) ? "#1A73E8" : "#999"}
                />
                <Text
                  style={[
                    styles.dropdownItemText,
                    values.includes(opt) && styles.dropdownItemTextActive,
                  ]}
                >
                  {opt}
                </Text>
...
function Dropdown({
  ...
  const triggerTestId =
    customTestId || `dropdown-${label.toLowerCase().replace(/\s+/g, "-")}`;
  return (
    <View style={styles.fieldContainer}>
      <Text style={styles.label}>
        {label}
      ...
      {open && (
        <View style={calStyles.container}>
          ...
          <View style={calStyles.dayNamesRow}>
            {dayNames.map((dn) => (
              <Text key={dn} style={calStyles.dayName}>
                {dn}
              </Text>
            ))}
          </View>
...
export default function NewProcedureScreen() {
  ...
  const [step, setStep] = useState<"details" | "implants" | "review">(
    "details",
  );
  ...
  const [createdProcedureId, setCreatedProcedureId] = useState<string | null>(
    null,
  );
  ...
  const [existingImplantDraft, setExistingImplantDraft] = useState<any | null>(
    null,
  );
  ...
  const [existingOrigProcedure, setExistingOrigProcedure] =
    useState<string>("");
  ...
  const onScrollExisting = (e: any) => {
    ...
    for (let i = 0; i < existingStepYs.current.length; i++) {
      if (existingStepYs.current[i] && existingStepYs.current[i] <= y + 120)
        idx = i;
    }
  ...
  const [formData, setFormData] = useState({
    ...
    supervisor_name:
      user?.role === "supervisor" || user?.role === "implant_incharge"
        ...
        : "",
    implant_incharge_id:
      user?.role === "implant_incharge" ? user?.id || "" : "",
    implant_incharge_name:
      user?.role === "implant_incharge" ? user?.name || "" : "",
    receipt_number: "",
    amount_paid: "",
    procedure_date: "",
    procedure_time: "",
  ...
  const [augmentationRequired, setAugmentationRequired] = useState<
    "" | "Yes" | "No"
  >("");
    ...
    lookupTimer.current = setTimeout(async () => {
      try {
        const res = await api.get(
          `/procedures/patient-lookup?registration_number=${encodeURIComponent(reg)}`,
        );
        if (res.data?.found) {
          ...
          setFormData((prev) => ({
            ...prev,
            linked_parent_case_id: res.data.latest_case_id || "",
          }));
        } else {
          setPatientLookup(null);
          setLookupAutofilled(false);
  ...
  const applyPatientAutofill = () => {
    ...
    setFormData((prev) => ({
      ...
      Object.keys(p.medical_assessment).length
        ? p.medical_assessment
        : prev.medical_assessment) as Record<string, string>,
  ...
  useEffect(() => {
    ...
    if (
      ...
      !!createdProcedureId ||
      !!patientLookup?.found ||
      nameCancelledFor.toLowerCase() === nm.toLowerCase()
    ) {
      setNameLookup(null);
      return;
    }
    nameTimer.current = setTimeout(async () => {
      try {
        const res = await api.get(
          `/procedures/patient-name-lookup?patient_name=${encodeURIComponent(nm)}`,
        );
  ...
  const submitAugmentationCase = async () => {
    ...
    try {
      const res = await api.post("/procedures/augmentation-case", {
        ...
        patient_email: (formData as any).patient_email || "",
      ...
      Alert.alert(
        "Case Created",
        "Pre-Implant Augmentation case created. Fill Step 1 — Pre-procedure Details from the case screen.",
      );
      ...
      setSubmittingAug(false);
    }
  };

  const effectiveProcType = isExistingImplantCase
    ? existingOrigProcedure
  ...
  const prostheticOptions = getProstheticOptions(
    formData.implant_procedure_type,
  ...
  useEffect(() => {
    ...
    setFormData((prev) => {
      ...
      if (cur.length === fresh.length && cur.every((t, i) => t === fresh[i]))
        return prev;
          ...
          try {
            ...
            if (proc.status === "draft") {
              ...
              setFormData((prev) => ({
                ...
                teeth_present: Array.isArray(proc.teeth_present)
                  ? proc.teeth_present
                  : [],
                missing_teeth: Array.isArray(proc.missing_teeth)
                  ? proc.missing_teeth
                  : [],
                ...
                clinical_exam_per_site:
                  proc.clinical_exam_per_site &&
                  typeof proc.clinical_exam_per_site === "object"
                    ? proc.clinical_exam_per_site
                    : {},
              ...
              if (proc.checklist?.pre_surgical?.items) {
                ...
                proc.checklist.pre_surgical.items.forEach((item: any) => {
                  if (item.id && typeof item.value === "boolean")
                    restored[item.id] = item.value;
                });
              ...
              if (
                Array.isArray(proc.cbct_files) &&
                proc.cbct_files.length > 0
              ) {
                const restoredCbct: (null | {
                  filename: string;
                ...
                if (proc.cbct_files.length > 2)
                  setExtraCbctCount(proc.cbct_files.length - 2);
              ...
              if (
                Array.isArray(proc.intraoral_photos) &&
                proc.intraoral_photos.length > 0
              ) {
                const restored: (null | {
                  filename: string;
                  original_name: string;
                ...
                if (proc.intraoral_photos.length > 2)
                  setExtraIntraoralCount(proc.intraoral_photos.length - 2);
              }
              ...
              const isExistingImplantDraft =
                ...
                (Array.isArray(proc.existing_implants) &&
                  proc.existing_implants.length > 0) ||
                !!proc.original_procedure_type;
              if (isExistingImplantDraft) {
                setExistingImplantDraft(proc);
                setFormData((prev) => ({
                  ...prev,
                  implant_procedure_type: "Existing Implant",
                }));
                setStep("details");
              } else {
                setStep("implants");
            ...
            /* ignore — draft may have been deleted */
          }
        };
        loadDraft();
  ...
  useEffect(() => {
    if (existingImplantDraft) {
      setStep("details");
      setFormData((prev) =>
        prev.implant_procedure_type === "Existing Implant"
          ? prev
          : { ...prev, implant_procedure_type: "Existing Implant" },
      );
    }
      ...
      if (
        ...
        try {
          await AsyncStorage.setItem(
            FORM_STORAGE_KEY,
            JSON.stringify({ formData, checklistItems }),
          );
        } catch {
          /* ignore */
        }
  ...
  const [bookedSlots, setBookedSlots] = useState<
    Record<string, { patient_name: string; scheduled_by: string }>
  >({});
  ...
  const [cbctFiles, setCbctFiles] = useState<
    (null | { filename: string; original_name: string; content_type: string })[]
  >([null, null]);
  ...
  const [intraoralPhotos, setIntraoralPhotos] = useState<
    (null | {
      filename: string;
      original_name: string;
      content_type: string;
  ...
  >([null, null]);
  const [intraoralUploadingIdx, setIntraoralUploadingIdx] = useState<
    number | null
  >(null);
  ...
  const pickConsentForm = async () => {
    try {
      ...
      Alert.alert(
        "Upload Failed",
        err.response?.data?.detail || "Could not upload consent form",
      );
    } finally {
    ...
    const loadFaculty = async () => {
      try {
        ...
        setSupervisors(
          users.filter(
            (u: any) =>
              u.role === "supervisor" || u.role === "implant_incharge",
          ),
        );
  ...
  useEffect(() => {
    ...
    setOpenHour(null);
    setOpenMinute(0);
    setOpenMeridiem("AM"); // reset open-mode picker on date change
    const fetchSlots = async () => {
      try {
        const res = await api.get(
          `/procedures/slots/${formData.procedure_date}`,
        );
        setBookedSlots(res.data?.booked_slots || {});
        setSchedMode(res.data?.mode || "default");
        setDaySlots(res.data?.day_slots || []);
        if (res.data?.open_window_hours)
          setOpenWindowHours(res.data.open_window_hours);
      } catch {
        setBookedSlots({});
      }
    };
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      api
        .get(`/procedures/slots/${formData.procedure_date}`)
        .then((res) => {
          setBookedSlots(res.data?.booked_slots || {});
          setSchedMode(res.data?.mode || "default");
          setDaySlots(res.data?.day_slots || []);
  ...
  const renderHaematologySection = (testidSuffix: string) => (
    <View
      style={styles.haematologyWrap}
      testID={`haematology-${testidSuffix}`}
      data-testid={`haematology-${testidSuffix}`}
    >
      <Text style={styles.haematologyHeading}>
        Haematology Examination{" "}
        <Text style={styles.haematologyOptional}>(all optional)</Text>
      </Text>
      {HAEMATOLOGY_FIELDS.map((f) => (
        <View key={f.id} style={styles.haematologyRow}>
          <View style={{ flex: 1 }}>
            ...
            {f.hint ? (
              <Text style={styles.haematologyHint}>{f.hint}</Text>
            ) : null}
          </View>
          <TextInput
  ...
  const pickCbctFileAtIndex = async (idx: number) => {
    try {
      ...
      Alert.alert(
        "Upload Failed",
        err.response?.data?.detail || "Could not upload CBCT file",
      );
    } finally {
  ...
  const pickIntraoralAtIndex = async (idx: number) => {
    try {
      ...
      Alert.alert(
        "Upload Failed",
        err.response?.data?.detail || "Could not upload intra-oral photograph",
      );
    } finally {
  ...
  const handleContinueToImplants = async () => {
    ...
    const sectionDetail: Array<{ label: string; fields: string[] }> =
      requiredIdx
        .filter((i) => !existingStepDone[i])
        .map((i) => ({
          label: FLOW_STEP_LABELS[i],
          fields: flowStepMissing[i],
        }));
    ...
    if (!isExistingImplantCase) {
      if (!sanitized.procedure_surgery_type) {
        Alert.alert(
          "Missing Field",
          "Please select the Procedure Type (Free Hand / Combination / Guided Surgery).",
        );
        return;
      }
      if (isGuidedApproach(sanitized.procedure_surgery_type)) {
        if (!sanitized.guided_surgery_type) {
          Alert.alert(
            "Missing Field",
            "Please select the Type of Guided Surgery.",
          );
          return;
        }
        if (sanitized.guided_surgery_type === "Static Guide") {
          if (!sanitized.static_guide_type) {
            Alert.alert(
              "Missing Field",
              "Please select the Type of Static Guide.",
            );
            return;
          }
        ...
        if (
          sanitized.guided_surgery_type === "Dynamic Navigation" &&
          !sanitized.dynamic_nav_system
        ) {
          Alert.alert(
            "Missing Field",
            "Please select the Dynamic Navigation Surgery System.",
          );
          return;
        }
      }
    ...
    if (
      PROCEDURES_WITH_NUM_IMPLANTS_QUESTION.has(
        sanitized.implant_procedure_type,
      )
    ) {
      if (!sanitized.num_implants) {
        Alert.alert(
          "Missing Field",
          'Please pick "Single Implant" or "Multiple Implants" under Number of Implants.',
        );
        return;
      }
    ...
    if (sanitized.implant_procedure_type === "Sinus Lift") {
      if (!sanitized.sinus_lift_type) {
        Alert.alert(
          "Missing Field",
          "Please select Type of Sinus Lift (Direct or Indirect).",
        );
        return;
      }
      if (!sanitized.bone_graft_material_details?.trim()) {
        Alert.alert(
          "Missing Field",
          "Please enter Details of Bone Graft Material.",
        );
        return;
      }
    ...
    if (!cbctFiles[0] || !cbctFiles[1]) {
      Alert.alert(
        "Missing Field",
        "Please upload both mandatory CBCT Reports before continuing.",
      );
      return;
    }
    ...
    if (sanitized.prosthetic_plan === "Overdenture with Attachment") {
      if (!sanitized.attachment_type) {
        Alert.alert(
          "Missing Field",
          "Please select the Type of Attachment for the Overdenture.",
        );
        return;
      }
      if (
        sanitized.attachment_type === "Other" &&
        !sanitized.attachment_type_other?.trim()
      ) {
        Alert.alert(
          "Missing Field",
          "Please specify the custom Attachment Type.",
        );
        return;
      }
    }
    ...
    try {
      ...
      const usingPerSite =
        ...
        sanitized.implant_procedure_type !== "Single Conventional Implant" &&
        !(
          NON_FULL_ARCH_TYPES.has(sanitized.implant_procedure_type) &&
          sanitized.prosthetic_plan === "Overdenture with Attachment"
        );
      ...
      if (usingPerSite) {
        ...
        const firstLeader =
          runs.length > 0
            ? clusterLeader(runs[0].positions) || runs[0].positions[0]
            : "";
        const firstSite =
          (sanitized.clinical_exam_per_site || {})[firstLeader] || {};
      ...
      const payload = {
        ...
        bone_graft_specifications: sanitizeString(
        ...
        prosthetic_plan:
          sanitized.prosthetic_plan === "Other"
      ...
      if (augResumeId) {
        ...
        res = await api.put(
          `/procedures/${augResumeId}/augmentation/complete-phase1`,
          payload,
        );
      } else {
      ...
      if (isFullArch && procId) {
        ...
        if (Object.keys(atrophyPayload).length > 0) {
          try {
            await api.put(
              `/procedures/${procId}/atrophy-assessment`,
              atrophyPayload,
            );
          } catch {
            /* best effort — don't block case creation */
          }
        }
      }
  ...
  if (step === "implants" && createdProcedureId) {
    return (
      <SafeAreaView
        ...
        data-testid="step2-implant-selection-view"
      >
        ...
        <View style={styles.submitContainer}>
          {phase1Done ? (
            ...
            >
              ...
              <TouchableOpacity
                onPress={() =>
                  router.replace(`/procedures/${createdProcedureId}`)
                }
                style={{ marginTop: 14 }}
                testID="phase1-view-case-link"
              >
                <Text
                  style={{
                    color: "#1565C0",
            ...
            <View style={{ width: "100%" }}>
                ...
                onPress={async () => {
                  ...
                  try {
                    const planRes = await api.get(
                      `/procedures/${createdProcedureId}/implant-plan`,
                    );
                    fetchedPlan = planRes.data?.implant_plans || [];
                    const positions: string[] = fetchedPlan.map(
                      (p: any) => p.position,
                    );
                    const finalCheck = validateImplantSelection(
                      formData.implant_procedure_type,
                      formData.teeth_present,
                      positions,
                    );
                    if (finalCheck.block) {
                    ...
                    if (finalCheck.bridgeCandidates.length > 0) {
                      const lines = finalCheck.bridgeCandidates
                        .map(
                          (c) =>
                    ...
                    if (finalCheck.cantileverCandidates.length > 0) {
                      const lines = finalCheck.cantileverCandidates
                        .map(
                          (c) =>
                      ...
                      { text: "Cancel" },
                      {
                        ...
                        onPress: async () => {
                          try {
                            ...
                            Alert.alert(
                              "Error",
                              e.response?.data?.detail || "Failed to submit",
                            );
                          }
                        },
                      },
              ...
              >
                <Ionicons
                  name={isPlanComplete ? "checkmark-circle" : "lock-closed"}
                  size={20}
                  color="#FFF"
                />
              ...
              {!isPlanComplete && (
                <Text
                  ...
                  data-testid="incomplete-implant-warning"
                >
                  Please select implants for all missing teeth to activate
                  submission.
                </Text>
              )}
            </View>
  ...
  if (step === "review" && createdProcedureId) {
    ...
    const row = (label: string, value?: string | number | null) => {
      ...
      return (
        <View key={label} style={{ flexDirection: "row", paddingVertical: 4 }}>
          ...
          <Text style={{ flex: 1, fontSize: 13, color: "#263238" }}>
            {String(value)}
          </Text>
        </View>
      );
    ...
    const confirmSubmit = async () => {
      ...
      try {
        ...
        if (isInchargeUser) {
          try {
            await api.post(`/procedures/${createdProcedureId}/approve`, {
              action: "approve",
              comment: "",
            });
          } catch {}
    ...
    return (
      <SafeAreaView
        style={{ flex: 1, backgroundColor: "#F5F7FA" }}
        data-testid="step-review-view"
      >
        <View style={styles.stepHeader}>
          <BackButton
            onPress={() => setStep("implants")}
        ...
        >
          ...
          {section(
            ...
            <>
              ...
              {row(
                "Age / Sex",
                [formData.age, formData.sex].filter(Boolean).join(" / "),
              )}
              ...
              {row(
                "Date / Time",
                [formData.procedure_date, formData.procedure_time]
                  .filter(Boolean)
                  .join(" "),
              )}
          ...
          {section(
            ...
            <>
              ...
              {row(
                ...
                Object.keys(formData.edentulous_site_measurements || {}).length
          ...
          {section(
            ...
            <>
              {Object.entries(formData.medical_assessment || {}).map(([k, v]) =>
                row(k.replace(/_/g, " "), v as string),
              )}
              {row(
                "Pre-Surgical Checklist",
                `${checkedCount} of ${checklistTotal} items checked`,
              )}
            </>,
          )}
                ...
                {reviewPlan.map((p: any, idx: number) => (
                  <View
                    ...
                    style={{
                      flexDirection: "row",
                      paddingVertical: 4,
                      borderBottomWidth: idx < reviewPlan.length - 1 ? 1 : 0,
                  ...
                  >
                    ...
                    <Text style={{ flex: 1, fontSize: 13, color: "#263238" }}>
                      {p.brand} {p.system} — {p.diameter}×{p.length}mm
                    </Text>
                  </View>
                ))}
              </>
        ...
        <View style={styles.submitContainer}>
          <TouchableOpacity
            style={
              loading ? { ...styles.submitBtn, opacity: 0.6 } : styles.submitBtn
            }
          ...
          >
            {loading ? (
              ...
              <>
                ...
                <Text style={styles.submitBtnText}>
                  {isInchargeUser ? "Confirm & Mark Done" : "Confirm & Submit"}
                </Text>
              </>
            )}
          </TouchableOpacity>
  ...
  if (!skipStrictPhase1) {
    ...
    if (!formData.registration_number?.trim())
      missCaseDetails.push("Registration number");
    if (!formData.chief_complaint?.trim())
      missCaseDetails.push("Chief complaint");
    if (!formData.supervisor_id) missCaseDetails.push("Supervising faculty");
    if (!formData.implant_incharge_id)
      missCaseDetails.push("Implant in-charge");
    if (!formData.receipt_number?.trim())
      missCaseDetails.push("Receipt number");
    if (!formData.amount_paid) missCaseDetails.push("Amount paid");

    if (isExistingImplantCase) {
      if (!existingOrigProcedure)
        missImplantDetails.push("Type of Implant Procedure Done");
      ...
      } else if (FULL_ARCH_GROUP.has(existingOrigProcedure) && !formData.arch)
        missImplantDetails.push("Arch");
      if ((existingImplantTeeth || []).length === 0)
        missImplantDetails.push("At least one tooth marked on FDI chart");
    } else {
      ...
      if (!formData.procedure_surgery_type)
        ...
        if (!formData.guided_surgery_type)
          missImplantDetails.push("Type of Guided Surgery");
        else if (formData.guided_surgery_type === "Static Guide") {
          if (!formData.static_guide_type)
            missImplantDetails.push("Type of Static Guide");
          else if (!formData.sleeve_type)
            missImplantDetails.push("Type of Sleeve");
        } else if (
          formData.guided_surgery_type === "Dynamic Navigation" &&
          !formData.dynamic_nav_system
        ) {
      ...
      if (formData.implant_procedure_type === "Sinus Lift") {
        if (!formData.sinus_lift_type)
          missImplantDetails.push("Type of Sinus Lift");
        if (!formData.bone_graft_material_details?.trim())
          missImplantDetails.push("Details of Bone Graft Material");
        const invalid = getInvalidSinusLiftTeeth(formData.missing_teeth || []);
        if (invalid.length > 0)
          missImplantDetails.push(
            `Sinus Lift requires maxillary posterior teeth (invalid: ${invalid.join(", ")})`,
          );
      }
    }
    ...
    if (isClusterMode) {
      ...
      for (const run of runs) {
        ...
        if (isCluster) {
          ...
          if (!measurements[leader]?.md) {
            missClinical.push(`Mesiodistal span (FDI ${leader})`);
          }
          for (const t of positions) {
            if (!measurements[t]?.oc)
              missClinical.push(`Occlusocervical height (FDI ${t})`);
          }
          if (ridgeRequired && !perSite[leader]?.ridge_contour)
            missClinical.push(`Ridge contour (FDI ${leader})`);
        } else {
          // Singleton — both oc + md on the tooth; ridge per-site (if required).
          const t = positions[0];
          if (!measurements[t]?.oc)
            missClinical.push(`Occlusocervical height (FDI ${t})`);
          if (!measurements[t]?.md)
            missClinical.push(`Mesiodistal space (FDI ${t})`);
          if (ridgeRequired && !perSite[t]?.ridge_contour)
            missClinical.push(`Ridge contour (FDI ${t})`);
        }
      }
    } else if (!isFullArch && !isOverdentureNonFullArch) {
      if (!formData.occlusocervical_height)
        missClinical.push("Occlusocervical height");
    ...
    if (isExistingImplantCase) {
      ...
      if (!ma.anticoagulant)
        missMedicalOrChecklist.push("Anticoagulant therapy");
      if (!ma.osteoporosis)
        missMedicalOrChecklist.push("Osteoporosis medication");
      if (!ma.radiation) missMedicalOrChecklist.push("Radiation therapy");
    } else {
      if (!cbctFiles[0] || !cbctFiles[1])
        missMedicalOrChecklist.push("Both CBCT Reports");
      if (!intraoralPhotos[0] || !intraoralPhotos[1])
      ...
      if (!ma.smoking)
        missMedicalOrChecklist.push("Smoking status (medical assessment)");
      if (!ma.anticoagulant)
        missMedicalOrChecklist.push(
          "Anticoagulant therapy (medical assessment)",
        );
      if (!ma.osteoporosis)
        missMedicalOrChecklist.push(
          "Osteoporosis medication (medical assessment)",
        );
      if (!ma.radiation)
        missMedicalOrChecklist.push("Radiation therapy (medical assessment)");
  ...
  const flowStepMissing: string[][] = [
    missCaseDetails,
    missImplantDetails,
    missClinical,
    missMedicalOrChecklist,
    [],
  ];
  ...
  flowStepMissing[4] = [
    ...
    ...(missImplantDetails.length
      ? [
          isExistingImplantCase
            ? "Implant Details has missing fields"
            : "Treatment Plan has missing fields",
        ]
      : []),
    ...(missClinical.length ? ["Clinical Examination has missing fields"] : []),
    ...(missMedicalOrChecklist.length
      ? [
          isExistingImplantCase
            ? "Medical Assessment has missing fields"
            : "Phase 1 Checklist has missing fields",
        ]
      : []),
  ];
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#F0F4F8" }}>
      ...
      >
        ...
        {showFlowStrip ? (
          <View
            style={styles.existingProgressBar}
            testID="existing-progress-strip"
          >
            ...
            {(() => {
              ...
              const pct = Math.round(
                (doneCount / FLOW_STEP_LABELS.length) * 100,
              );
              return (
                <>
                  <View
                  ...
                  >
                    <Text
                      style={styles.existingProgressLabel}
                      numberOfLines={1}
                    >
                      Step {currentExistingStep + 1} of{" "}
                      {FLOW_STEP_LABELS.length} —{" "}
                      {FLOW_STEP_LABELS[currentExistingStep]}
                    </Text>
                  ...
                  <PillWrap key={label} {...pillWrapProps}>
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        ...
                        Alert.alert(`Missing: ${label}`, body, [
                      ...
                      style={[
                        styles.existingStepPill,
                        active && styles.existingStepPillActive,
                        done && !active && styles.existingStepPillDone,
                      ]}
                    ...
                    >
                      {done ? (
                        ...
                        <Text
                          style={[
                            styles.existingStepPillNum,
                            active && styles.existingStepPillTextActive,
                          ]}
                        >
                          {idx + 1}.
                        </Text>
                      ...
                      <Text
                        style={[
                          styles.existingStepPillText,
                          active && styles.existingStepPillTextActive,
                          done && !active && styles.existingStepPillTextDone,
                        ]}
                        numberOfLines={1}
                      >
                        {label}
        ...
        <View
          ...
          onLayout={showFlowStrip ? onExistingStepLayout(0) : undefined}
        >
          <Text style={styles.sectionTitle}>Patient Information</Text>
          <View style={styles.fieldContainer}>
            <Text style={styles.label}>
              Patient Name <Text style={{ color: "#DC3545" }}>*</Text>
            </Text>
            <TextInput
              style={styles.input}
              value={formData.patient_name}
              onChangeText={(v) => updateForm("patient_name", v)}
          ...
          {!!nameLookup?.found && !patientLookup?.found && (
            <PatientNameMatchBanner
              key={(nameLookup.patients || [])
                .map((p: any) => p.registration_number)
                .join("|")}
              lookup={nameLookup}
          ...
          <View style={{ flexDirection: "row", gap: 10 }}>
            <View style={[styles.fieldContainer, { flex: 1 }]}>
              <Text style={styles.label}>
                Age (years) <Text style={{ color: "#DC3545" }}>*</Text>
              </Text>
              <TextInput
                style={styles.input}
                value={formData.age}
                onChangeText={(v) =>
                  updateForm("age", v.replace(/[^0-9]/g, ""))
                }
                placeholder="e.g. 45"
            ...
            <View style={[styles.fieldContainer, { flex: 1 }]}>
              <Text style={styles.label}>
                Sex <Text style={{ color: "#DC3545" }}>*</Text>
              </Text>
                  ...
                  <TouchableOpacity
                    ...
                    style={[
                      styles.dropdown,
                      {
                        ...
                        paddingVertical: 10,
                        backgroundColor:
                          formData.sex === opt ? "#1A73E8" : "#FFF",
                      },
                    ]}
                    onPress={() => updateForm("sex", opt)}
                    data-testid={`sex-${opt.toLowerCase()}-btn`}
                  >
                    <Text
                      style={{
                        color: formData.sex === opt ? "#FFF" : "#333",
                        fontWeight: formData.sex === opt ? "700" : "400",
                        fontSize: 13,
                      }}
                    >
                      {opt}
          ...
          <View style={styles.fieldContainer}>
            <Text style={styles.label}>
              Profession <Text style={{ color: "#DC3545" }}>*</Text>
            </Text>
            <TextInput
              style={styles.input}
              value={formData.profession}
              onChangeText={(v) => updateForm("profession", v)}
          ...
          <View style={styles.fieldContainer}>
            <Text style={styles.label}>
              Mobile Number <Text style={{ color: "#DC3545" }}>*</Text>
            </Text>
            <TextInput
              style={styles.input}
              value={formData.mobile_number}
              onChangeText={(v) =>
                updateForm("mobile_number", v.replace(/[^0-9+\-\s]/g, ""))
              }
          ...
          <View style={styles.fieldContainer}>
            ...
            <TextInput
              ...
              onChangeText={(v) => updateForm("patient_email", v)}
          ...
          <View style={styles.fieldContainer}>
            <Text style={styles.label}>
              Registration Number <Text style={{ color: "#DC3545" }}>*</Text>
            </Text>
            <TextInput
              style={styles.input}
        ...
        <View style={styles.section}>
          ...
          <View style={styles.fieldContainer}>
            <Text style={styles.label}>
              Chief Complaint <Text style={{ color: "#DC3545" }}>*</Text>
            </Text>
            <TextInput
              style={[
                styles.input,
            ...
            >
              {
                formData.chief_complaint.trim().split(/\s+/).filter(Boolean)
                  .length
              }
              /100 words
            </Text>
        ...
        {user?.role === "implant_incharge" ? null : (
          <View style={styles.section}>
              ...
              <View style={styles.fieldContainer}>
                ...
                >
                  <Text
                    style={[
                      styles.dropdownText,
                      !formData.supervisor_name && { color: "#999" },
                    ]}
                  >
            ...
            <View style={styles.fieldContainer}>
              <Text style={styles.label}>
                Implant In-Charge <Text style={{ color: "#DC3545" }}>*</Text>
              </Text>
              <TouchableOpacity
                style={styles.dropdown}
                onPress={() => setShowInchargePicker(!showInchargePicker)}
              >
                <Text
                  style={[
                    styles.dropdownText,
                    !formData.implant_incharge_name && { color: "#999" },
                  ]}
                >
                  {formData.implant_incharge_name || "Select Implant In-Charge"}
                </Text>
                <Ionicons
                  name={showInchargePicker ? "chevron-up" : "chevron-down"}
                  size={18}
                  color="#666"
                />
              </TouchableOpacity>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.fieldContainer}>
            <Text style={styles.label}>
              Receipt Number <Text style={{ color: "#DC3545" }}>*</Text>
            </Text>
            <TextInput
              style={styles.input}
              value={formData.receipt_number}
              onChangeText={(v) => updateForm("receipt_number", v)}
              placeholder="Enter receipt number"
            />
          </View>
          <View style={styles.fieldContainer}>
            <Text style={styles.label}>
              Amount Paid <Text style={{ color: "#DC3545" }}>*</Text>
            </Text>
            <TextInput
              style={styles.input}
              value={formData.amount_paid}
              keyboardType="numeric"
              onChangeText={(v) => updateForm("amount_paid", v)}
          ...
          <View onLayout={onExistingStepLayout(1)}>
            <ExistingImplantSection
              ...
              validatePatient={() => {
                ...
                if (!formData.registration_number?.trim())
                  return "MR / Registration number is required.";
                if (!formData.supervisor_id)
                  return "Please select a supervisor.";
                if (!formData.implant_incharge_id)
                  return "Please select an implant in-charge.";
                if (!formData.receipt_number?.trim())
        ...
        {!!augResumeId && (
          <View
            ...
            testID="aug-resume-banner"
            data-testid="aug-resume-banner"
          >
            <View
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                ...
                >
                  Pre-Implant Augmentation Approved
                </Text>
                <Text
                  style={{
                    color: "#33691E",
                ...
                >
                  The bone graft has healed and been signed off. Complete the
                  Phase 1 implant details below to proceed to Phase 2.
                </Text>
              </View>
            </View>
          </View>
        ...
        {!isExistingImplantCase && !augResumeId && (
          <View
            style={styles.section}
            testID="augmentation-question-section"
            data-testid="augmentation-question-section"
          >
            <Text style={styles.sectionTitle}>
              Is Bone Augmentation Required Before Implant Placement?{" "}
              <Text style={{ color: "#DC3545" }}>*</Text>
            </Text>
            <View style={styles.chipRow}>
              {["Yes", "No"].map((o) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.chip,
                    augmentationRequired === o && styles.chipActive,
                  ]}
                  onPress={() => setAugmentationRequired(o as any)}
                  testID={`aug-required-${o.toLowerCase()}`}
                  data-testid={`aug-required-${o.toLowerCase()}`}
                >
                  <Text
                    style={[
                      styles.chipText,
                      augmentationRequired === o && styles.chipTextActive,
                    ]}
                  >
                    {o}
                  </Text>
            ...
            {isAugCase && (
              <View
                style={[
                  styles.riskBadge,
                  { backgroundColor: "#EFEBE9", marginTop: 10 },
                ]}
              >
                <Text style={{ color: "#5D4037", fontSize: 12.5 }}>
                  Bone grafting will be completed and reviewed first. The
                  implant-specific sections (Procedure Type, CBCT, Implant
                  Selection…) are deferred until the graft heals and is approved
                  — then the regular Phase 1 workflow resumes.
                </Text>
              </View>
            )}
          </View>
        ...
        {isAugCase && (
          <>
            <View
              style={styles.section}
              testID="aug-schedule-section"
              data-testid="aug-schedule-section"
            >
              <Text style={styles.sectionTitle}>
                Schedule Pre-Implant Augmentation Surgery
              </Text>
              <CalendarPicker
              ...
              {formData.procedure_date &&
                (() => {
                  ...
                  const availableSlots = PROCEDURE_TIME_SLOTS.filter((sl) =>
                    sl.days.includes(dayName),
                  );
                  return (
                    <View style={styles.fieldContainer}>
                      <Text style={styles.label}>
                        Time Slot <Text style={{ color: "#DC3545" }}>*</Text>
                      </Text>
                      <View style={styles.chipRow}>
                        {availableSlots.map((slot) => {
                          ...
                          const isSelected =
                            formData.procedure_time === slot.value;
                          return (
                            <View key={slot.value}>
                              <TouchableOpacity
                                style={[
                                  ...
                                  isBooked && styles.chipBooked,
                                ]}
                                onPress={() =>
                                  !isBooked &&
                                  updateForm("procedure_time", slot.value)
                                }
                                disabled={isBooked}
                                testID={`aug-slot-${slot.value}`}
                                data-testid={`aug-slot-${slot.value}`}
                              >
                                <Text
                                  style={[
                                    styles.chipText,
                                    isSelected && styles.chipTextActive,
                                    isBooked && styles.chipBookedText,
                                  ]}
                                >
                                  {slot.label}
                                </Text>
            ...
            <View style={{ paddingHorizontal: 16, paddingBottom: 28 }}>
              <TouchableOpacity
                style={[styles.submitBtn, submittingAug && { opacity: 0.6 }]}
                onPress={submitAugmentationCase}
                disabled={submittingAug}
                testID="aug-create-case-btn"
                data-testid="aug-create-case-btn"
              >
                {submittingAug ? (
                  <ActivityIndicator color="#FFF" />
                ) : (
                  <>
                    <Ionicons name="bandage" size={18} color="#FFF" />
                    <Text style={styles.submitBtnText}>
                      Create Case — Pre-Implant Augmentation
                    </Text>
                  </>
                )}
              </TouchableOpacity>
        ...
        {!isExistingImplantCase && !isAugCase && (
            ...
            <View style={styles.section}>
              ...
              {formData.procedure_date &&
                ...
                (() => {
                  ...
                  const dayName = dayNames[dayOfWeek];
                  const availableSlots = PROCEDURE_TIME_SLOTS.filter((s) =>
              ...
              {formData.procedure_date &&
                ...
                (daySlots.length === 0 ? (
                  <View
                    style={[styles.riskBadge, { backgroundColor: "#FFF3E0" }]}
                  >
                    <Text
                      style={{
                        color: "#E65100",
                  ...
                  <View style={styles.fieldContainer}>
                    ...
                    <View style={styles.chipRow}>
                      {daySlots.map((slot) => {
                        ...
                        const isBooked = !!booked;
                        const isSelected =
                          formData.procedure_time === slot.time;
                        return (
                          <View key={slot.time}>
                            <TouchableOpacity
                              style={[
                              ...
                              onPress={() =>
                                !isBooked &&
                                updateForm("procedure_time", slot.time)
                              }
                              disabled={isBooked}
              ...
              {formData.procedure_date &&
                ...
                (() => {
                  ...
                  const handleOpenPicker = () => {
                    setTempHour(
                      openHour ? String(openHour).padStart(2, "0") : "07",
                    );
                  ...
                  return (
                    <View style={styles.fieldContainer}>
                          ...
                          >
                            ...
                            <View style={styles.timeModalInputRow}>
                              ...
                              <View style={{ alignItems: "center" }}>
                                ...
                                <Text style={styles.timeModalSubLabel}>
                                  Hour
                                </Text>
                              </View>
                              ...
                              <View style={styles.timeModalMeridiemContainer}>
                                <TouchableOpacity
                                  style={[
                                    styles.timeModalMeridiemBtn,
                                    tempMeridiem === "AM" &&
                                      styles.timeModalMeridiemBtnActive,
                                    {
                                ...
                                >
                                  <Text
                                    style={[
                                      styles.timeModalMeridiemText,
                                      tempMeridiem === "AM" &&
                                        styles.timeModalMeridiemTextActive,
                                    ]}
                                  >
                                    AM
                                  </Text>
                                  ...
                                  <View style={styles.clockDial}>
                                    ...
                                    {activeInput === "hour"
                                      ? [
                                          ...
                                          const numX =
                                            center +
                                            handLength * Math.cos(theta) -
                                            16;
                                          const numY =
                                            center +
                                            handLength * Math.sin(theta) -
                                            16;
                                          const isSelected =
                                            parseInt(tempHour, 10) === h ||
                                            (h === 12 &&
                                              parseInt(tempHour, 10) === 0);
                                          return (
                                            <TouchableOpacity
                                              key={h}
                                              style={[
                                            ...
                                            >
                                              <Text
                                                style={[
                                                  ...
                                                  isSelected &&
                                                    styles.clockNumberTextSelected,
                                                ]}
                                              >
                                                {h}
                                              </Text>
                            ...
                            <View
                              style={[
                                styles.timeModalFooter,
                                { marginTop: showDialMode ? 24 : 12 },
                              ]}
                            >
                              <TouchableOpacity
                                onPress={() => setShowDialMode((prev) => !prev)}
                                style={{ padding: 4 }}
                              >
                              ...
                              <View style={{ flexDirection: "row", gap: 20 }}>
                                <TouchableOpacity
                                  onPress={() => setShowOpenTimePicker(false)}
                                >
                                  <Text style={styles.timeModalFooterText}>
                                    Cancel
                                  </Text>
                                </TouchableOpacity>
                                <TouchableOpacity
                      ...
                      {endTimeLabel && (
                        ...
                        >
                          Occupies until {endTimeLabel} ({openWindowHours}h
                          slot)
                        </Text>
                      )}
                        ...
                        <View style={{ marginTop: 8 }}>
                          ...
                          {existingBookings.map(([t, info]) => (
                            <Text
                              ...
                              numberOfLines={1}
                            >
                              {formatTimeLabelLocal(t)} — {info.patient_name} (
                              {info.scheduled_by})
                            </Text>
                          ))}
                        </View>
          ...
          block (below), not left in its old spot above Schedule. */}
            <View style={styles.section}>
              ...
              <Dropdown
                ...
                value={formData.implant_procedure_type}
              ...
              {formData.guided_surgery_type === "Static Guide" &&
                isGuidedApproach(formData.procedure_surgery_type) && (
                  <Dropdown
                    ...
                    options={
                      ["All on 4", "All on 6", "All on X"].includes(
                        formData.implant_procedure_type,
                      )
                        ? STATIC_GUIDE_TYPES.filter(
                            (o) => o !== "Tooth Supported Guide",
                          )
                        : STATIC_GUIDE_TYPES
                    }
                    onChange={(v) => {
            ...
            Required. Counter sits beneath the input for clear UX. */}
              {formData.implant_procedure_type === "Sinus Lift" && (
                <View style={styles.fieldContainer}>
                  <Text style={styles.label}>
                    Details of Bone Graft Material{" "}
                    <Text style={{ color: "#DC3545" }}>*</Text>
                  </Text>
                  <TextInput
                    style={[
                    ...
                    onChangeText={(v) =>
                      updateForm("bone_graft_material_details", v.slice(0, 150))
                    }
          ...
          its own FDI chart driven by the implant tooth-positions. */}
                  {formData.implant_procedure_type &&
                    ...
                    (() => {
                      ...
                      const EXTRACT_SET = new Set([
                        "Immediate Implant",
                        "Partial Extraction Therapy",
                      ]);
                      const isExtractFlow = EXTRACT_SET.has(ptype);
                      const sectionTitle = isExtractFlow
                        ? "Select teeth"
                        : "Missing Teeth";
                      ...
                      if (
                        ptype === "Conventional Single Implant" &&
                        missing.length !== 1 &&
                        missing.length > 0
                      ) {
                        countError =
                          "Conventional Single Implant requires exactly 1 tooth.";
                      } else if (
                        ptype === "Multiple Conventional Implants" &&
                        missing.length === 1
                      ) {
                        countError =
                          "Multiple Conventional Implants requires at least 2 teeth.";
                      }
                      return (
                        <View style={styles.section}>
                          <Text style={styles.sectionTitle}>
                          ...
                          <FdiAnatomicalChart
                            ...
                            value={missing}
                            onChange={(next) =>
                              updateForm("missing_teeth", next as string[])
                            }
                            selectedLabel={
                              isExtractFlow
                          ...
                          {missing.length > 0 && (
                            ...
                            >
                              {missing.length}{" "}
                              {missing.length === 1 ? "tooth" : "teeth"} marked
                              — {missing.sort().join(", ")}
                            </Text>
                          )}
                          {countError && (
          ...
          into `formData.missing_teeth` so cluster utilities work identically. */}
            {effectiveProcType && (
              <View
                style={styles.section}
                onLayout={showFlowStrip ? onExistingStepLayout(2) : undefined}
              >
                  ...
                  <>
                    ...
                    <Text
                      style={[
                        ...
                        { fontSize: 14, color: "#1565C0", marginTop: 4 },
                      ]}
                    >
                      Edentulous Site
                    </Text>
                    {(formData.missing_teeth || []).length >= 2 ? (
                      ...
                      (() => {
                        const runs = findMissingRuns(
                          formData.missing_teeth || [],
                        );
                        const setOc = (tooth: string, v: string) => {
                          const next = {
                            ...(formData.edentulous_site_measurements || {}),
                          };
                        ...
                        const setSite = (
                          ...
                          const next = {
                            ...(formData.clinical_exam_per_site || {}),
                          };
                        ...
                        return (
                          <View style={{ marginBottom: 8 }}>
                            ...
                            >
                              Enter the measurements for each tooth marked on
                              the FDI chart. Adjacent missing teeth share one
                              mesiodistal span.
                            </Text>
                            {runs.map((run) => {
                              const archLabel =
                              ...
                              const isCluster = positions.length >= 2;
                              const leader =
                                clusterLeader(positions) || positions[0];
                              const leaderRow =
                                (formData.edentulous_site_measurements || {})[
                                  leader
                                ] || {};
                                      ...
                                      (() => {
                                        ...
                                        return (
                                          ...
                                          >
                                            <Dropdown
                                              label="Ridge Contour"
                                              value={site.ridge_contour || ""}
                                              options={RIDGE_CONTOUR_OPTIONS}
                                              onChange={(v) =>
                                                setSite(
                                                  tooth,
                                                  "ridge_contour",
                                              ...
                                              data-testid={`ridge-contour-${tooth}`}
                                            />
                                            <Dropdown
                                              label="Soft Tissue Thickness"
                                              value={
                                                site.soft_tissue_thickness || ""
                                              }
                                              options={SOFT_TISSUE_OPTIONS}
                                              onChange={(v) =>
                                                setSite(
                                                  tooth,
                                                  "soft_tissue_thickness",
                                              ...
                                              data-testid={`soft-tissue-${tooth}`}
                                            />
                                            <Dropdown
                                              label="Keratinized Mucosa"
                                              value={
                                                site.keratinized_mucosa || ""
                                              }
                                              options={
                                                KERATINIZED_MUCOSA_OPTIONS
                                              }
                              ...
                              return (
                                <View
                                  key={`ed-cluster-${run.arch}-${leader}`}
                                  style={{
                                    backgroundColor: "#FAFAFA",
                                    borderRadius: 10,
                                    padding: 10,
                                ...
                                >
                                  ...
                                  >
                                    ...
                                    >
                                      Adjacent Missing Cluster ({archLabel})
                                    </Text>
                                    {positions.map((t) => (
                                      <View
                                        key={`pill-${t}`}
                                  ...
                                  <View style={{ marginBottom: 10 }}>
                                    <Text
                                      ...
                                      numberOfLines={1}
                                    >
                                      Mesiodistal Space — total cluster span
                                      (mm) *
                                    </Text>
                                    <TextInput
                                      style={[
                                    ...
                                    >
                                      Measure between the two natural teeth
                                      bordering this missing cluster.
                                    </Text>
                                  </View>
                                  <Text
                                    style={{
                                  ...
                                  >
                                    Occlusocervical Height per tooth (mm) *
                                  </Text>
                                  {positions.map((tooth) => {
                                    const row =
                                      (formData.edentulous_site_measurements ||
                                    ...
                                    return (
                                      ...
                                      >
                                        <Dropdown
                                          ...
                                          options={RIDGE_CONTOUR_OPTIONS}
                                          onChange={(v) =>
                                            setSite(leader, "ridge_contour", v)
                                          }
                                          data-testid={`ridge-contour-${leader}`}
                                        />
                                        <Dropdown
                                          label="Soft Tissue Thickness"
                                          value={
                                          ...
                                          data-testid={`soft-tissue-${leader}`}
                                        />
                                        <Dropdown
                                          label="Keratinized Mucosa"
                                          value={site.keratinized_mucosa || ""}
                                          options={KERATINIZED_MUCOSA_OPTIONS}
                                          onChange={(v) =>
                                            setSite(
                                              leader,
                                              "keratinized_mucosa",
                                          ...
                                          data-testid={`keratinized-${leader}`}
                                        />
                                      </View>
                                    );
                                  })()}
                      ...
                      <>
                        <View style={{ marginBottom: 8 }}>
                          ...
                          <TextInput
                            ...
                            onChangeText={(v) =>
                              updateForm("occlusocervical_height", v)
                            }
                        ...
                        <View style={{ marginBottom: 8 }}>
                          ...
                          <TextInput
                            ...
                            onChangeText={(v) =>
                              updateForm("mesiodistal_space", v)
                            }
                  ...
                  per-cluster INSIDE each cluster card above. */}
                    {((formData.missing_teeth || []).length < 2 ||
                      ...
                      <>
                        <Dropdown
                          label="Ridge Contour"
                          value={formData.ridge_contour}
                          options={RIDGE_CONTOUR_OPTIONS}
                          onChange={(v) => updateForm("ridge_contour", v)}
                        />
                        <Dropdown
                          label="Soft Tissue Thickness"
                          value={formData.soft_tissue_thickness}
                          options={SOFT_TISSUE_OPTIONS}
                          onChange={(v) =>
                            updateForm("soft_tissue_thickness", v)
                          }
                        />
                        <Dropdown
                          label="Keratinized Mucosa"
                          value={formData.keratinized_mucosa}
                          options={KERATINIZED_MUCOSA_OPTIONS}
                          onChange={(v) => updateForm("keratinized_mucosa", v)}
                        />
                      </>
                    )}
                  </>
              ...
              Non-Full-Arch + Overdenture-with-Attachment (treated as full-arch) */}
                  ...
                  in Procedure Information, so surface it here. */}
                    {isOverdentureNonFullArch && (
                      <Dropdown
                        ...
                        onChange={(v) => updateForm("arch", v)}
                        required
                        data-testid="overdenture-arch-dropdown"
                      />
                    )}
                    <Dropdown
                      label={
                      ...
                      options={ARCH_CONDITION_OPTIONS}
                      onChange={(v) => updateForm("arch_condition", v)}
                    />
                    <Dropdown
                      label="Ridge Contour"
                      value={formData.ridge_contour}
                    ...
                    <Dropdown
                      ...
                      value={formData.soft_tissue_thickness}
                      options={SOFT_TISSUE_OPTIONS}
                      onChange={(v) => updateForm("soft_tissue_thickness", v)}
                    />
                    <Dropdown
                      label="Keratinized Mucosa"
                      value={formData.keratinized_mucosa}
                ...
                {(formData.implant_procedure_type ===
                  ...
                  <>
                    ...
                    <Dropdown
                      ...
                      onChange={(v) => updateForm("periodontal_status", v)}
                ...
                {isNonFullArch && (
                  <>
                    ...
                    <Dropdown
                      ...
                      value={formData.occlusal_scheme}
                      options={OCCLUSAL_SCHEME_OPTIONS}
                      onChange={(v) => updateForm("occlusal_scheme", v)}
                    />
                    <Dropdown
                      label="Parafunction Habit"
                      value={formData.parafunction_habit}
                      options={PARAFUNCTION_HABIT_OPTIONS}
                      onChange={(v) => updateForm("parafunction_habit", v)}
                    />
                    <Dropdown
                      label="Opposing Dentition"
                      value={formData.opposing_dentition}
                      options={[
                        "Natural Dentition",
                  ...
                  <>
                    ...
                    <View style={styles.fieldContainer}>
                      ...
                      >
                        <Text style={styles.label}>
                          {formData.arch === "Maxillary"
                            ? "Maxillary Restorative Space (mm)"
                            : formData.arch === "Mandibular"
                              ? "Mandibular Restorative Space (mm)"
                              : "Restorative Space (mm)"}
                        </Text>
                        <TouchableOpacity
                          onPress={() =>
                            Alert.alert(
                      ...
                      <TextInput
                        style={styles.input}
                        value={formData.available_interarch_space}
                        keyboardType="decimal-pad"
                        onChangeText={(v) =>
                          updateForm("available_interarch_space", v)
                        }
                        placeholder="Enter in mm"
                        data-testid="restorative-space-input"
                      />
                    </View>
                    <Dropdown
                      label="Opposing Arch"
                      value={formData.opposing_arch}
                      options={[
                        "Natural Dentition",
                        "Fixed Partial Denture",
                        "Fixed Implant Prosthesis",
                ...
                {isNonFullArch && (
                  <>
                    ...
                    <Dropdown
                      label="Smile Line"
                      value={formData.smile_line}
                      options={SMILE_LINE_OPTIONS}
                      onChange={(v) => updateForm("smile_line", v)}
                    />
                    <Dropdown
                      label="Gingival Biotype"
                      value={formData.gingival_biotype}
                      options={GINGIVAL_BIOTYPE_OPTIONS}
                      onChange={(v) => updateForm("gingival_biotype", v)}
                    />
                  </>
                )}
              </View>
            ...
            <View style={styles.section}>
              <Text style={styles.sectionTitle}>
                Type of Loading <Text style={{ color: "#DC3545" }}>*</Text>
              </Text>
              <View style={styles.chipRow}>
                {LOADING_TYPES.map((lt) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.chip,
                      formData.loading_type.includes(lt) && styles.chipActive,
                    ]}
                  ...
                  >
                    <Text
                      style={[
                        styles.chipText,
                        formData.loading_type.includes(lt) &&
          ...
          captured directly on the implant cards instead. */}
            {!isExistingImplantCase && (
              <View style={styles.section}>
                <Text style={styles.sectionTitle}>
                  CBCT Report <Text style={{ color: "#DC3545" }}>*</Text>
                </Text>
                {cbctFiles.map((file, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <View style={{ flex: 1 }}>
                        {file ? (
                          ...
                          <TouchableOpacity
                            ...
                            onPress={() => pickCbctFileAtIndex(idx)}
                            disabled={cbctUploadingIdx === idx}
                      ...
                      {isExtra && (
                        <TouchableOpacity
                          onPress={() => removeExtraCbct(idx)}
                          data-testid={`remove-extra-cbct-${idx}`}
                        >
                          <Ionicons
                            name="remove-circle"
                            size={26}
                ...
                <TouchableOpacity
                  ...
                  onPress={addExtraCbct}
                  data-testid="add-extra-cbct-btn"
                >
                ...
                <Text style={{ fontSize: 11, color: "#999", marginTop: 2 }}>
                  Minimum 2 CBCT Reports required. Accepted: PDF, PNG, JPG, HEIC
                  (Max 25MB each)
                </Text>
              </View>
            )}
          ...
          label. */}
            {!isExistingImplantCase && (
              <View style={styles.section}>
                ...
                {intraoralPhotos.map((file, idx) => {
                  ...
                  return (
                    <View
                      ...
                      data-testid={`intraoral-slot-${idx}`}
                    >
                      {isExtra ? (
                        <TextInput
                          style={{
                      ...
                      >
                        ...
                        <View style={{ flex: 1 }}>
                          {file ? (
                            ...
                            >
                              ...
                              <TouchableOpacity
                                onPress={() => {
                                  const u = [...intraoralPhotos];
                                  u[idx] = null;
                                  setIntraoralPhotos(u);
                                }}
                            ...
                            >
                              {intraoralUploadingIdx === idx ? (
                                ...
                                <>
                                  ...
                                  <Text style={styles.cbctUploadBtnText}>
                                    {idx === 0
                                      ? "Upload Occlusal View Photo"
                                      : idx === 1
                                        ? "Upload Lateral/Frontal View"
                                        : "Upload Photograph"}
                                  </Text>
                                </>
                              )}
                        ...
                        {isExtra && (
                          <TouchableOpacity
                            onPress={() => removeExtraIntraoral(idx)}
                            data-testid={`remove-extra-intraoral-${idx}`}
                          >
                            <Ionicons
                              name="remove-circle"
                              size={26}
                ...
                <Text style={{ fontSize: 11, color: "#999", marginTop: 2 }}>
                  Minimum 2 Photographs required. Accepted: PNG, JPEG, HEIC (Max
                  20 MB each)
                </Text>
              </View>
            )}
          ...
          checklist items because no surgery is performed). */}
            {!isExistingImplantCase && (
              <View
                style={styles.section}
                onLayout={showFlowStrip ? onExistingStepLayout(3) : undefined}
              >
                <Text style={styles.sectionTitle}>
                  Phase 1 Checklist <Text style={{ color: "#DC3545" }}>*</Text>
                </Text>
                {CHECKLIST_DATA.pre_surgical.items
                  .filter((item) => item.id !== "medical_assessment")
                  .filter(
                    (item) => !(isFullArch && item.id === "oral_prophylaxis"),
                  )
                  .map((item) => (
                    <View key={item.id} style={styles.checklistRow}>
                      ...
                      >
                        <Text style={[styles.checklistLabel, { flex: 1 }]}>
                          {item.label}
                        </Text>
                ...
                <View style={styles.medicalSection}>
                  ...
                  {MEDICAL_RISK_FACTORS.map((factor) => (
                    <View key={factor.id}>
                        ...
                        <View style={styles.yesNoRow}>
                          {factor.options.map((opt) => (
                            <TouchableOpacity
                              ...
                              style={[
                                styles.yesNoBtn,
                                formData.medical_assessment[factor.id] ===
                                  opt &&
                                  (opt === "No"
                                    ? styles.noActive
                                    : styles.yesActive),
                      ...
                      {factor.id === "diabetes" &&
                        (formData.medical_assessment.diabetes ===
                          "Controlled" ||
                          formData.medical_assessment.diabetes ===
                            "Uncontrolled") && (
                          <View
                            style={styles.hba1cRow}
                            testID="hba1c-row-routine"
                            data-testid="hba1c-row-routine"
                          >
                            <Text style={styles.hba1cLabel}>
                              HbA1c Value{" "}
                              <Text style={styles.hba1cOptional}>
                                (optional, %)
                              </Text>
                            </Text>
                            <TextInput
                  ...
                  {Object.keys(formData.medical_assessment).length > 0 &&
                    (() => {
                      const risk = calculateMedicalRisk(
                        formData.medical_assessment,
                      );
                      return (
                        <View>
                          ...
                          >
                            ...
                            >
                              Medical Risk: {risk.level} (Score: {risk.score}
                              /15)
                            </Text>
                          </View>
        ...
        {isExistingImplantCase && (
          <View style={styles.section} onLayout={onExistingStepLayout(3)}>
            <Text style={styles.sectionTitle}>
              Medical Assessment <Text style={{ color: "#DC3545" }}>*</Text>
            </Text>
            <View style={styles.medicalSection}>
              {MEDICAL_RISK_FACTORS.map((factor) => (
                <View key={factor.id}>
                    ...
                    <View style={styles.yesNoRow}>
                      {factor.options.map((opt) => (
                        <TouchableOpacity
                          ...
                          style={[
                            styles.yesNoBtn,
                            formData.medical_assessment[factor.id] === opt &&
                              (opt === "No"
                                ? styles.noActive
                                : styles.yesActive),
                          ]}
                  ...
                  {factor.id === "diabetes" &&
                    (formData.medical_assessment.diabetes === "Controlled" ||
                      formData.medical_assessment.diabetes ===
                        "Uncontrolled") && (
                      <View
                        style={styles.hba1cRow}
                        testID="hba1c-row-existing"
                        data-testid="hba1c-row-existing"
                      >
                        <Text style={styles.hba1cLabel}>
                          HbA1c Value{" "}
                          <Text style={styles.hba1cOptional}>
        ...
        {isExistingImplantCase &&
          ...
          (() => {
            ...
            return (
              <View
                style={[styles.section, { gap: 6 }]}
                testID="existing-impl-action-buttons"
                onLayout={onExistingStepLayout(4)}
              >
                <TouchableOpacity
                  style={[
                    styles.continueBtn,
                    {
                      backgroundColor: canSubmit ? "#43A047" : "#B0BEC5",
                      marginVertical: 0,
                      marginHorizontal: 0,
                    },
                    existingSubmitApi.submitting && { opacity: 0.6 },
                  ]}
                  onPress={() =>
                    guard(() => existingSubmitApi.submit("phase3"))
                  }
                  disabled={existingSubmitApi.submitting}
                  data-testid="ei-move-phase3-bottom"
                >
                  <Ionicons
                    name={canSubmit ? "arrow-forward-circle" : "lock-closed"}
                    size={20}
                    color="#FFF"
                  />
                  <Text style={styles.continueBtnText} numberOfLines={2}>
                    {existingSubmitApi.labels.phase3}
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[
                    styles.continueBtn,
                    {
                      backgroundColor: canSubmit ? "#1565C0" : "#B0BEC5",
                      marginVertical: 0,
                      marginHorizontal: 0,
                    },
                    existingSubmitApi.submitting && { opacity: 0.6 },
                  ...
                  onPress={() =>
                    guard(() => existingSubmitApi.submit("phase4_step1"))
                  }
                  disabled={existingSubmitApi.submitting}
                ...
                >
                  {existingSubmitApi.submitting ? (
                    ...
                    <>
                      <Ionicons
                        name={
                          canSubmit ? "arrow-forward-circle" : "lock-closed"
                        }
                        size={20}
                        color="#FFF"
                      />
                      <Text style={styles.continueBtnText} numberOfLines={2}>
                        {existingSubmitApi.labels.phase4}
                      </Text>
                    </>
                  )}
                </TouchableOpacity>
                {!canSubmit && !existingSubmitApi.submitting && (
                  ...
                  >
                    {incompleteCount} section{incompleteCount > 1 ? "s" : ""}{" "}
                    still incomplete — press Continue to see the full list
                  </Text>
                )}
                {!existingSubmitApi.isDraftResume && (
                  ...
                  >
                    ...
                    <Text
                      style={[styles.continueBtnText, { color: "#37474F" }]}
                    >
                      Save Draft
                    </Text>
                  </TouchableOpacity>
        ...
        {!isExistingImplantCase && !isAugCase && (
            ...
            {formData.implant_procedure_type.includes("Bone") && (
              <View style={styles.section}>
                ...
                <TextInput
                  ...
                  onChangeText={(v) =>
                    updateForm("bone_graft_specifications", v)
                  }
                  placeholder="Enter bone graft details"
                  multiline
                />
              </View>
            )}
          ...
          the holistic Alert listing what's missing. */}
            {!isExistingImplantCase &&
              (() => {
                const canContinue = [0, 1, 2, 3].every(
                  (i) => existingStepDone[i],
                );
                const incompleteCount = [0, 1, 2, 3].filter(
                  (i) => !existingStepDone[i],
                ...
                return (
                  <View
                    onLayout={
                      showFlowStrip ? onExistingStepLayout(4) : undefined
                    }
                  >
                    <TouchableOpacity
                      style={[
                        styles.continueBtn,
                        !canContinue && { backgroundColor: "#B0BEC5" },
                      ]}
                    ...
                    {!canContinue && !loading && (
                      ...
                      >
                        {incompleteCount} section
                        {incompleteCount > 1 ? "s" : ""} still incomplete —
                        press Continue to see the full list
                      </Text>
                    )}
                  </View>
                );
