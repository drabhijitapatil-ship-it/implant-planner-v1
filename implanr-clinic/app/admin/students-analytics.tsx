...
export default function StudentsAnalyticsScreen() {
  const { user } = useAuth();
  ...
  return (
    <SafeAreaView style={s.safe}>
      <View style={s.header}>
        <BackButton />
        <Text style={s.title}>Student Analytics</Text>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity
            style={s.card}
            activeOpacity={0.85}
            onPress={() => router.push({ pathname: '/admin/student/[id]', params: { id: item.id } })}
          >
            <View style={s.cardTop}>
              ...
              {item.kpis.pending_approval > 0 && (
                <View style={s.badge}>
                  <Text style={s.badgeText}>{item.kpis.pending_approval} pending</Text>
                </View>
              )}
            ...
            <View style={s.kpiRow}>
              <KPIBox label="Total" value={item.kpis.total} color="#555" />
              <KPIBox label="Active" value={item.kpis.active} color="#1565C0" />
...
const s = StyleSheet.create({
  ...
  avatarPlaceholder: { backgroundColor: '#1565C0', justifyContent: 'center', alignItems: 'center' },
  ...
  kpiRow: { flexDirection: 'row', borderTopWidth: 1, borderTopColor: '#f0f0f0', paddingTop: 10 },
