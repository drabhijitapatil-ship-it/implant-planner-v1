...
function DefaultProceduresScreen() {
  ...
  const getMenuActions = (item: any) => {
    ...
    const canShare = role === 'chief_dentist'
      || (role === 'dentist' && item.supervisor_id === user?.id)
      || (role === 'dentist' && (item.student_id === user?.id || item.created_by_id === user?.id));
    if (canShare) {
      actions.push({
        key: 'forum',
    ...
    const isFaculty = role === 'dentist' || role === 'chief_dentist' || role === 'administrator';
