...
const ROLE_META = [
  { value: 'chief_dentist', label: 'Chief Dentist / Owner', subRoles: [] },
  { value: 'dentist', label: 'Dentist / Consultant', subRoles: [] },
  {
    value: 'dental_assistant', label: 'Auxiliary Staff', subRoles: [
...
const ROLE_COLOR: Record<string, string> = {
  chief_dentist: '#E65100',
  dentist: '#1B5E20',
  dental_assistant: '#880E4F',
  administrator: '#4A148C',
};
...
export default function UserManagementScreen() {
  const { user } = useAuth();
  const isAdmin = user?.role === 'chief_dentist' || user?.role === 'administrator';
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#F4FAF5' }}>
      {/* Header */}
      <LinearGradient colors={['#2E7D32', '#388E3C']} style={s.header}>
          ...
          <View style={s.pickerSheet}>
            ...
            {ROLE_META.map(r => (
              ...
              >
                ...
                {inviteForm.role === r.value && <Ionicons name="checkmark" size={16} color="#2E7D32" />}
              </TouchableOpacity>
            ))}
          </View>
        </Pressable>
          ...
          <View style={s.pickerSheet}>
            ...
            {(selectedRoleMeta?.subRoles ?? []).map((r: any) => (
              ...
              >
                ...
                {inviteForm.sub_role === r.value && <Ionicons name="checkmark" size={16} color="#2E7D32" />}
              </TouchableOpacity>
            ))}
          </View>
        </Pressable>
...
function PendingCard({ invite, onRevoke, onResend }: {
  ...
  return (
    <View style={cs.card}>
      <View style={[cs.avatar, { backgroundColor: '#FFF8E1' }]}>
        <Ionicons name="mail-outline" size={20} color="#F57F17" />
      </View>
      <View style={{ flex: 1 }}>
...
const s = StyleSheet.create({
  loadingContainer: { flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#F4FAF5' },
  ...
  tabTxtActive: { color: '#2E7D32', fontWeight: '700' },
  ...
  sendBtn: { backgroundColor: '#2E7D32', borderRadius: 12, paddingVertical: 15, alignItems: 'center', marginTop: 24, flexDirection: 'row', justifyContent: 'center', gap: 8 },
  ...
  pickerSheet: { backgroundColor: '#FFF', borderRadius: 16, padding: 8, minWidth: 260, maxWidth: 340 },
  ...
  pickerItemSel: { backgroundColor: '#E8F5E9' },
  pickerItemTxt: { fontSize: 15, color: '#37474F' },
  pickerItemTxtSel: { color: '#2E7D32', fontWeight: '700' },
});
...
const cs = StyleSheet.create({
  ...
  resendBtn: { borderRadius: 8, paddingHorizontal: 10, paddingVertical: 4, backgroundColor: '#E8F5E9' },
  resendTxt: { fontSize: 12, color: '#2E7D32', fontWeight: '600' },
