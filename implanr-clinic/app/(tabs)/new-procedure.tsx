...
export default function NewProcedureScreen() {
        ...
        const loadDraft = async () => {
          try {
            ...
            if (proc.status === 'draft') {
              ...
              // iter-296b: restore CBCT file slots from the backend
  ...
  useEffect(() => {
    const loadFaculty = async () => {
      try {
        ...
        setSupervisors(users.filter((u: any) => u.role === 'dentist' || u.role === 'chief_dentist'));
        setIncharges(users.filter((u: any) => u.role === 'chief_dentist'));
      } catch (e) { /* ignore */ }
    };
    loadFaculty();
  }, []);
  ...
  // iter-242 / iter-296-fix: MOVED before step-2 early return to avoid
        ...
        <View style={styles.submitContainer}>
          ...
          <TouchableOpacity style={styles.submitBtn} data-testid="submit-for-approval"
            onPress={async () => {
              ...
              const isInchargeUser = user?.role === 'chief_dentist';
            ...
            <Text style={styles.submitBtnText}>{user?.role === 'chief_dentist' ? 'Done' : 'Submit for Approval'}</Text>
          </TouchableOpacity>
          )}
        </View>
      </View>
