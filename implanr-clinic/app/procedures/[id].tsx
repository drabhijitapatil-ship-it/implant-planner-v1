...
export default function ProcedureDetailScreen() {
  ...
  // Edit mode state
  const isEditMode = edit === 'true' && (user?.role === 'chief_dentist' || user?.role === 'dentist');
  ...
  const generateAiSurgicalSummary = async () => {
    ...
    try {
      ...
      Alert.alert('Error', e?.response?.data?.detail || 'Failed to generate surgical summary');
    } finally {
      setAiSurgicalLoading(false);
    }
  };

  const saveAiSummary = async (summaryType: 'case_summary' | 'surgical_notes', content: string) => {
    const setSaving = summaryType === 'case_summary' ? setAiSavingClinical : setAiSavingSurgical;
    setSaving(true);
    try {
      await api.patch(`/procedures/${id}/ai-summary`, { summary_type: summaryType, content });
      await loadProcedure();
      ...
      Alert.alert('Error', e?.response?.data?.detail || 'Failed to save summary');
    } finally {
      setSaving(false);
    }
  };
  ...
  const canApprove = () => {
    ...
    if (user?.role === 'dental_assistant' || user?.role === 'dentist') return false;
    ...
    const isInchargeSelfCreated = procedure.created_by_role === 'chief_dentist' && user?.id === procedure.created_by_id;
  ...
  return (
    ...
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      >
        ...
        {procedure.status !== 'completed' && (
          <AugmentationChecklist
            ...
            canSignOff={['dentist', 'chief_dentist', 'administrator'].includes(user?.role || '')}
          />
        )}
        {/* Edit Mode Banner */}
        {isEditMode && (
          ...
          <>
              ...
              <View style={styles.completedFactsGrid} testID="completed-facts">
                {(() => {
                  const operator = procedure.student_name
                    || (procedure.created_by_role !== 'dentist' ? procedure.created_by_name : null);
                  const operatorLabel = procedure.student_name
                    ? 'Dentist'
                    : procedure.created_by_role === 'dentist'
                      ? 'Dentist'
                      : procedure.created_by_role === 'chief_dentist'
                        ? 'Chief Dentist'
                        : 'Operator';
            ...
            <View style={{ marginHorizontal: 16, marginTop: 12, marginBottom: 16, backgroundColor: '#FFFFFF', borderRadius: 14, borderWidth: 1.5, borderColor: '#43A047', padding: 16 }} testID="digital-signoff-card">
              ...
              {(() => {
                ...
                const rows = [
                  { role: 'Dentist', name: procedure.supervisor_name, at: procedure.supervisor_final_delivery_approved_at, note: procedure.phase4_step2_supervisor_notes },
                  { role: 'Chief Dentist', name: procedure.implant_incharge_name, at: procedure.implant_incharge_final_delivery_approved_at, note: procedure.phase4_step2_incharge_notes },
                ];
                return rows.map((r, i) => (
        ...
        {(() => {
          ...
          const isReviewer = user?.role === 'dentist'
        ...
        {(() => {
          ...
          const canResolve = user?.role === 'chief_dentist' || user?.role === 'administrator'
            || (user?.role === 'dentist' && procedure?.supervisor_id === user?.id);
        ...
        {(() => {
          const isOwner = !!(procedure.student_id && user?.id === procedure.student_id)
            || !!(procedure.created_by_id && user?.id === procedure.created_by_id);
          const canUpload = user?.role === 'dental_assistant' || isOwner;
          const canViewOnly = !canUpload
            && (user?.role === 'dentist' || user?.role === 'chief_dentist' || user?.role === 'administrator');
          ...
          return (
            <View style={styles.consentActionRow} testID="consent-action-row">
              ...
              {user?.role === 'chief_dentist' && !canUpload && (
                <TouchableOpacity
          ...
          <View style={styles.phase2ButtonContainer}>
            {procedure.patient_consent_form ? (
              <>
                ...
                {user?.role !== 'dental_assistant' && procedure.instruments_autoclaved?.marked && (
          ...
          {user?.role === 'dental_assistant' ? (
            ...
            <>
              {procedure.student_name ? (
                <InfoRow icon="school" label="Dentist" value={procedure.student_name} />
              ) : procedure.created_by_name && procedure.created_by_role !== 'dentist' ? (
                <InfoRow icon="person" label={procedure.created_by_role === 'chief_dentist' ? 'Operator (Chief Dentist)' : 'Operator'} value={procedure.created_by_name} />
              ) : null}
              <InfoRow icon="school" label="Dentist" value={procedure.supervisor_name} />
              <InfoRow icon="medkit" label="Chief Dentist" value={procedure.implant_incharge_name} />
            </>
          ) : procedure.created_by_role === 'chief_dentist' ? (
            // Chief Dentist scheduled the case — show only Chief Dentist.
            <InfoRow
              icon="medkit"
              label="Chief Dentist"
              value={procedure.implant_incharge_name || procedure.created_by_name}
            />
          ) : procedure.created_by_role === 'dentist' ? (
            // Dentist scheduled the case — show Dentist + Chief Dentist only.
            <>
              <InfoRow
                icon="school"
                label="Dentist"
            ...
            // Dentist-scheduled (default) — show Dentist + Chief Dentist.
            <>
              {procedure.student_name ? (
                <InfoRow icon="school" label="Dentist" value={procedure.student_name} />
              ) : null}
        ...
        <View style={styles.section}>
          <View style={resStyles.sectionHeaderRow}>
            ...
            {(() => {
              ...
              const isFaculty = user?.role === 'dentist' || user?.role === 'chief_dentist' || user?.role === 'administrator';
            ...
            {(['maxilla', 'mandible'] as const).map((arch) => {
              ...
              return (
                <View key={arch} style={{ marginBottom: 14, padding: 12, backgroundColor: '#FAFCFF', borderRadius: 10 }} data-testid={`atrophy-detail-${arch}`}>
                  <Text style={{ fontSize: 13, fontWeight: '700', color: '#0D47A1', textTransform: 'capitalize', marginBottom: 8 }}>{arch}</Text>
        ...
        {user?.role !== 'dental_assistant' && procedure.phase2_data && Object.keys(procedure.phase2_data).length > 0 && (
          <View
            ...
            {(procedure.phase2_student_notes || procedure.phase2_remark) && (
              <View style={{ marginBottom: 8, backgroundColor: '#F5F9FF', borderRadius: 8, padding: 12 }}>
                <Text style={{ fontSize: 14, fontWeight: '700', color: '#1565C0', marginBottom: 8 }}>
                  {procedure.created_by_role === 'dentist' ? "Student's Notes" : "Operator's Notes"}
                </Text>
        ...
        {(() => {
          const phase2Approved = ['phase2_approved','pending_stage2_surgical','stage2_surgical_approved','pending_stage2_prosthetic','stage2_prosthetic_step1_approved','pending_final_delivery','completed'].includes(procedure.status);
          ...
          return (
            <View style={[styles.section, { borderLeftWidth: 4, borderLeftColor: '#3F51B5' }]} testID="ai-surgical-summary-section">
              <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                ...
                {hasNotes && !editingAiSurgical && (
                  <View style={{ flexDirection: 'row', gap: 8 }}>
                    <TouchableOpacity
                      onPress={() => { setAiSurgicalDraft(procedure.ai_surgical_notes || ''); setEditingAiSurgical(true); }}
                      style={{ padding: 6, borderRadius: 8, backgroundColor: '#E8EAF6' }}
                    ...
                    <TouchableOpacity
                      onPress={() => Alert.alert(
                        ...
                        [
                          { text: 'Cancel', style: 'cancel' },
                          { text: 'Regenerate', style: 'destructive', onPress: generateAiSurgicalSummary },
                        ],
                      )}
              ...
              {hasNotes && editingAiSurgical && (
                <View>
                  <TextInput
                    style={{ borderWidth: 1.5, borderColor: '#9FA8DA', borderRadius: 10, padding: 12, fontSize: 13, backgroundColor: '#FFF', minHeight: 160, textAlignVertical: 'top', color: '#37474F', lineHeight: 20 }}
        ...
        {user?.role !== 'dental_assistant' && (procedure.phase3_data || procedure.stage2_surgical_remark || procedure.phase3_student_notes) && (
          <View
        ...
        {user?.role !== 'dental_assistant' && procedure.phase4_step1_data && (
        ...
        {user?.role !== 'dental_assistant' && (procedure.phase4_step2_data || procedure.phase4_step2_student_notes) && (
        ...
        {editingAiClinical && (
          <View style={{ marginHorizontal: 16, marginBottom: 16, backgroundColor: '#E8EAF6', borderRadius: 14, padding: 16, borderLeftWidth: 4, borderLeftColor: '#3F51B5' }} testID="ai-summary-edit-card">
            ...
            <TextInput
              style={{ borderWidth: 1.5, borderColor: '#9FA8DA', borderRadius: 10, padding: 12, fontSize: 13, backgroundColor: '#FFF', minHeight: 200, textAlignVertical: 'top', color: '#37474F', lineHeight: 20 }}
            ...
            <ScrollView contentContainerStyle={{ padding: 16 }}>
              {(procedure.edit_log?.length || 0) === 0 ? (
                ...
                [...procedure.edit_log].reverse().map((entry: any, idx: number) => {
                  ...
                  const roleLabel = entry.edited_by_role === 'chief_dentist' ? 'Chief Dentist'
                    : entry.edited_by_role === 'dental_assistant' ? 'Assistant'
                    : entry.edited_by_role === 'dentist' ? 'Dentist' : '';
                  return (
            ...
            {canViewAiSummary() && (
              <TouchableOpacity
                ...
                onPress={async () => {
                  ...
                  try {
                    await api.post('/ai/case-summary', { procedure_id: procedure.id || procedure._id });
                    await loadProcedure();
                  } catch (e: any) {
              ...
              >
                {aiSummaryLoading ? (
                  ...
                  <>
                    ...
                    <Text style={styles.barButtonTextCompact}>{procedure.ai_case_summary ? 'REGEN AI' : 'AI SUMMARY'}</Text>
                  </>
                )}
              </TouchableOpacity>
            )}
      ...
      {procedure.status !== 'draft' && user?.role !== 'dental_assistant' && (
        <View
