        ...
        <View style={{ flex: 1, backgroundColor: '#FFF', paddingTop: Math.max(insets.top, Platform.OS === 'ios' ? 44 : 16) }}>
          ...
          <View style={s.chipRow}>
            {(() => {
              ...
              if (c.supervisor) chips.push({ key: 'sup', label: 'All Supervisors', role: 'dentist', count: c.supervisor });
              if (c.student) chips.push({ key: 'stu', label: 'All Students', role: 'dentist', count: c.student });
              if (c.implant_incharge) chips.push({ key: 'inc', label: 'All In-Charges', role: 'chief_dentist', count: c.implant_incharge });
              return (
                <>
                  {chips.map(ch => (
                    <TouchableOpacity
