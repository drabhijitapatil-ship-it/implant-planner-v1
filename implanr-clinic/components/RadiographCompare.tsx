...
export default function RadiographCompare({ procedure, iopaUploads, opgUpload }: Props) {
  ...
  const canEditNotes = useMemo(() => {
    ...
    if (role === 'administrator' || role === 'chief_dentist') return true;
    if (role === 'dentist' && procedure.supervisor_id === user.id) return true;
    if (role === 'dentist' && procedure.student_id === user.id) return true;
    return false;
  }, [user, procedure]);
  ...
  const baselineByTooth = useMemo(() => {
    ...
    if (isExisting) {
      ...
      // iter-306: Phase 2 stores tooth_label as "Tooth #31" but Phase 4
      // keys by raw position "31". Strip prefix and index both forms.
  ...
  // baseline is itself important feedback. Only bail when no teeth to compare.
