...
function getProgressTrail(
  ...
  if (status === "completed") {
    return isExisting
      ...
      : {
          phases: ["Phase 1", "Phase 2", "Phase 3", "Phase 4", "Done"],
          currentIndex: 4,
        };
  }
  return { phases: ["Phase 1"], currentIndex: 0 };
...
const FIELD_OPTIONS: Record<string, FieldOptionsConfig> = {
  ...
  "medical_assessment.diabetes": {
    options: ["No", "Controlled", "Uncontrolled"],
  },
  "medical_assessment.smoking": {
    options: ["No", "Light (<10/day)", "Heavy (>10/day)"],
  },
  ...
  "phase2_data.implant_seated_correctly": {
    options: ["Yes", "No"],
    bool: true,
  },
  ...
  "phase4_step1_data.overdenture_attachment": {
    options: OVERDENTURE_ATTACHMENT_OPTIONS,
  },
  ...
  "phase4_step1_data.components_available": {
    options: ["Yes", "No"],
    bool: true,
  },
};
...
function resolveFieldOptions(
  ...
  if (fieldKey === "prosthetic_plan" && procedure?.implant_procedure_type) {
    const opts = getProstheticOptions(
      procedure.implant_procedure_type,
      procedure.loading_type || [],
...
export default function ProcedureDetailScreen() {
  ...
  // ── AI Summary editing state (Clinical + Surgical) ─────────────────
  const [editingAiClinical, setEditingAiClinical] = useState(false);
  const [editingAiSurgical, setEditingAiSurgical] = useState(false);
  const [aiClinicalDraft, setAiClinicalDraft] = useState("");
  const [aiSurgicalDraft, setAiSurgicalDraft] = useState("");
  const [aiSavingClinical, setAiSavingClinical] = useState(false);
  ...
  const isEditMode =
    edit === "true" &&
    (user?.role === "implant_incharge" || user?.role === "supervisor");
  ...
  const uploadConsentForProcedure = async () => {
    try {
      ...
      Alert.alert(
        "Uploaded",
        "Patient consent form uploaded. Phase 2 is now unlocked.",
      );
    } catch (err: any) {
      Alert.alert(
        "Upload Failed",
  ...
  useEffect(() => {
    ...
    const tappable =
      ["Phase 1", "Phase 2", "Phase 3", "Phase 4"].includes(pair.current) &&
      !!pair.next;
    if (!tappable) return;
    let cancelled = false;
    AsyncStorage.getItem(PILL_HINT_KEY)
      .then((v) => {
    ...
    try {
      ...
      Alert.alert(
        ...
        error?.response?.data?.detail ||
          "Failed to generate Smart Planner report",
      );
    } finally {
  ...
  const generateAiSurgicalSummary = async () => {
    setAiSurgicalLoading(true);
    try {
      await api.post("/ai/surgical-notes", { procedure_id: id });
      await loadProcedure();
    } catch (e: any) {
      Alert.alert(
        "Error",
        e?.response?.data?.detail || "Failed to generate surgical summary",
      );
    } finally {
      setAiSurgicalLoading(false);
    }
  };

  const saveAiSummary = async (
    summaryType: "case_summary" | "surgical_notes",
    content: string,
  ) => {
    const setSaving =
    ...
    setSaving(true);
    try {
      await api.patch(`/procedures/${id}/ai-summary`, {
        summary_type: summaryType,
        content,
      });
      await loadProcedure();
      if (summaryType === "case_summary") {
        setEditingAiClinical(false);
        setAiSummary("");
      } else {
        setEditingAiSurgical(false);
      }
    } catch (e: any) {
      Alert.alert(
        "Error",
        e?.response?.data?.detail || "Failed to save summary",
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Error",
        error.response?.data?.detail || "Failed to reject procedure",
      );
    } finally {
      setActionLoading(false);
    }
  ...
  const getApproveEndpoint = () => {
    if (procedure?.status === "pending_stage2_surgical")
      return `/procedures/${id}/stage2/surgical/approve`;
    if (procedure?.status === "pending_stage2_prosthetic")
      return `/procedures/${id}/stage2/prosthetic/approve`;
    if (procedure?.status === "pending_final_delivery")
      return `/procedures/${id}/stage2/prosthetic/step2/approve`;
  ...
  const canEdit = () => {
    ...
    if (user?.role === "supervisor" && user?.id === procedure.supervisor_id)
      return true;
    if (
      user?.role === "student" &&
  ...
  const renderChecklistSection = (sectionKey: string, sectionTitle: string) => {
    ...
    const checklistDef =
      CHECKLIST_DATA[sectionKey as keyof typeof CHECKLIST_DATA];
    const isFullArch = ['All on 4', 'All on 6', 'All on X'].includes(procedure.implant_procedure_type || '');

    return (
      <View key={sectionKey} style={styles.section}>
        <Text style={styles.sectionTitle}>{sectionTitle}</Text>
        {sectionData.items.filter((item: any) => !(isFullArch && item.id === 'oral_prophylaxis')).map((item: any) => {
          const itemDef = checklistDef?.items.find(
            (i: any) => i.id === item.id,
          );
          return (
            <View key={item.id} style={styles.checklistItem}>
              ...
              <Text style={styles.checklistLabel}>
                {itemDef?.label || item.label || item.id}
              </Text>
            </View>
          );
        })}

        {sectionData.additional_fields &&
          Object.keys(sectionData.additional_fields).length > 0 && (
            <View style={styles.additionalFields}>
              {Object.entries(sectionData.additional_fields).map(
                ([key, value]) => (
      ...
      <SafeAreaView style={styles.container} edges={["top", "bottom"]}>
        ...
        >
          ...
          {isEditMode && (
            ...
            >
              ...
              >
                Edit Mode — Tap pencil icon to edit
              </Text>
              <TouchableOpacity
                style={{
                            ...
                            >
                              <Text
                                style={[
                                  ...
                                  isCompleted &&
                                    styles.progressChipTextCompleted,
                                ]}
                              >
                                {label}
                              </Text>
            ...
            <>
              <View style={styles.completedBanner}>
                ...
                <Text style={styles.completedSubtext}>
                  All protocols have been approved successfully
                </Text>
                ...
                {procedure.case_id && (
                  <Text
                    style={styles.completedCaseId}
                    testID="completed-case-id"
                  >
                    {procedure.case_id}
                  </Text>
                )}
                <View
                  style={styles.completedFactsGrid}
                  testID="completed-facts"
                >
                  {(() => {
                    const operator =
                    ...
                    const completedOn = procedure.completed_at
                      ? new Date(procedure.completed_at).toLocaleDateString(
                          "en-IN",
                    ...
                    const facts: {
                      ...
                      },
                      {
                        ...
                        label: "Implants",
                        value:
                          procedure.number_of_implants != null
                            ? String(procedure.number_of_implants)
                            : null,
                      },
                      {
                        icon: "calendar",
                    ...
                    return facts
                      ...
                      .map((f, i) => (
                        <View key={i} style={styles.completedFactRow}>
                          ...
                          <Text style={styles.completedFactLabel}>
                            {f.label}:
                          </Text>
                          <Text
                            style={styles.completedFactValue}
                            numberOfLines={1}
                          >
                            {f.value}
                          </Text>
              ...
              >
                ...
                {(() => {
                  ...
                  const rows = [
                    {
                      role: "Supervising Faculty",
                      name: procedure.supervisor_name,
                      at: procedure.supervisor_final_delivery_approved_at,
                      note: procedure.phase4_step2_supervisor_notes,
                    },
                    {
                      role: "Implant In-Charge",
                      name: procedure.implant_incharge_name,
                      at: procedure.implant_incharge_final_delivery_approved_at,
                      note: procedure.phase4_step2_incharge_notes,
                    },
                  ];
                  return rows.map((r, i) => (
                    <View
                    ...
                    >
                      ...
                      <Text
                        style={{ fontSize: 12, color: "#558B2F", marginTop: 2 }}
                      >
                        {fmt(r.at)}
                      </Text>
                      {r.note ? (
                ...
                >
                  Auto-stamped on case completion. Subsequent edits are recorded
                  in the case audit log.
                </Text>
              </View>
            </>
          )}
          ...
          {procedure.case_origin === "existing_implants" && (
            <View
              ...
              testID="existing-implants-block"
            >
              ...
              <View style={{ alignItems: "center", marginBottom: 12 }}>
                ...
                >
                  ...
                  >
                    Existing Implants · Prosthesis-Only
                  </Text>
                </View>
              </View>

              {Array.isArray(procedure.existing_implants) &&
              procedure.existing_implants.length > 0 ? (
                procedure.existing_implants.map((row: any, idx: number) => (
                  <View
                    ...
                    testID={`existing-implant-row-${idx}`}
                  >
                    <View
                      style={{
                        flexDirection: "row",
                    ...
                    {!row.system_unknown && (
                      ...
                      >
                        {row.connection_type && (
                          <Text style={{ fontSize: 12, color: "#37474F" }}>
                            Conn:{" "}
                        ...
                        {row.gingival_height_mm != null && (
                          <Text style={{ fontSize: 12, color: "#37474F" }}>
                            GH:{" "}
                            <Text style={{ fontWeight: "700" }}>
                              {row.gingival_height_mm} mm
                            </Text>
                          </Text>
                        )}
                      </View>
                    ...
                    {(row.surgery_date ||
                      row.original_surgeon ||
                      row.abutment_present !== null) && (
                      <Text
                        style={{
                          fontSize: 11,
                          color: "#607D8B",
                      ...
                      >
                        ...
                        {row.surgery_date && row.original_surgeon
                          ...
                          : ""}
                        {row.original_surgeon
                          ? `Surgeon: ${row.original_surgeon}`
                          : ""}
                        {(row.surgery_date || row.original_surgeon) &&
                        row.abutment_present !== null
                          ? "  •  "
                          : ""}
                        {row.abutment_present === true
                          ? "Abutment in place"
                          : row.abutment_present === false
                            ? "No abutment"
                            : ""}
                      </Text>
                    )}
                    ...
                    {(() => {
                      const perImplant =
                        row.iopa_url ||
                        (procedure.radiographs?.iopas || [])[idx] ||
                        "";
                      if (!perImplant) return null;
                      return (
                        <RadiographThumb
                          filename={String(perImplant)}
                          testID={`existing-impl-iopa-${idx}`}
                          label={`IOPA · Tooth ${row.tooth || "—"}`}
                        />
                      );
                    })()}
                  </View>
              ...
              {(procedure.radiographs?.opg_url || procedure.cbct_file) && (
                <View
                  style={{ marginTop: 4 }}
                  testID="existing-implants-opg-block"
                >
                  <RadiographThumb
                    filename={String(
                      procedure.radiographs?.opg_url || procedure.cbct_file,
              ...
              {procedure.prosthesis_history &&
                procedure.prosthesis_history.had_prosthesis && (
                  <View
                    style={{
                      marginTop: 8,
                      paddingTop: 12,
                    ...
                    testID="prosthesis-history-block"
                  >
                    <Text
                      style={{
                        fontSize: 13,
                    ...
                    >
                      {procedure.prosthesis_history.prosthesis_type && (
                        <Text style={{ fontSize: 12, color: "#37474F" }}>
                          Type:{" "}
                          <Text style={{ fontWeight: "700" }}>
                            {procedure.prosthesis_history.prosthesis_type}
                      ...
                      {procedure.prosthesis_history.material && (
                        <Text style={{ fontSize: 12, color: "#37474F" }}>
                          Material:{" "}
                          <Text style={{ fontWeight: "700" }}>
                            {procedure.prosthesis_history.material}
                      ...
                      {procedure.prosthesis_history.placement_date && (
                        <Text style={{ fontSize: 12, color: "#37474F" }}>
                          Placed:{" "}
                          <Text style={{ fontWeight: "700" }}>
                            {procedure.prosthesis_history.placement_date}
                      ...
                      {procedure.prosthesis_history.lab_name && (
                        <Text style={{ fontSize: 12, color: "#37474F" }}>
                          Lab:{" "}
                          <Text style={{ fontWeight: "700" }}>
                            {procedure.prosthesis_history.lab_name}
                    ...
                    {procedure.prosthesis_history.failed && (
                      ...
                      >
                        ...
                        {Array.isArray(
                          procedure.prosthesis_history.failure_categories,
                        ) &&
                          procedure.prosthesis_history.failure_categories
                            .length > 0 && (
                            <View
                            ...
                            >
                              {procedure.prosthesis_history.failure_categories.map(
                                (c: string) => (
                                  <View
                                    key={c}
                                    style={{
                        ...
                        {Array.isArray(
                          procedure.prosthesis_history.failure_modes,
                        ) &&
                          procedure.prosthesis_history.failure_modes.length >
                            0 && (
                            <View style={{ marginBottom: 6 }}>
                              ...
                              >
                                {procedure.prosthesis_history.failure_modes.map(
                                  (m: string) => (
                                    <View
                                      key={m}
                                      style={{
                        ...
                        {Array.isArray(
                          procedure.prosthesis_history.suspected_root_causes,
                        ) &&
                          procedure.prosthesis_history.suspected_root_causes
                            .length > 0 && (
                            <View style={{ marginBottom: 6 }}>
                              ...
                              >
                                {procedure.prosthesis_history.suspected_root_causes.map(
                                  (c: string) => (
                                    <View
                                      key={c}
                                      style={{
                        ...
                        {Array.isArray(
                          procedure.prosthesis_history.attachments,
                        ) &&
                          procedure.prosthesis_history.attachments.length >
                            0 && (
                            <View
                              style={{
                                flexDirection: "row",
                            ...
                            >
                              {procedure.prosthesis_history.attachments.map(
                                (url: string, i: number) => (
                                  <View
                                    key={i}
                                    style={{
          ...
          <View
            ...
            testID="treatment-timeline"
            data-testid="treatment-timeline"
          >
            ...
            <View style={styles.timelineSteps}>
              {(() => {
                ...
                if (inc && inc !== sup)
                  approverParts.push(withDoctorTitle(inc));
                const phaseApprover = approverParts.length
                  ? `Approved by ${approverParts.join(" & ")}`
                  : null;
                const allSteps = [
                  {
                    key: "phase1",
                    label: "Phase 1",
                    subtitle: isExistingImplant
                      ? "Examination and Case Details"
                  ...
                  },
                  {
                    ...
                    active: ["phase1_approved", "pending_phase2"].includes(
                      procedure.status,
                    ),
                  ...
                  },
                  {
                    key: "stage2s",
                    label: "Phase 3",
                    subtitle: "Healing and Second Stage Surgery",
                    done: [
                      "stage2_surgical_approved",
                      "pending_stage2_prosthetic",
                      "completed",
                  ...
                  },
                  {
                    key: "stage2p",
                    label: "Phase 4",
                    subtitle: "Prosthetic Rehabilitation",
                ...
                return isExistingImplant
                  ? allSteps.filter((s) => s.key !== "phase2")
                  : allSteps;
                ...
                <View key={step.key} style={styles.timelineStep}>
                  ...
                  <View style={styles.timelineLabelCol}>
                    ...
                    {step.done && step.approver ? (
                      <Text
                        style={styles.timelineApprover}
                        numberOfLines={2}
                        data-testid={`timeline-approver-${step.key}`}
                      >
                        {step.approver}
                      </Text>
                    ) : null}
          ...
          {(() => {
            const isOwner =
              !!(procedure.student_id && user?.id === procedure.student_id) ||
              !!(
            ...
            const waitingMap: Record<string, string> = {
              phase1_approved:
                "Awaiting student to start Phase 2 — Implant Surgery",
              phase2_approved:
                "Awaiting student to start Phase 3 — Healing and Second Stage Surgery",
              stage2_surgical_approved:
                "Awaiting student to start Phase 4 — Prosthetic Rehabilitation",
              stage2_prosthetic_step1_approved:
                "Awaiting student to start Phase 4 Step 2 — Final Restoration",
            };
            ...
            return (
              <View
                ...
                testID="awaiting-next-phase-indicator"
              >
          ...
          {(() => {
            const pending = Array.isArray(procedure?.phase2_edit_requests)
              ? procedure.phase2_edit_requests.find(
            ...
            const canResolve =
              ...
              user?.role === "administrator" ||
              (user?.role === "supervisor" &&
                procedure?.supervisor_id === user?.id);
            if (!canResolve) return null;
            return (
              <View
                style={styles.phase2EditBanner}
                testID="phase2-edit-request-banner"
              >
                <View
                  style={{
                    flexDirection: "row",
                ...
                >
                  ...
                  <Text style={styles.phase2EditBannerTitle}>
                    {pending.requested_by_name || "Student"} requested Phase 2
                    edit
                  </Text>
                </View>
                {!!pending.note && (
                  <Text style={styles.phase2EditBannerNote}>
                    “{pending.note}”
                  </Text>
                )}
                <TouchableOpacity
            ...
            if (canViewOnly && !consentUploaded) {
              return (
                ...
                >
                  <View
                    style={styles.awaitingRow}
                    testID="awaiting-consent-indicator"
                  >
                    <Ionicons
                    ...
                    <Text style={styles.awaitingText}>
                      Awaiting Student/Nurse to upload the patient consent form
                    </Text>
                  </View>
                </View>
              );
              ...
              <View style={styles.consentActionRow} testID="consent-action-row">
                {canUpload && (
                  <TouchableOpacity
                    style={[
                      ...
                      styles.consentActionBtnPrimary,
                      uploadingConsent && styles.buttonDisabled,
                    ]}
                  ...
                  >
                    {uploadingConsent ? (
                      ...
                      <>
                        <Ionicons
                          name={
                            consentUploaded ? "refresh" : "cloud-upload-outline"
                          }
                          size={16}
                          color="#FFF"
                        />
                ...
                {canUpload && !consentUploaded && (
                  <Text style={styles.consentActionHint}>
                    Template is pre-filled with patient & procedure details.
                    Print, get the patient to sign, then tap Upload above.
                  </Text>
                )}
          ...
          {(procedure.status === "pending_phase1" ||
            procedure.status === "pending_phase2" ||
            procedure.status === "pending_stage2_surgical" ||
            procedure.status === "pending_stage2_prosthetic") && (
              ...
              <View style={styles.approvalRow}>
                <Ionicons
                  name={
                    (
                      procedure.status === "pending_phase1"
                        ? procedure.supervisor_phase1_approved
                        : procedure.status === "pending_phase2"
                          ? procedure.supervisor_phase2_approved
                          : procedure.status === "pending_stage2_surgical"
                            ? procedure.supervisor_stage2_surgical_approved
              ...
              <View style={styles.approvalRow}>
                <Ionicons
                  name={
                    (
                      procedure.status === "pending_phase1"
                        ? procedure.implant_incharge_phase1_approved
                        : procedure.status === "pending_phase2"
                          ? procedure.implant_incharge_phase2_approved
                          : procedure.status === "pending_stage2_surgical"
                            ? procedure.implant_incharge_stage2_surgical_approved
                            : procedure.implant_incharge_stage2_prosthetic_approved
                    )
          ...
          {procedure.status === "permanently_rejected" && (
            <View
              ...
              data-testid="permanent-rejection-banner"
            >
              <View
                style={{
                  flexDirection: "row",
              ...
              >
                ...
                <Text
                  style={{ fontSize: 16, fontWeight: "700", color: "#B71C1C" }}
                >
                  Permanently Rejected
                </Text>
              </View>
              <Text style={{ fontSize: 13, color: "#C62828", marginBottom: 6 }}>
                Phase {procedure.rejected_phase?.replace("phase", "") || "?"}{" "}
                was permanently rejected by{" "}
                {procedure.rejected_by ||
                  procedure.phase2_rejected_by ||
              ...
              >
                ...
                <Text style={{ fontSize: 13, color: "#333", marginTop: 2 }}>
                  {procedure.rejection_reason ||
                    procedure.phase2_rejection_reason ||
          ...
          {procedure.rejected_phase &&
            procedure.status !== "permanently_rejected" &&
            (() => {
              const phase = procedure.rejected_phase;
              const isReconsider =
                (phase === "phase1" &&
                  procedure.rejection_type === "reconsider") ||
                (phase === "phase2" &&
                  procedure.phase2_rejection_type === "reconsider") ||
                (phase === "phase3" &&
                  procedure.stage2_surgical_rejection_type === "reconsider") ||
                (phase === "phase4" &&
                  procedure.stage2_prosthetic_rejection_type === "reconsider");
              const reason =
                phase === "phase1"
                  ? procedure.rejection_reason
                  : phase === "phase2"
              ...
              return (
                <View
                  ...
                  data-testid="reconsider-rejection-banner"
                >
                  <View
                    style={{
                      flexDirection: "row",
                  ...
                  <Text
                    style={{ fontSize: 13, color: "#BF360C", marginBottom: 6 }}
                  >
                    Phase {phase.replace("phase", "")} was sent back for
                    revision by {rejBy || "reviewer"}.
                  </Text>
                  <View
                    style={{
                      backgroundColor: "#FFF",
                  ...
                  <Text style={{ fontSize: 11, color: "#888", marginTop: 8 }}>
                    Please make the required changes and resubmit this phase for
                    approval.
                  </Text>
                </View>
              );
              ...
              {procedure.patient_consent_form ? (
                  ...
                  >
                    ...
                    <View style={styles.phase2ButtonTextContainer}>
                      ...
                      <Text style={styles.phase2ButtonSubtitle}>
                        Tap to complete Phase 2 - Implant Surgery
                      </Text>
                    </View>
                  ...
                  {user?.role !== "nurse" &&
                    procedure.instruments_autoclaved?.marked && (
                      <View
                        ...
                        testID="instruments-autoclaved-badge"
                      >
                ...
                <TouchableOpacity
                  style={[
                    styles.phase2Button,
                    {
                      backgroundColor: uploadingConsent ? "#90CAF9" : "#1565C0",
                    },
                  ]}
                  onPress={uploadConsentForProcedure}
                  disabled={uploadingConsent}
          ...
          {canSubmitStage2Prosthetic() && (
            <View style={styles.phase2ButtonContainer}>
              <TouchableOpacity
                ...
                onPress={() =>
                  router.push(`/procedures/submit-stage2-prosthetic/${id}`)
                }
              ...
              >
                ...
                <View style={styles.phase2ButtonTextContainer}>
                  ...
                  {procedure.case_origin === "existing_implants" &&
                  procedure.phase3_skipped ? (
                    <>
                      <Text style={styles.phase2ButtonTitle}>
                        PHASE 1 APPROVED
                      </Text>
                      <Text style={styles.phase2ButtonSubtitle}>
                        Tap to start Phase 4 Step 1 Prosthetic Planning
                      </Text>
                    </>
                  ) : (
                    <>
                      ...
                      <Text style={styles.phase2ButtonSubtitle}>
                        Tap to start Phase 4 Step 1 - Final Prosthesis &
                        Impressions
                      </Text>
                    </>
                  )}
          ...
          {canSubmitPhase4Step2() && (
            <View style={styles.phase2ButtonContainer}>
              <TouchableOpacity
                ...
                onPress={() =>
                  router.push(`/procedures/submit-phase4-step2/${id}`)
                }
              ...
              >
                ...
                <View style={styles.phase2ButtonTextContainer}>
                  ...
                  <Text style={styles.phase2ButtonSubtitle}>
                    Tap to complete Step 2 - Trial & Prosthesis Delivery
                  </Text>
                </View>
                <Ionicons name="chevron-forward" size={24} color="#FFF" />
              </TouchableOpacity>
          ...
          {/* iter-271: Removed the redundant 4-cell `CaseSubmissionStatus`
            widget that used to sit here. The vertical Treatment Progress
            timeline near the top of the page already conveys the same
            ...
            {user?.role === "nurse" ? (
              ...
              <>
                <InfoRow
                  icon="school"
                  label="Supervisor"
                  value={procedure.supervisor_name || procedure.created_by_name}
                />
                <InfoRow
                  icon="medkit"
                  label="Implant Incharge"
              ...
              >
                ...
                {(["maxilla", "mandible"] as const).map((arch) => {
                  ...
                  return (
                    <View
                      key={arch}
                      style={{ marginBottom: 14, padding: 12, backgroundColor: "#FAFCFF", borderRadius: 10 }}
                      data-testid={`atrophy-detail-${arch}`}
                    >
                      <Text style={{ fontSize: 13, fontWeight: "700", color: "#0D47A1", textTransform: "capitalize", marginBottom: 8 }}>
                        {arch}
                      </Text>
                      {(a.anterior_definition || a.posterior_definition) && (
                        <View style={{ backgroundColor: p.bg, borderColor: p.border, borderLeftWidth: 4, borderRadius: 8, padding: 10, marginBottom: 8 }}>
            ...
            Object.keys(procedure.phase2_data).length > 0 && (
              ...
              >
                ...
                <View style={{ marginBottom: 16 }}>
                            ...
                            {procedure.phase2_data.multi_unit_abutment_details.map(
                              (row: any, idx: number) => (
                                ...
                                >
                                  ...
                                  >
                                    <Text
                                      style={{ fontSize: 13, color: "#01579B" }}
                                    >
                  ...
                  {procedure.phase2_data.hemostasis_achieved !== undefined && (
                    <InfoRow
                      ...
                      value={
                        procedure.phase2_data.hemostasis_achieved ? "Yes" : "No"
                      }
                            ...
                            >
                              ...
                              <View style={{ flex: 1 }}>
                                ...
                                <Text
                                  ...
                                  numberOfLines={1}
                                >
                                  {procedure.phase2_data.opg_file.original_name}
                                </Text>
                              </View>
                              <TouchableOpacity
                                style={{
                ...
                {procedure.phase2_data.post_op_checklist &&
                  Object.keys(procedure.phase2_data.post_op_checklist).length >
                    0 && (
                    <View style={{ marginBottom: 16 }}>
                      <Text
                        style={{
          ...
          {(() => {
            ...
            return (
              <View
                style={[
                  styles.section,
                  { borderLeftWidth: 4, borderLeftColor: "#3F51B5" },
                ]}
                testID="ai-surgical-summary-section"
              >
                <View
                  style={{
                    flexDirection: "row",
                ...
                >
                  ...
                  >
                    ...
                    <Text
                      style={[
                        styles.sectionTitle,
                        { marginBottom: 0, color: "#3F51B5", fontSize: 17 },
                      ]}
                    >
                      AI Surgical Summary
                    </Text>
                  </View>
                  {hasNotes && !editingAiSurgical && (
                    <View style={{ flexDirection: "row", gap: 8 }}>
                      <TouchableOpacity
                        onPress={() => {
                          setAiSurgicalDraft(procedure.ai_surgical_notes || "");
                          setEditingAiSurgical(true);
                        }}
                        style={{
                          padding: 6,
                          borderRadius: 8,
                        ...
                        data-testid="ai-surgical-edit-btn"
                      >
                        <Ionicons name="pencil" size={16} color="#3F51B5" />
                      </TouchableOpacity>
                      <TouchableOpacity
                        onPress={() =>
                          Alert.alert(
                            "Regenerate Summary",
                            "This will replace the current AI Surgical Summary with a freshly generated one. Edits you have made will be lost. Continue?",
                            [
                              { text: "Cancel", style: "cancel" },
                              {
                                text: "Regenerate",
                        ...
                        style={{
                          ...
                          borderRadius: 8,
                          backgroundColor: "#E8EAF6",
                          opacity: aiSurgicalLoading ? 0.5 : 1,
                        }}
                ...
                {!hasNotes && phase2Approved && (
                  <TouchableOpacity
                    style={{
                      ...
                      paddingVertical: 14,
                      paddingHorizontal: 20,
                      opacity: aiSurgicalLoading ? 0.7 : 1,
                    }}
                    onPress={generateAiSurgicalSummary}
                    disabled={aiSurgicalLoading}
                ...
                {hasNotes && !editingAiSurgical && (
                  ...
                  >
                    <Text
                      style={{ fontSize: 13, color: "#37474F", lineHeight: 20 }}
                    >
                ...
                {hasNotes && editingAiSurgical && (
                  <View>
                    ...
                    >
                      ...
                      <TouchableOpacity
                        onPress={() =>
                          saveAiSummary("surgical_notes", aiSurgicalDraft)
                        }
                        disabled={aiSavingSurgical}
                        style={{
                          paddingVertical: 10,
                          ...
                          borderRadius: 8,
                          backgroundColor: "#3F51B5",
                          opacity: aiSavingSurgical ? 0.6 : 1,
                        }}
              ...
              >
                ...
                >
                  ...
                  <Text
                    style={[
                      styles.sectionTitle,
                      { marginBottom: 0, color: "#2E7D32", fontSize: 17 },
                    ]}
                  >
                        ...
                        (isArrISQ ? (
                          ...
                          >
                            ...
                            {(isqRaw as any[]).map((val: any, idx: number) => {
                              ...
                              const toothLabel = procedure.implant_plans?.[idx]
                                ?.position
                  ...
                  <View style={{ marginBottom: 16 }}>
                    ...
                    {procedure.phase4_step1_data.prosthetic_material && (
                      <InfoRow
                        ...
                        value={procedure.phase4_step1_data.prosthetic_material}
            ...
            [
              ...
              >
                ...
                <TouchableOpacity
                  ...
                  disabled={
                    labSlipNote.trim().split(/\s+/).filter(Boolean).length > 150
                  }
                  onPress={async () => {
                    try {
              ...
              {!rejectionType ? (
                <>
                  ...
                  >
                    ...
                    <View style={{ flex: 1 }}>
                      <Text style={styles.rejectTypeBtnText}>
                        Reject Permanently
                      </Text>
                      <Text style={styles.rejectTypeDesc}>
                        Case will be stopped. No further phases can proceed.
                      </Text>
                    </View>
                  </TouchableOpacity>
                  <TouchableOpacity
                  ...
                  >
                    ...
                    <View style={{ flex: 1 }}>
                      <Text style={styles.rejectTypeBtnText}>
                        Reject with Consideration
                      </Text>
                      <Text style={styles.rejectTypeDesc}>
                        Student can edit the rejected phase and resubmit.
                      </Text>
                    </View>
                  </TouchableOpacity>
                  <TouchableOpacity
                ...
                <>
                  ...
                  >
                    <Ionicons
                      ...
                      size={20}
                      color={
                        rejectionType === "permanent" ? "#D32F2F" : "#F57C00"
                      }
                    />
                  ...
                  <View
                    style={{
                      backgroundColor:
                        rejectionType === "permanent" ? "#FFEBEE" : "#FFF3E0",
                      padding: 10,
                      borderRadius: 8,
                      marginBottom: 12,
                    }}
                  >
                    <Text
                      style={{
                        fontSize: 12,
                        color:
                          rejectionType === "permanent" ? "#C62828" : "#E65100",
                      }}
                    >
                      {rejectionType === "permanent"
                  ...
                  <TextInput
                    ...
                    placeholder={
                      rejectionType === "permanent"
                        ? "Enter reason for permanent rejection"
                        : "Enter reason for reconsideration"
                    }
                    multiline
                    numberOfLines={4}
                    ...
                    <TouchableOpacity
                      style={[
                        rejectionType === "permanent"
                          ...
                          : styles.dialogReconsiderConfirmBtn,
                        actionLoading && styles.buttonDisabled,
                      ]}
          ...
          {Array.isArray(procedure.implant_edit_history) &&
            procedure.implant_edit_history.length > 0 && (
              <View
                style={resStyles.editHistoryWrap}
                data-testid="implant-edit-history"
              >
                <View style={resStyles.editHistoryHeader}>
                  <Ionicons
                    ...
                    color="#1565C0"
                  />
                  <Text style={resStyles.editHistoryTitle}>
                    Implant edit history
                  </Text>
                  <View style={resStyles.editHistoryCountChip}>
                    <Text style={resStyles.editHistoryCountTxt}>
                      {procedure.implant_edit_history.length}
                    </Text>
                  </View>
                </View>
                {procedure.implant_edit_history
                  .slice()
                  .reverse()
                  .map((h: any, idx: number) => (
                    <View key={h.id || idx} style={resStyles.editHistoryItem}>
                      <View style={resStyles.editHistoryItemHeader}>
                        <Text style={resStyles.editHistoryPosition}>
                          Tooth #{h.position}
                        </Text>
                        <View
                          style={[
                            resStyles.editHistoryKindChip,
                            h.kind === "added"
                              ? resStyles.kindAdded
                              : h.kind === "removed"
                                ? resStyles.kindRemoved
                                : resStyles.kindEdited,
                          ]}
                        >
                        ...
                        <View style={resStyles.editChangesList}>
                          {Object.entries(h.changes).map(
                            ([field, diff]: [string, any]) => (
                              <Text
                                key={field}
                                style={resStyles.editChangeLine}
                              >
                                <Text style={{ fontWeight: "700" }}>
                                  {field.replace(/_/g, " ")}:
                                </Text>
                                <Text style={resStyles.editChangeFrom}>
                                  {" "}
                                  {String(diff.from ?? "—")}
                                </Text>
                                <Text> → </Text>
                                <Text style={resStyles.editChangeTo}>
                                  {String(diff.to ?? "—")}
                                </Text>
                              </Text>
                            ),
                      ...
                      h.snapshot ? (
                        <Text style={resStyles.editChangeLine}>
                          {h.snapshot.brand} {h.snapshot.system} · D{" "}
                          {h.snapshot.diameter}mm × L {h.snapshot.length}mm
                        </Text>
                      ) : null}
          ...
          {(procedure.ai_case_summary || aiSummary) && !editingAiClinical ? (
            ...
            >
              ...
              >
                ...
                {user?.role !== "nurse" && procedure.ai_case_summary && (
                  <View style={{ flexDirection: "row", gap: 8 }}>
                    <TouchableOpacity
                      onPress={() => {
                        setAiClinicalDraft((procedure.ai_case_summary || "").replace(/\*\*(.*?)\*\*/g, "$1").replace(/(?<!\w)\*(.+?)\*(?!\w)/g, "$1"));
                        setEditingAiClinical(true);
                      }}
                      style={{ padding: 4 }}
              ...
              <Text style={{ fontSize: 13, color: "#37474F", lineHeight: 20 }}>
                {(procedure.ai_case_summary || aiSummary || "").replace(/\*\*(.*?)\*\*/g, "$1").replace(/(?<!\w)\*(.+?)\*(?!\w)/g, "$1")}
              </Text>
            </View>
          ) : null}
          ...
          {editingAiClinical && (
            ...
            >
              ...
              >
                <Ionicons name="sparkles" size={16} color="#3F51B5" />
                <Text
                  style={{ fontSize: 14, fontWeight: "700", color: "#3F51B5" }}
                >
                  Edit AI Clinical Summary
                </Text>
              </View>
              <TextInput
                style={{
              ...
              >
                ...
                <TouchableOpacity
                  ...
                  style={{
                    ...
                    borderRadius: 8,
                    backgroundColor: "#3F51B5",
                    opacity: aiSavingClinical ? 0.6 : 1,
                  }}
          ...
          {procedure.last_edited_by && procedure.last_edited_at && (
            ...
            >
              ...
              <Text style={styles.editHistoryText}>
                Last edited by{" "}
                <Text style={styles.editHistoryName}>
                  {procedure.last_edited_by}
                </Text>
                {" on "}
                <Text style={styles.editHistoryDate}>
                  {format(
                    new Date(procedure.last_edited_at),
                    "MMM dd, yyyy • hh:mm a",
                  )}
                </Text>
              </Text>
          ...
          <View
            style={{ height: canExportPDF() || canViewAiSummary() ? 70 : 10 }}
          />
        </ScrollView>
          ...
          >
            ...
            >
              ...
              >
                ...
                <TouchableOpacity
                  onPress={() => setShowInchargeConsentEdit(false)}
                  style={{ padding: 4 }}
                  testID="close-incharge-consent-edit"
                >
              ...
              <View style={{ padding: 16, gap: 10 }}>
                <TouchableOpacity
                  ...
                  onPress={async () => {
                    ...
                    await downloadConsentTemplate(id as string);
                  }}
                  activeOpacity={0.85}
                ...
                >
                  Any replacement is logged in the case's edit history with your
                  name and timestamp.
                </Text>
              </View>
            </View>
            ...
            >
              ...
              >
                ...
                <TouchableOpacity
                  ...
                  style={{ padding: 4 }}
                  data-testid="close-edit-history"
                >
                  <Ionicons name="close-circle" size={28} color="#78909C" />
                </TouchableOpacity>
              </View>
              ...
              <ScrollView contentContainerStyle={{ padding: 16 }}>
                {(procedure.edit_log?.length || 0) === 0 ? (
                  <View style={{ alignItems: "center", padding: 30 }}>
                    ...
                    >
                      No detailed edit history yet. Future edits will be logged
                      here.
                    </Text>
                  </View>
                ) : (
                  [...procedure.edit_log]
                    ...
                    .map((entry: any, idx: number) => {
                      ...
                      const roleLabel =
                        entry.edited_by_role === "implant_incharge"
                          ? "Implant In-Charge"
                          : entry.edited_by_role === "supervisor"
                            ? "Supervisor"
                            : entry.edited_by_role === "student"
                              ? "Student"
                              : "";
                      return (
                        <View
                          ...
                          data-testid={`edit-log-entry-${idx}`}
                        >
                          {/* Timeline dot + line */}
                          <View
              ...
              {canViewAiSummary() && (
                <TouchableOpacity
                  style={[
                    ...
                    aiSummaryLoading && styles.buttonDisabled,
                  ]}
                  onPress={async () => {
                    setAiSummaryLoading(true);
                    try {
                      await api.post("/ai/case-summary", {
                        procedure_id: procedure.id || procedure._id,
                      });
                      await loadProcedure();
                    } catch (e: any) {
                      Alert.alert(
                ...
                >
                  {aiSummaryLoading ? (
                    ...
                    <>
                      ...
                      <Text style={styles.barButtonTextCompact}>
                        {procedure.ai_case_summary ? "REGEN AI" : "AI SUMMARY"}
                      </Text>
                    </>
                  )}
                </TouchableOpacity>
          ...
          >
            ...
            >
              ...
              >
                ...
                <TouchableOpacity
                  onPress={() => setAiChatVisible(false)}
                ...
                {aiChatHistory.length === 0 && (
                  <View style={{ alignItems: "center", paddingVertical: 30 }}>
                    ...
                    >
                      ...
                      immediately load?" or "Is grafting needed?"
                    </Text>
                  </View>
                )}
              ...
              >
                ...
                <TouchableOpacity
                  style={{
                    ...
                    borderRadius: 21,
                    backgroundColor:
                      aiChatSending || !aiChatInput.trim()
                        ? "#B0BEC5"
                        : "#0D47A1",
                    justifyContent: "center",
                    alignItems: "center",
        ...
        {procedure && (
          <Phase2EditModal
            ...
            procedure={procedure}
            request={
              Array.isArray(procedure.phase2_edit_requests)
                ? procedure.phase2_edit_requests.find(
                    (r: any) => r?.status === "pending",
                  )
                : null
...
function InfoRow({
  ...
  const editValue =
    editValueProp ?? (useCtx ? (editCtx.editValues[key] ?? "") : "");
  ...
  const pickerCfg =
    useCtx && isEditing ? resolveFieldOptions(key, editCtx.procedure) : null;

  // Normalize options to {value, label}[]
  const normOptions = pickerCfg
    ? (pickerCfg.options as any[]).map((o) =>
                  ...
                  {normOptions.map((opt) => {
                    ...
                    return (
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          if (pickerCfg.multi) {
                            const curr = Array.isArray(editValue)
                              ? editValue
                              : [];
