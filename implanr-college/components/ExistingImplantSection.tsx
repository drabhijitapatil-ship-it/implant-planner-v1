...
function getProsthesisTypeOptions(
  originalProcedure: string,
  stage: "temporary" | "final",
  implantCount: number = 0,
): string[] {
  // Full arch beats everything.
  if (FULL_ARCH_PROCEDURES.has(originalProcedure)) {
    return stage === "temporary"
      ? TEMP_FULL_ARCH_OPTIONS
      : FINAL_FULL_ARCH_OPTIONS;
  }
  ...
  if (COUNT_DRIVEN_PROCEDURES.has(originalProcedure)) {
    ...
    if (stage === "temporary")
      return isMulti ? TEMP_MULTIPLE_OPTIONS : TEMP_SINGLE_OPTIONS;
  ...
  if (stage === "temporary") {
    ...
    if (MULTIPLE_PROCEDURES.has(originalProcedure))
      return TEMP_MULTIPLE_OPTIONS;
  } else {
    if (SINGLE_PROCEDURES.has(originalProcedure)) return FINAL_SINGLE_OPTIONS;
    if (MULTIPLE_PROCEDURES.has(originalProcedure))
      return FINAL_MULTIPLE_OPTIONS;
  }
  return [PROSTHESIS_TYPE_OTHER];
}

function getProsthesisDefaults(
  originalProcedure: string,
  stage: "temporary" | "final",
  implantCount: number = 0,
): { type: string; material: string } {
  // Default Type = first non-"Other" option from the institution-curated list.
  const opts = getProsthesisTypeOptions(
    originalProcedure,
    stage,
    implantCount,
  ).filter((o) => o !== PROSTHESIS_TYPE_OTHER);
  const type = opts[0] || "";
...
export default function ExistingImplantSection({
  patient,
  validatePatient,
  draft,
  onOriginalProcedureChange,
  onImplantTeethChange,
  arch,
  onArchChange,
  extraSubmitFields,
  hideActionButtons,
    ...
    (async () => {
      try {
        ...
        if (catRes.status === "fulfilled") {
          const sysl = (catRes.value.data?.systems || []).filter(
            (s: any) => !s.is_stub && !s.is_shared_instruments_doc,
          );
          setCatalog(sysl);
        }
  ...
  const systemsForBrand = (brand: string) => {
    ...
    const libSys = library
      .filter((s) => s.brand === brand)
      .map((s) => s.system);
    const catSys = catalog
      .filter((s: any) => s.brand === brand)
      .map((s: any) => s.name);
    return Array.from(new Set([...libSys, ...catSys].filter(Boolean))).sort();
  };
  const lookupLibSystem = (brand: string, system: string) =>
    library.find((s) => s.brand === brand && s.system === system);
  const lookupCatalog = (brand: string, system: string) =>
    catalog.find((s: any) => s.brand === brand && s.name === system);
  ...
  const diametersForSystem = (brand: string, system: string): string[] => {
    ...
    const fromCat: number[] =
      (cat as any)?.implant?.diameters_mm || (cat as any)?.diameters_mm || [];
    const merged = Array.from(new Set([...fromLib, ...fromCat])).sort(
  ...
  const lengthsForSystem = (brand: string, system: string): string[] => {
    ...
    const cat = lookupCatalog(brand, system);
    const fromCat: number[] =
      (cat as any)?.implant?.lengths_mm || (cat as any)?.lengths_mm || [];
    const merged = Array.from(new Set([...fromLib, ...fromCat])).sort(
      (a: any, b: any) => Number(a) - Number(b),
    );
    return merged.map((v) => String(v));
  ...
  const [prosthesisStage, setProsthesisStage] = useState<
    "" | "temporary" | "final"
  >("");
  ...
  const [uploadingIdx, setUploadingIdx] = useState<number | string | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const op =
      draft.original_procedure_type || draft.implant_procedure_type || "";
    setOriginalProcedure(op);
    const isFA = FULL_ARCH_DONE.has(op);
    const savedImpls: any[] = Array.isArray(draft.existing_implants)
      ? draft.existing_implants
      : [];
    const rows: ImplantRow[] = savedImpls.map((r: any) => ({
      ...
      gingival_height_mm:
        r.gingival_height_mm != null ? String(r.gingival_height_mm) : "",
      present_component: (r.present_component || "None") as PresentComponent,
      pc_gingival_height:
        r.present_component_gh != null ? String(r.present_component_gh) : "",
      pc_angle:
        r.present_component_angle != null
          ? String(r.present_component_angle)
          : "",
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      ...
      (window as any).__RESET_EFFECT__.push({
        originalProcedure,
        draftId: draft?.id,
        hydratedRefMatches: !!(
          draft?.id && hydratedDraftId.current === draft.id
        ),
        ts: Date.now(),
      });
    }
  ...
  useEffect(() => {
    ...
    setImplants((prev) => {
      const byTooth = new Map(
        prev.filter((r) => r.tooth).map((r) => [r.tooth, r]),
      );
    ...
    setImplants((prev) =>
      prev.map((r, i) => {
        ...
        if ((key === "system" || key === "brand") && !next.manual_brand) {
          ...
          const hit = lookupCatalog(
            key === "brand" ? val : next.brand,
            key === "system" ? val : next.system,
          );
          if (hit) {
            ...
            const rawPlat =
              (hit as any).platform || (hit as any).implant?.platform || "";
            let plat = "";
            if (rawPlat && typeof rawPlat === "object") {
              plat = String(
                rawPlat.type || rawPlat.name || rawPlat.subtype || "",
              ).trim();
            } else if (rawPlat) {
              plat = String(rawPlat);
            }
  ...
  const addImplant = () => {
    if (implants.some((r) => !r.saved)) {
      Alert.alert(
        "Save the current implant first",
        "Please save the current implant before adding another one.",
      );
      return;
    }
  ...
  const removeImplant = (idx: number) => {
    if (implants.length === 1) {
      Alert.alert(
        "At least one implant required",
        "Add details for the implant in this row.",
      );
      return;
    }
  ...
  const validateRow = (r: ImplantRow, idx: number): string | null => {
    ...
    if (!r.gingival_height_mm)
      return `Gingival height is required for Implant #${idx + 1}.`;
    if (!r.present_component)
      return `Present Prosthetic Component is required for Implant #${idx + 1}.`;
    ...
    if (
      (r.present_component === "Final Abutment" ||
        r.present_component === "Multi-Unit Abutment") &&
      !r.pc_angle
    ) {
      return `Angle is required for Implant #${idx + 1} (${r.present_component}).`;
    }
    if (!r.surgery_date)
      return `Surgery date is required for Implant #${idx + 1}.`;
    if (!r.original_surgeon)
      return `Original surgeon is required for Implant #${idx + 1}.`;
    if (!isFullArchDone && !r.iopa_url)
      return `IOPA Radiograph / CBCT upload is required for Implant #${idx + 1}.`;
    return null;
  };
  const saveImplantRow = (idx: number) => {
    ...
    setImplants((prev) =>
      prev.map((r, i) => (i === idx ? { ...r, saved: true } : r)),
    );
  };
  const editImplantRow = (idx: number) => {
    setImplants((prev) =>
      prev.map((r, i) => (i === idx ? { ...r, saved: false } : r)),
    );
  };
  ...
  const [pendingPick, setPendingPick] = useState<{
    source: "library" | "camera" | "pdf";
    target: { kind: "iopa" | "opg"; idx?: number };
  } | null>(null);
  ...
  const uploadBlob = async (
    ...
    try {
      ...
      const up = await api.post("/uploads/media-temp", fd, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      const filename = up.data?.filename;
      if (!filename) throw new Error("Upload returned no filename");
      if (target.kind === "iopa" && typeof target.idx === "number")
        updateImplant(target.idx, "iopa_url", filename);
      else setOpgUrl(filename);
    } catch (e: any) {
      Alert.alert(
        "Upload failed",
        e?.response?.data?.detail || e?.message || "Could not upload the file.",
      );
    } finally {
  ...
  const pickFromLibraryInternal = async (target: {
    kind: "iopa" | "opg";
    idx?: number;
  }) => {
    try {
      ...
      await uploadBlob(
        a.uri,
        a.fileName || `${target.kind}-${Date.now()}.jpg`,
  ...
  const takePhotoInternal = async (target: {
    kind: "iopa" | "opg";
    idx?: number;
  }) => {
    try {
  ...
  const pickPdfInternal = async (target: {
    kind: "iopa" | "opg";
    idx?: number;
  }) => {
    try {
      ...
      await uploadBlob(
        a.uri,
        a.name || `${target.kind}-${Date.now()}.pdf`,
  ...
  const validate = (): string | null => {
    const pErr = validatePatient();
    if (pErr) return pErr;
    if (!originalProcedure)
      return "Please pick the Type of Implant Procedure Done.";
    // iter-239: full-arch case Arch picker is now compulsory before submission.
    if (isFullArchDone && !arch)
      return "Please pick the Arch (Maxillary, Mandibular, or Both).";
    if (missingTeeth.length === 0)
      return "Mark at least one tooth on the FDI chart.";
    ...
    if (isFullArchDone && !opgUrl)
      return "Please upload the Radiograph (OPG / CBCT) for the case.";
    if (hadProsthesis === null)
      return "Please answer the Prosthetic History question.";
    if (hadProsthesis === true && !prosthesisStage)
      return "Pick Temporary or Final for the existing prosthesis.";
    ...
    if (
      !exf.occlusocervical_height ||
      !exf.mesiodistal_space ||
      !exf.ridge_contour
    ) {
    ...
    if (
      !exf.diabetes ||
      !exf.smoking_status ||
      !exf.anticoagulant_therapy ||
      !exf.osteoporosis_medication ||
      !exf.radiation_therapy
    ) {
    ...
    try {
      ...
      const payload: any = {
        ...
        existing_implants: implants.map((r) => ({
          ...
          gingival_height_mm: r.gingival_height_mm
            ? parseFloat(r.gingival_height_mm)
            : null,
          ...
          present_component_gh: r.pc_gingival_height
            ? parseFloat(r.pc_gingival_height)
            : null,
      ...
      Alert.alert(
        ...
        `Case ${res.data?.case_id ? `(${res.data.case_id}) ` : ""}created. ${
          phaseToStart === "phase3"
            ? "Routed to Phase 3 inbox."
            : phaseToStart === "phase4_step1"
              ? "Routed to Phase 4 Step 1 inbox."
              : "Saved as draft."
        }`,
        [
          {
            text: "Open case",
            onPress: () => router.replace(`/procedures/${res.data.id}`),
          },
        ],
      );
    } catch (e: any) {
  ...
  return (
    <View
      testID="existing-implant-section"
      /* @ts-ignore */ data-testid="existing-implant-section"
    >
      <View style={styles.headerCard}>
        <Ionicons name="information-circle" size={20} color="#1565C0" />
        <Text style={styles.headerText}>
          Patient already has implants placed. Surgical phases will be skipped —
          choose Phase 3 or Phase 4 Step 1 below to route the case.
        </Text>
      </View>
      ...
      {originalProcedure && (
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>
            Mark Existing Implant Position(s) *
          </Text>
          <Text style={styles.helperText}>
            Tap each tooth that already has an implant. One Implant Selection
            card will be added per tooth below.
          </Text>
          <FdiAnatomicalChart
            mode="multi"
            value={missingTeeth}
          ...
          {missingTeeth.length > 0 && (
            <Text style={styles.fdiSummary}>
              {missingTeeth.length}{" "}
              {missingTeeth.length === 1 ? "tooth" : "teeth"} marked —{" "}
              {[...missingTeeth].sort().join(", ")}
            </Text>
          )}
        </View>
      )}
      ...
      {originalProcedure && missingTeeth.length > 0 && (
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>
            Implant Selection ({implants.length})
          </Text>
          {implants.map((row, idx) => {
            ...
            const showAngle =
              row.present_component === "Final Abutment" ||
              row.present_component === "Multi-Unit Abutment";
            ...
            const isLockedAhead =
              !row.saved && activeIdx >= 0 && idx > activeIdx;
            ...
            if (isCollapsedSaved) {
              const summary = [
                ...
                row.brand && row.system
                  ? `${row.brand} · ${row.system}`
                  : row.brand || null,
                row.diameter_mm && row.length_mm
                  ? `Ø${row.diameter_mm} × ${row.length_mm} mm`
                  : null,
                row.gingival_height_mm
                  ? `GH ${row.gingival_height_mm} mm`
                  : null,
              ]
                .filter(Boolean)
                .join(" · ");
              return (
                <View
                  key={`saved-${idx}-${row.tooth}`}
                  style={styles.implantCardSaved}
                  testID={`ei-row-${idx}`}
                >
                  <View
                    style={{
                  ...
                  <View style={{ flexDirection: "row", gap: 6 }}>
                    ...
                    <TouchableOpacity
                      onPress={() => removeImplant(idx)}
                      hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
                      style={styles.iconBtn}
                      testID={`ei-delete-${idx}`}
                    >
                      <Ionicons
                        name="trash-outline"
                        size={20}
            ...
            if (isLockedAhead) {
              return (
                <View
                  key={`locked-${idx}-${row.tooth}`}
                  style={styles.implantCardLocked}
                  testID={`ei-row-${idx}`}
                >
                  <Ionicons
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={styles.lockedTitle}>
                      Implant #{idx + 1}
                      {row.tooth ? ` · FDI #${row.tooth}` : ""}
                    </Text>
                    <Text style={styles.lockedHint}>
                      Save the previous implant to unlock this card.
                    </Text>
                  </View>
                </View>
              );
              ...
              >
                ...
                <View style={styles.fieldContainer}>
                  ...
                  {row.manual_brand ? (
                    <View style={{ gap: 6 }}>
                      ...
                      <TouchableOpacity
                        onPress={() =>
                          updateImplant(idx, "manual_brand", false as any)
                        }
                        style={{
                ...
                <View style={styles.fieldContainer}>
                  ...
                  {row.manual_brand ? (
                    ...
                    <DropDown
                      ...
                      placeholder={
                        row.brand ? "Select implant system" : "Pick brand first"
                      }
                      disabled={!row.brand}
                ...
                <View style={styles.fieldContainer}>
                  ...
                  {diaOpts.length > 0 ? (
                    ...
                    <TextInput
                      ...
                      placeholder={
                        row.system ? "e.g. 4.2" : "Pick system to see options"
                      }
                      testID={`ei-d-${idx}`}
                    />
                  )}
                ...
                <View style={styles.fieldContainer}>
                  ...
                  {lenOpts.length > 0 ? (
                    ...
                    <TextInput
                      ...
                      placeholder={
                        row.system ? "e.g. 11.5" : "Pick system to see options"
                      }
                      testID={`ei-l-${idx}`}
                    />
                  )}
                ...
                <View style={styles.fieldContainer}>
                  ...
                  <TextInput
                    style={styles.input}
                    keyboardType="decimal-pad"
                    value={row.gingival_height_mm}
                    onChangeText={(t) =>
                      updateImplant(idx, "gingival_height_mm", t)
                    }
                    placeholder="e.g. 2"
                    testID={`ei-gh-${idx}`}
                  />
                ...
                <View style={styles.fieldContainer}>
                  <Text style={styles.label}>
                    ISQ Value <Text style={styles.optional}>(optional)</Text>
                  </Text>
                  <TextInput
                    style={styles.input}
                    keyboardType="decimal-pad"
                ...
                <View style={styles.chipRow}>
                  {PRESENT_COMPONENTS.map((pc) => (
                    <Chip
                      key={pc}
                      label={pc}
                      active={row.present_component === pc}
                      onPress={() =>
                        updateImplant(idx, "present_component", pc)
                      }
                ...
                {showGH && (
                  <View style={[styles.fieldContainer, styles.conditionalGap]}>
                    <Text style={styles.label}>
                      Cuff / Gingival Height (mm) *
                    </Text>
                    <TextInput
                      style={styles.input}
                      keyboardType="decimal-pad"
                      value={row.pc_gingival_height}
                      onChangeText={(t) =>
                        updateImplant(idx, "pc_gingival_height", t)
                      }
                      placeholder="e.g. 2"
                      testID={`ei-pcgh-${idx}`}
                    />
                ...
                <View style={styles.fieldContainer}>
                  ...
                  <TextInput
                    style={styles.input}
                    value={row.original_surgeon}
                    onChangeText={(t) =>
                      updateImplant(idx, "original_surgeon", t)
                    }
                ...
                {!isFullArchDone && (
                  <View style={styles.fieldContainer}>
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() => setPicker({ kind: "iopa", idx })}
                    ...
                    {row.iopa_url ? (
                      <RadiographThumb
                        filename={row.iopa_url}
                        testID={`ei-iopa-thumb-${idx}`}
                      />
                    ) : null}
                  </View>
                )}
        ...
        <View style={styles.section}>
          ...
          <View style={styles.yesNoRow}>
            <TouchableOpacity
              style={[
                styles.yesNoBtn,
                hadProsthesis === true && styles.yesActive,
              ]}
            ...
            >
              <Text
                style={[
                  styles.yesNoText,
                  hadProsthesis === true && styles.yesNoTextActive,
                ]}
              >
                Yes
              </Text>
            ...
            <TouchableOpacity
              style={[
                styles.yesNoBtn,
                hadProsthesis === false && styles.noActive,
              ]}
              onPress={() => {
                setHadProsthesis(false);
                setProsthesisStage("");
                setProsthesisType("");
                setProsthesisMaterial("");
            ...
            >
              <Text
                style={[
                  styles.yesNoText,
                  hadProsthesis === false && styles.yesNoTextActive,
                ]}
              >
                No
              </Text>
          ...
          {hadProsthesis === true && (
            <View style={{ marginTop: 14 }}>
              ...
              <View style={styles.chipRow}>
                <Chip
                  label="Temporary"
                  active={prosthesisStage === "temporary"}
                  onPress={() => {
                    setProsthesisStage("temporary");
                    setManualProsthesisType(false);
                    const count = isFullArchDone
                      ? implants.length
                      : missingTeeth.length;
                    const def = getProsthesisDefaults(
                      originalProcedure,
                      "temporary",
                      count,
                    );
                ...
                <View style={{ marginTop: 16 }}>
                  <View style={styles.fieldContainer}>
                    ...
                    {manualProsthesisType ? (
                      ...
                      <DropDown
                        value={prosthesisType}
                        options={getProsthesisTypeOptions(
                          originalProcedure,
                          prosthesisStage,
                          isFullArchDone
                            ? implants.length
                    ...
                    <Text style={styles.helperHint}>
                      Pre-filled based on the original procedure type & number
                      of implants. Edit if needed.
                    </Text>
                  </View>
                </View>
              ) : null}
          ...
          >
            {uploadingIdx === "opg" ? (
              ...
              <>
                ...
                />
                <Text style={styles.uploadText}>
                  {opgUrl ? "Re-upload" : "Upload"}
                </Text>
              </>
            )}
          </TouchableOpacity>
          {opgUrl ? (
            <RadiographThumb filename={opgUrl} testID="ei-opg-thumb" />
          ) : null}
        </View>
      )}
          ...
          >
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.chooserHint}>
                  JPG / PNG / HEIC from your photo library
                </Text>
              </View>
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.chooserHint}>
                  Capture a fresh photo with the camera
                </Text>
              </View>
              <Ionicons name="chevron-forward" size={18} color="#90A4AE" />
            </TouchableOpacity>
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.chooserHint}>
                  Select a PDF radiograph report
                </Text>
              </View>
              <Ionicons name="chevron-forward" size={18} color="#90A4AE" />
            </TouchableOpacity>
            ...
            >
              <Text style={{ color: "#90A4AE", fontWeight: "600" }}>
                Cancel
              </Text>
            </TouchableOpacity>
          </Pressable>
                ...
                options.map((o, i) => (
                  ...
                  >
                    <Text
                      style={[
                        styles.modalRowText,
                        value === o && styles.modalRowTextActive,
                      ]}
                    >
                      {o}
                    </Text>
  ...
  return (
    ...
    >
      {isPdf || !uri ? (
        ...
        >
          <Ionicons
            name={isPdf ? "document-text" : "image-outline"}
            size={28}
            color="#2E7D32"
          />
        </View>
      ...
      <View style={{ flex: 1 }}>
        <Text
          style={{ fontSize: 12, fontWeight: "700", color: "#1B5E20" }}
          numberOfLines={1}
        >
          ✓ Uploaded
        </Text>
        <Text style={{ fontSize: 11, color: "#558B2F" }} numberOfLines={1}>
          {filename}
        </Text>
        <Text
          style={{
...
function SurgeryDatePicker({
  ...
  return (
    <NativeCalendarTrigger value={value} onChange={onChange} testID={testID} />
  );
}

function NativeCalendarTrigger({
  value,
  onChange,
  testID,
}: {
        ...
        <Pressable style={styles.modalBackdrop} onPress={() => setOpen(false)}>
          ...
          >
            ...
            >
              ...
              <Text
                style={{ fontSize: 16, fontWeight: "700", color: "#0D47A1" }}
              >
  ...
  return (
    <View>
      ...
      >
        <Pressable style={styles.modalBackdrop} onPress={() => setOpen(false)}>
          <Pressable
            style={[styles.modalCard, { padding: 14, maxWidth: "95%" }]}
            onPress={(e) => e.stopPropagation()}
          >
