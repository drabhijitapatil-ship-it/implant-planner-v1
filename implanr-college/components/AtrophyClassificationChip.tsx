        ...
        >
          {result.treatment_options.map((opt, i) => (
            <View
              ...
              style={{
                marginBottom:
                  i === result.treatment_options!.length - 1 ? 0 : 8,
              }}
            >
              <Text
                style={{ fontSize: 12, fontWeight: "700", color: palette.fg }}
              >
                Treatment Option {i + 1}: {opt.implant_count} implants (
                {opt.kind})
              </Text>
              <Text style={{ fontSize: 11, color: "#37474F", marginTop: 2 }}>
                {opt.placement}
              </Text>
              {opt.tilt && opt.tilt !== "—" && (
                <Text
